...
启动命令:
    python3 start_poseidon_simple.py
...
# 注册 8 个 Marine Channels
for channel_name in ["nmea_parser", "vessel_ais", "engine_monitor", "power_management", 
                      "navigation_data", "cargo_monitor", "weather_routing", "web"]:
    channels.register_channel(channel_name, SimpleChannel(channel_name, f"{channel_name} data"))
print("✅ 8 个 Marine Channels 已注册")
    ...
    def do_POST(self):
        ...
        if path.startswith('/api/data/'):
            ...
            try:
                ...
                channels.update_data(channel_name, data)
                self._set_headers()
