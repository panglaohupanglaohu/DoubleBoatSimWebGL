...
// ============= WebGL 错误处理 =============
function _showWebGLError(error) {
  const errorMsg = `
    <div style="padding: 20px; max-width: 600px; line-height: 1.6;">
      <h2 style="color: #ff6b6b; margin-bottom: 15px;">⚠️ WebGL 初始化失败 | WebGL Initialization Failed</h2>
      <p style="margin-bottom: 15px; color: #333;">
        <strong>错误原因 | Error:</strong> ${error.message || '无法创建 WebGL 上下文 | Could not create WebGL context'}
      </p>
      <div style="background: #f5f5f5; padding: 15px; border-radius: 5px; margin-bottom: 15px;">
        <strong>可能的解决方案 | Possible Solutions:</strong>
...
function _setupRenderer() {
  ...
  // 注意：Three.js r165 中，physicallyCorrectLights 已废弃，使用 toneMapping 代替
    ...
    if (errorMessage.includes('WebGL') || errorMessage.includes('context')) {
      ...
      if (!webgl2 && !webgl) {
        errorMessage = '您的浏览器或系统不支持 WebGL | Your browser or system does not support WebGL';
      } else if (!webgl2) {
        errorMessage = '您的浏览器不支持 WebGL 2.0（Three.js r163+ 需要 WebGL 2.0）| Your browser does not support WebGL 2.0 (Three.js r163+ requires WebGL 2.0)';
      }
    }
...
function setupLights() {
  // 大幅增强环境光，确保透明船体在任何缩放级别都可见
  hemiLight = new THREE.HemisphereLight(0xaadfff, 0x0b1120, 6.0); // 增强到6.0，提供充足的基础照明
  scene.add(hemiLight);

  // 主方向光（大幅增强强度，确保透明材质可见）
  dirLight = new THREE.DirectionalLight(0xffffff, 6.0); // 增强到6.0
  dirLight.position.set(500, 400, 300); // 提高位置，扩大覆盖范围
  dirLight.castShadow = true;
  dirLight.shadow.mapSize.set(4096, 4096);
  
  // 存储原始强度，用于根据天气调整
  dirLight.userData.originalIntensity = 6.0; // 更新原始强度
  
  // 扩大阴影相机范围以覆盖500米距离
  dirLight.shadow.camera.near = 1;
  dirLight.shadow.camera.far = 1000; // 增加到1000米
  dirLight.shadow.camera.left = -600; // 扩大左右范围
  dirLight.shadow.camera.right = 600;
  dirLight.shadow.camera.top = 600; // 扩大上下范围
  dirLight.shadow.camera.bottom = -600;
  scene.add(dirLight);

  // 补充方向光（从不同角度，增强远距离可见性）
  const dirLight2 = new THREE.DirectionalLight(0xffffff, 3.0); // 增强到3.0
  dirLight2.position.set(-300, 250, -200);
  scene.add(dirLight2);

  // 第三个方向光（从另一角度补充）
  const dirLight3 = new THREE.DirectionalLight(0xffffff, 2.5); // 增强到2.5
  dirLight3.position.set(0, 300, -400);
  scene.add(dirLight3);

  // 中心点光源（增强亮度和范围，覆盖500米）
  const pointLight = new THREE.PointLight(0xffffff, 3.0, 1000); // 强度3.0，范围1000米
  pointLight.position.set(0, 100, 0);
  scene.add(pointLight);

  // 额外的点光源（在场景边缘，增强远距离照明）
  const pointLight2 = new THREE.PointLight(0xffffff, 2.0, 800);
  pointLight2.position.set(300, 150, 300);
  scene.add(pointLight2);

  const pointLight3 = new THREE.PointLight(0xffffff, 2.0, 800);
  pointLight3.position.set(-300, 150, -300);
  scene.add(pointLight3);

  // 环境光补充（大幅增强，确保透明船体在任何缩放级别都清晰可见）
  const ambientLight = new THREE.AmbientLight(0xffffff, 2.0); // 增强到2.0
  scene.add(ambientLight);

  console.log('✅ 光源系统已配置，支持透明船体在任何缩放级别可见 | Lighting system configured for transparent ship visibility');
  console.log(`   环境光强度 | Hemisphere light intensity: 6.0`);
  console.log(`   主方向光强度 | Main directional light intensity: 6.0`);
  console.log(`   环境光补充 | Ambient light intensity: 2.0`);
  console.log(`   阴影范围 | Shadow camera range: ±600m, far: 1000m`);
  console.log(`   点光源范围 | Point light range: 800-1000m`);
}
...
function createWater() {
  ...
  const material = new THREE.ShaderMaterial({
    ...
    wireframe: config.display.wireframeWater
  });

  waterMeshFar = new THREE.Mesh(geom, material);
  waterMeshFar.receiveShadow = true;
  waterMeshFar.castShadow = false;
  scene.add(waterMeshFar);

  console.log('✅ 水面已创建 | Water created');
}
