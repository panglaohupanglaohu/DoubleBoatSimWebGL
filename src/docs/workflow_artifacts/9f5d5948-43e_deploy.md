...
**影响范围**: 中等
- **后端**: 新增 API 模块，不影响现有接口
- **前端**: 在现有页面添加新功能区域，不破坏现有功能
- **配置**: 新增配置项，向后兼容
...
#### 1.2.1 新增文件
```
src/backend/api/deepseek_test.py          # 新增 DeepSeek 连通性测试 API
tests/test_deepseek_connection.py         # 新增后端测试
tests/frontend/test_deepseek_button.js    # 新增前端测试
```

#### 1.2.2 修改文件
```
src/backend/models/schemas.py             # 扩展数据模型
src/backend/app/main.py                   # 注册新路由
src/backend/core/config.py                # 新增配置项
src/frontend/token_factory.html           # 添加测试界面
src/frontend/js/token_factory.js          # 添加前端逻辑
src/frontend/css/style.css                # 添加新样式
...
### 2.1 部署类型: **Feature Deployment**

**选择理由**:
1. ✅ **非破坏性变更**: 新增功能不影响现有接口
2. ✅ **API 向后兼容**: 现有 API 签名不变
3. ✅ **无数据库迁移**: 不涉及 schema 变更
4. ✅ **前端增量更新**: 在现有页面添加新区域
5. ✅ **配置向后兼容**: 新增配置有默认值
...
# 3. 配置验证
python -c "from src.backend.core.config import settings; print('Config loaded successfully')"

# 4. 静态文件检查
find src/frontend -name "*.html" -exec html5validator {} \;
```

#### 阶段 2: 后端部署
```bash
# 1. 停止服务 (优雅停机)
sudo systemctl stop doubleboat-backend || pkill -f "uvicorn"

# 2. 备份当前版本
BACKUP_DIR="/opt/doubleboat/backup/$(date +%Y%m%d_%H%M%S)"
mkdir -p $BACKUP_DIR
cp -r src/backend $BACKUP_DIR/

# 3. 部署新代码
cp -r src/backend /opt/doubleboat/
cp requirements.txt /opt/doubleboat/
...
# 5. 启动服务
sudo systemctl start doubleboat-backend || \
uvicorn src.backend.app.main:app --host 0.0.0.0 --port 8000 --workers 4 &

# 6. 健康检查
sleep 5
curl -f http://localhost:8000/health || exit 1
curl -f http://localhost:8000/api/deepseek-test/docs || exit 1
```

#### 阶段 3: 前端部署
```bash
# 1. 备份当前前端
FRONTEND_BACKUP="/opt/doubleboat/frontend_backup/$(date +%Y%m%d_%H%M%S)"
mkdir -p $FRONTEND_BACKUP
cp -r /var/www/doubleboat/* $FRONTEND_BACKUP/

# 2. 部署新前端
cp -r src/frontend/* /var/www/doubleboat/

# 3. 设置权限
chown -R www-data:www-data /var/www/doubleboat
chmod -R 755 /var/www/doubleboat

# 4. 重启 Web 服务器
sudo systemctl reload nginx || sudo systemctl restart apache2
```

#### 阶段 4: 验证测试
```bash
# 1. API 功能测试
curl -X POST http://localhost:8000/api/deepseek-test/test-connection \
  -H "Content-Type: application/json" \
  -d '{"api_key":"test-key","model":"deepseek-chat"}' \
  --max-time 10
...
虽然本次部署不需要蓝绿部署，但可以实施**功能开关**进行灰度控制：

```javascript
// 在 src/frontend/js/token_factory.js 中添加功能开关
const FEATURE_FLAGS = {
  DEEPSEEK_TEST_ENABLED: true,  // 可配置为环境变量
  // 未来可扩展其他功能开关
};

if (FEATURE_FLAGS.DEEPSEEK_TEST_ENABLED) {
  // 初始化 DeepSeek 测试器
  new DeepSeekTester();
}
```
...
**灰度比例控制**:
```nginx
# nginx 配置示例 - 基于 Cookie 的灰度
location /token_factory.html {
  set $show_feature 0;
  
  # 10% 用户看到新功能
  if ($cookie_gray_user = "1") {
    set $show_feature 1;
  }
  
  if ($show_feature = 1) {
    # 返回包含新功能的页面
    rewrite ^ /token_factory_with_test.html last;
  }
...
# 1. 查找最新的备份
LATEST_BACKUP=$(ls -td /opt/doubleboat/backup/* | head -1)
LATEST_FRONTEND_BACKUP=$(ls -td /opt/doubleboat/frontend_backup/* | head -1)
...
# 2. ��止服务
echo "停止服务..."
sudo systemctl stop doubleboat-backend || true

# 3. 回滚后端
echo "回滚后端代码..."
rm -rf /opt/doubleboat/src/backend
cp -r $LATEST_BACKUP/backend /opt/doubleboat/src/

# 4. 回滚前端
echo "回滚前端代码..."
rm -rf /var/www/doubleboat/*
cp -r $LATEST_FRONTEND_BACKUP/* /var/www/doubleboat/
...
### 4.2 回滚检查清单
- [ ] 验证 API 端点 `/api/deepseek-test/test-connection` 已移除
- [ ] 验证 Token Factory 页面无 DeepSeek 测试按钮
- [ ] 验证系统日志无相关错误
- [ ] 验证原有功能正常工作
...
### 5.1 新增监控指标
```python
# 在 deepseek_test.py 中添加监控
import prometheus_client

deepseek_test_requests = prometheus_client.Counter(
    'deepseek_test_requests_total',
    'Total DeepSeek connection test requests'
)

deepseek_test_success = prometheus_client.Counter(
    'deepseek_test_success_total',
    'Successful DeepSeek connection tests'
)

deepseek_test_latency = prometheus_client.Histogram(
    'deepseek_test_latency_seconds',
    'DeepSeek connection test latency'
)
```
...
groups:
  - name: deepseek_test
    rules:
      - alert: DeepSeekTestFailureRateHigh
        expr: rate(deepseek_test_requests_total[5m]) > 0 and rate(deepseek_test_success_total[5m]) / rate(deepseek_test_requests_total[5m]) < 0.8
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "DeepSeek 连通性测试失败率过高"
          description: "过去5分钟内 DeepSeek 连通性测试失败率超过20%"
```
...
| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|----------|
| DeepSeek API 不可用 | 中 | 低 | 前端显示友好错误，不影响核心功能 |
| 新依赖包冲突 | 低 | 中 | 预部署环境充分测试，使用虚拟环境 |
| 前端兼容性问题 | 低 | 低 | 多浏览器测试，渐进增强 |
| 配置错误 | 中 | 中 | 配置验证脚本，环境变量默认值 |

### 7.2 应急预案
1. **问题检测**: 部署后监控 15 分钟关键指标
2. **自动回滚**: 如果错误率 > 5%，自动触发回滚
3. **人工干预**: 保留运维人员联系方式，随时待命
4. **用户通知**: 准备维护公告模板

## 8. 文档更新

### 8.1 需要更新的文档
- [ ] API 文档 (`/docs` 或 Swagger)
- [ ] 用户操作手册
- [ ] 运维手册
- [ ] 故障排除指南
...
### 变更概要
- 新增 DeepSeek 连通性测试功能
- 在 Token Factory 页面添加测试按钮

### 部署时间
- 开始: 2024-04-08 19:00 UTC
- 结束: 2024-04-08 19:30 UTC
...
**部署策略文件路径**: `/Users/panglaohu/Downloads/DoubleBoatClawSystem/docs/deployment/deployment_strategy.md`

**控制台输出**: 部署策略文档已保存至 `/Users/panglaohu/Downloads/DoubleBoatClawSystem/docs/deployment/deployment_strategy.md`
