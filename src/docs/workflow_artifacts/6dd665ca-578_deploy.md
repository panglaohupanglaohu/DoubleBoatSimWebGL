...
|------|------|
| **变更类型** | 小改动 (功能增强) |
| **影响范围** | 仅 `main.js` 文件，新增 `createCargoOrbitHUD()` 函数 |
| **API 变更** | 无 |
| **HTML 页面变更** | 无 (HUD 通过 JS 动态创建 DOM 元素) |
| **部署策略** | ✅ **就地更新 (hotfix)** |
...
|------|------|
| **变更类型** | 新增文件 |
| **影响范围** | 新增 `cargo_orbit_telemetry.py`，不修改现有文件 |
| **API 变更** | 无 (新增 Channel，通过现有事件机制集成) |
| **核心逻辑变更** | 否 (新增辅助 Channel) |
| **部署策略** | ✅ **就地更新 (新增文件)** |
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `src/frontend/digital-twin/main.js` | **修改** | 添加 `createCargoOrbitHUD()` 函数及动画更新逻辑 |
| `src/backend/channels/cargo_orbit_telemetry.py` | **新建** | 货船轨道遥测上报 Channel |

### 部署步骤

#### Step 1: 后端 — 新建 Channel 文件
```bash
# 文件已就绪: src/backend/channels/cargo_orbit_telemetry.py
# 无需额外操作，系统自动应用
```

#### Step 2: 前端 — 修改 main.js
```bash
# 文件已修改: src/frontend/digital-twin/main.js
# 系统自动应用变更
```
...
# 2. 刷新前端页面
# 打开浏览器访问 http://localhost:8000/digital-twin.html

# 3. 验证 HUD 显示
# - 检查右下角是否显示 wabi-sabi 风格 HUD
# - 确认显示当前角度和距离双体船的距离
# - 确认数值随货船圆周运动实时更新

# 4. 验证后端 Channel
curl -X POST http://localhost:8000/api/v1/channels/cargo_orbit_telemetry/process \
  -H "Content-Type: application/json" \
  -d '{"type": "cargo_orbit_telemetry", "x": 80.0, "z": 0.0, "angle_deg": 0.0, "distance": 80.0}'
# 预期返回: {"status": "ok", "action": "telemetry_reported", "latitude": 31.2304, "longitude": 121.4817, ...}
```
...
```bash
# 恢复备份文件
cp src/frontend/digital-twin/main.js.bak src/frontend/digital-twin/main.js
```
...
# 2. 刷新前端
# 3. 确认 HUD 消失
# 4. 确认 cargo_orbit_telemetry Channel 不可用
```
...
| **影响文件数** | 2 (1 修改 + 1 新建) |
| **风险等级** | 🟢 低 |
| **是否需要重启服务** | 是 (后端需重启以注册新 Channel) |
| **是否需要刷新前端** | 是 (HUD 在 init() 中创建) |
| **回滚复杂度** | 🟢 简单 (文件级回滚) |
...
**变更内容已就绪，系统将自动应用到项目代码库。**

部署完成后，请验证:
1. 右下角显示 wabi-sabi 风格 HUD overlay
2. HUD 显示货船当前角度和距离双体船的距离
3. 数值随货船圆周运动实时更新
4. 后端 `CargoOrbitTelemetryChannel` 可正常处理遥测事件
