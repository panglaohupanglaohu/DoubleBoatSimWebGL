...
**目标**: 在 `src/frontend/digital-twin/main.js` 中让 cargo ship 围绕双体船做圆周轨道运动，半径 80m，角速度 0.015 rad/帧。
...
通过查看 `src/frontend/digital-twin/main.js`，发现当前系统使用 Three.js 进行 3D 渲染，并有以下关键组件：
...
当前系统已有基本的动画循环，但 cargo ship 是静态的或做直线运动。需要修改为圆周运动。

### 2.3 技术方案选择

**方案**: 在动画循环中，使用三角函数计算 cargo ship 的圆周位置

```javascript
// 每帧更新 cargo ship 位置
angle += 0.015; // 角速度 rad/帧
cargoShip.position.x = catamaran.position.x + 80 * Math.cos(angle);
cargoShip.position.z = catamaran.position.z + 80 * Math.sin(angle);
```
...
**唯一需要修改的文件**: `src/frontend/digital-twin/main.js`

### 3.2 具体修改内容

#### 修改 1: 添加角度变量

在文件顶部或 cargo ship 初始化附近添加：

```javascript
// 货船圆周运动参数
let cargoShipAngle = 0; // 初始角度
const ORBIT_RADIUS = 80; // 轨道半径（米）
const ANGULAR_VELOCITY = 0.015; // 角速度（rad/帧）
```

#### 修改 2: 在动画循环中更新位置

在 `animate()` 函数中，找到 cargo ship 的位置更新逻辑，替换为：
...
function animate() {
    ...
    // 更新货船圆周运动
    if (cargoShip && catamaran) {
        cargoShipAngle += ANGULAR_VELOCITY;
        cargoShip.position.x = catamaran.position.x + ORBIT_RADIUS * Math.cos(cargoShipAngle);
        cargoShip.position.z = catamaran.position.z + ORBIT_RADIUS * Math.sin(cargoShipAngle);
        
        // 可选：让货船朝向运动方向
        cargoShip.lookAt(
            catamaran.position.x + ORBIT_RADIUS * Math.cos(cargoShipAngle + 0.1),
            cargoShip.position.y,
            catamaran.position.z + ORBIT_RADIUS * Math.sin(cargoShipAngle + 0.1)
        );
    }
...
1. 刷新前端页面
2. 观察 cargo ship 是否围绕双体船做圆周运动
3. 检查运动是否平滑，半径是否约为 80m

## 4. 实施步骤

### 步骤 1: 定位代码

打开 `src/frontend/digital-twin/main.js`，找到：
- `cargoShip` 变量定义位置
- `catamaran` 变量定义位置
- `animate()` 函数
...
在 cargo ship 初始化代码附近添加轨道参数变量。

### 步骤 3: 修改动画循环

在 `animate()` 函数中，在 `requestAnimationFrame` 之后添加圆周运动更新逻辑。

### 步骤 4: 测试验证

刷新页面，确认 cargo ship 做圆周运动。

## 5. 注意事项

1. **坐标系**: 确认 Three.js 场景的坐标系（Y 轴向上），确保圆周运动在 XZ 平面
2. **单位**: 确认场景单位是米，80m 半径在场景中是否合适
3. **性能**: 圆周运动计算量很小，不会影响性能
4. **初始位置**: 确保 cargo ship 初始位置在圆周上，避免跳变

## 6. 代码示例（完整修改）

```javascript
// 在文件顶部或 cargo ship 初始化附近添加
let cargoShipAngle = 0;
const ORBIT_RADIUS = 80;
const ANGULAR_VELOCITY = 0.015;
