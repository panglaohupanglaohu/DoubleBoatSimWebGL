...
好的，作为 PoseidonX 系统的架构师，我将基于项目上下文、PM 的分解和研究员的技术调研，为“复杂任务测试V4”设计详细的技术方案。

## 技术方案: 货船轨道遥测 HUD 与后端 Channel

### 1. 概述

本方案旨在为数字孪生场景中的货船圆周运动添加一个具有侘寂（wabi-sabi）美学风格的 HUD 覆盖层，实时显示货船相对于双体船的角度和距离。同时，新建一个后端 Channel (`CargoOrbitTelemetryChannel`)，负责将货船在 3D 场景中的坐标转换为地理坐标 (lat/lon) 并上报。

### 2. 架构设计

本任务涉及前端和后端的协同工作，数据流如下：

1.  **前端 (main.js)**:
    *   `animate()` 循环中更新货船 (`state.cargoShip`) 的位置，模拟圆周运动。
    *   每次更新后，计算货船相对于双体船 (`state.boatMesh`) 的**角度**和**距离**。
    *   更新一个 HTML 覆盖层 (`#cargo-orbit-hud`) 来显示这些数据。
    *   (可选) 通过 WebSocket 将货船的 3D 坐标 (`x`, `z`) 发送到后端。

2.  **后端 (cargo_orbit_telemetry.py)**:
    *   新建 `CargoOrbitTelemetryChannel` 类，继承自 `MarineChannel`。
    *   实现 `process_event` 方法，处理 `cargo_orbit_telemetry` 类型的事件。
    *   事件数据包含货船的 3D 坐标 (`x`, `z`)。
    *   使用预定义的坐标转换函数 (`_scene_to_geo`) 将 `(x, z)` 转换为 `(lat, lon)`。
    *   将转换后的地理坐标和原始遥测数据存储并上报。

### 3. 详细设计

#### 3.1. 前端 HUD 覆盖层 (`src/frontend/digital-twin/main.js`)

**目标**: 在 `init()` 函数中创建并管理一个显示货船遥测数据的 HUD 元素。

**修改内容**:

1.  **新增函数 `createCargoOrbitHUD()`**:
    *   创建一个 `div` 元素，ID 为 `cargo-orbit-hud`。
    *   应用 wabi-sabi 风格的 CSS：
        *   背景：半透明、带有纹理感的深色 (`rgba(20, 15, 10, 0.7)`)。
        *   边框：粗糙、不规则的边框效果（使用 `box-shadow` 或 `border-image` 模拟）。
        *   字体：衬线字体或手写风格字体 (`'Georgia', serif`)，颜色为褪色的金色或米色 (`#d4c5a9`)。
        *   布局：固定在屏幕右下角，带有轻微的不对称边距。
        *   内容：显示“方位”和“距离”两个数据项。
    *   将该 `div` 添��到 `document.body`。
    *   返回创建的 HUD 元素引用，存储在 `state` 中。

2.  **修改 `init()` 函数**:
    *   在调用 `createCargoShip()` 之后，调用 `createCargoOrbitHUD()`。

3.  **修改 `animate()` 函数**:
    *   在货船位置更新逻辑之后，添加代码以更新 HUD 内容。
    *   获取货船 (`state.cargoShip`) 和双体船 (`state.boatMesh`) 的世界位置。
    *   计算相对向量。
    *   计算角度（使用 `Math.atan2`）和距离（使用 `Vector3.distanceTo`）。
    *   更新 HUD 元素的 `innerHTML` 或 `textContent`。
...
function createCargoOrbitHUD() {
    ...
    hud.id = 'cargo-orbit-hud';
    hud.innerHTML = `
        <div class="hud-title">~ 货船遥测 ~</div>
        <div class="hud-data">
            <span class="hud-label">方位</span>
            <span class="hud-value" id="cargo-angle">---</span>
        </div>
        <div class="hud-data">
            <span class="hud-label">距离</span>
            <span class="hud-value" id="cargo-distance">---</span>
        </div>
    `;
    // 应用 wabi-sabi 风格
...
function animate() {
    ...
    // 更新 HUD
    if (state.cargoShip && state.boatMesh && state.cargoOrbitHUD) {
        const cargoPos = new THREE.Vector3();
        state.cargoShip.getWorldPosition(cargoPos);
        
        const boatPos = new THREE.Vector3();
        state.boatMesh.getWorldPosition(boatPos);
        ...
        angleDeg = (angleDeg + 360) % 360; // 转换为 0-360 度
        
        // 计算距离
        const distance = cargoPos.distanceTo(boatPos);

        // 更新 DOM
        const angleEl = document.getElementById('cargo-angle');
        const distEl = document.getElementById('cargo-distance');
        if (angleEl) angleEl.textContent = `${angleDeg.toFixed(1)}°`;
        if (distEl) distEl.textContent = `${distance.toFixed(1)} m`;
    }
...
#### 3.2. 后端 Channel (`src/backend/channels/cargo_orbit_telemetry.py`)

**目标**: 创建一个新的 MarineChannel，用于接收并处理货船遥测数据。

**文件路径**: `src/backend/channels/cargo_orbit_telemetry.py`

**核心逻辑**:
*   继承 `MarineChannel`。
*   在 `__init__` 中初始化坐标转换常量和数据缓存。
*   实现 `process_event` 方法，处理 `cargo_orbit_telemetry` 事件。
*   实现 `_scene_to_geo` 辅助函数，用于坐标转换。
*   实现 `get_latest_telemetry` ��� `get_telemetry_history` 方法供外部查询。

**代码实现** (基于已有文件 `cargo_orbit_telemetry.py` 的完善):
...
class CargoOrbitTelemetryChannel(MarineChannel):
    ...
    dependencies: list[str] = ["cargo_ship_orbit"]
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._telemetry_history: list[Dict[str, Any]] = []
        self._max_history: int = 1000
        logger.info("📡 CargoOrbitTelemetryChannel initialized")
    ...
    def _handle_telemetry(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.debug("📡 Telemetry: lat=%.6f, lon=%.6f", lat, lon)
        return {
            "status": "ok",
            "action": "telemetry_reported",
            "latitude": lat,
...
#### 4.1. 前端 HUD 接口 (无 API，纯 DOM 操作)
*   **元素 ID**: `cargo-orbit-hud`
*   **数据元素 ID**: `cargo-angle`, `cargo-distance`
*   **更新方式**: 在 `animate()` 循环中直接修改 DOM 元素的 `textContent`。

#### 4.2. 后端 Channel 事件接口
*   **事件类型**: `cargo_orbit_telemetry`
*   **请求数据格式**:
    ```json
    {
...
*   **响应数据格式**:
    ```json
    {
      ...
      "action": "telemetry_reported",
      "latitude": 31.229072,
      "longitude": 121.478220
    }
    ```

### 5. 实施步骤 (面向开发工程师)

1.  **修改前端文件 `src/frontend/digital-twin/main.js`**:
    *   **步骤 A**: 在 `state` 对象中添加 `cargoOrbitHUD: null`。
    *   **步骤 B**: 在 `init()` 函数中，`createCargoShip()` 调用之后，添加 `createCargoOrbitHUD();`。
    *   **步骤 C**: 在文件末尾或 `init()` 之前，添加 `createCargoOrbitHUD()` 函数的完整实现（参考 3.1 节代码）。
    *   **步骤 D**: 找到 `animate()` 函数，在更新货船位置（`state.cargoShip.position`）的代码之后，添加更新 HUD 的代码（参考 3.1 节 `animate` 函数中的 HUD 更新逻辑）。

2.  **创建后端文件 `src/backend/channels/cargo_orbit_telemetry.py`**:
    *   在 `src/backend/channels/` 目录下创建此文件。
    *   将 3.2 节的完整代码复制到该文件中。
    *   **注意**: 确保 `from .marine_base import ...` 的导入路径正确。

3.  **注册后端 Channel**:
    *   检查 `src/backend/register_channels.py` 或类似的 Channel 注册文件。
    *   添加 `CargoOrbitTelemetryChannel` 到 Channel 列表中，以便系统启动时自动加载。
    *   示例代码:
        ```python
        # 在 register_channels.py 中
...
4.  **测试**:
    *   启动前端和后端服务。
    *   观察 3D 场景右下角是否出现 wabi-sabi 风格的 HUD，并实时更新角度和距离。
    *   通过后端 API 或日志，验证 `CargoOrbitTelemetryChannel` 是否正确接收事件并转换坐标。
