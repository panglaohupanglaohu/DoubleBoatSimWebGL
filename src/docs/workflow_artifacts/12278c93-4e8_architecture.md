...
### 1.1 核心需求
让货船以双体船为圆心进行圆周运动，模拟船舶在海上绕圈航行的场景。

### 1.2 技术约束
- 前端使用 Three.js 3D 场景
- 双体船已有姿态控制 (wpc-attitude-v1)
- 货船模型已加载 (GLB_20251223141542.glb)
- 需要保持双体船静止作为圆心
...
**圆周运动参数:**
- 圆心: 双体船位置 (catamaran.position)
- 半径: 可配置 (默认 50 单位)
- 角速度: 可配置 (默认 0.01 rad/s)
- 货船朝向: 始终指向圆心 (切线方向)
...
**1. `src/frontend/digital-twin/main.js`** (主要修改)
- 添加货船圆周运动逻辑
- 集成到现有动画循环

**2. `src/frontend/digital-twin/PoseidonX.js`** (次要修改)
- 添加圆周运动控制接口
- 暴露配置参数

**3. `src/frontend/digital-twin/layer1-interface/DigitalTwinMap.js`** (可选)
- 在 2D 地图上显示圆周轨迹

#### 2.3.2 后端文件 (可选)

**4. `src/backend/channels/wpc_attitude_control.py`** (如果存在)
- 添加圆周运动状态同步
...
```javascript
// 在动画循环中添加圆周运动
class CargoShipOrbit {
    constructor(cargoShip, centerObject, radius = 50, angularVelocity = 0.01) {
    ...
    update(deltaTime) {
        if (!this.isActive || !this.cargoShip || !this.centerObject) return;

        // 更新角度
        this.angle += this.angularVelocity * deltaTime;

        // 计算新位置
        const centerPos = this.centerObject.position;
        this.cargoShip.position.x = centerPos.x + this.radius * Math.cos(this.angle);
        this.cargoShip.position.z = centerPos.z + this.radius * Math.sin(this.angle);
        ...
        this.cargoShip.lookAt(
            this.cargoShip.position.clone().add(direction)
        );
    }
...
// 在场景初始化后创建
const cargoOrbit = new CargoShipOrbit(cargoShip, catamaran, 50, 0.01);

// 在动画循环中调用
function animate() {
    requestAnimationFrame(animate);
    const deltaTime = clock.getDelta();
    cargoOrbit.update(deltaTime);
    // ... 其他渲染逻辑
}
```

#### 2.4.2 PoseidonX.js 修改
...
class PoseidonX {
    ...
    startCargoOrbit(radius = 50, speed = 0.01) {
        if (this.cargoOrbit) {
            this.cargoOrbit.setRadius(radius);
            this.cargoOrbit.setSpeed(speed);
            this.cargoOrbit.isActive = true;
        }
    }

    stopCargoOrbit() {
        if (this.cargoOrbit) {
            this.cargoOrbit.isActive = false;
        }
    }
...
|------|--------|------|
| radius | 50 | 圆周半径 (单位) |
| angularVelocity | 0.01 | 角速度 (rad/s) |
| isActive | true | 是否启用圆周运动 |
| centerOffset | (0, 0, 0) | 圆心偏移量 |

### 2.6 实施步骤

1. **步骤 1**: 在 main.js 中找到货船和双体船的 Three.js 对象引用
2. **步骤 2**: 实现 CargoShipOrbit 类
3. **步骤 3**: 集成到现有动画循环
4. **步骤 4**: 添加控制接口 (启动/停止/参数调整)
5. **步骤 5**: 测试圆周运动效果
6. **步骤 6**: 可选: 添加 UI 控制面板

### 2.7 风险与注意事项

1. **模型引用**: 需要确认货船和双体船的 Three.js 对象名称
2. **性能**: 圆周运动计算量很小，不影响性能
3. **碰撞检测**: 如果半径太小，货船可能与双体船重叠
4. **朝向计算**: 需要确保货船始终朝向运动方向
