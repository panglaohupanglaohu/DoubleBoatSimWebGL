    ...
    **compliance_checker.py**:
    ...
    class ComplianceChecker:
        ...
        def check_view_compliance(self, data: Dict[str, Any], view_level: ViewLevel) -> Dict[str, Any]:
            ...
            return {
                "status": "passed" if not missing_fields else "
...
### 1.1 变更范围评估
基于前序步骤的分析，本次任务涉及对设备健康页面的Agent系统状态显示进行优化，而非移除。根据船长安全指令，必须保持关键航行信息持续可见，因此任务转换为界面优化。
...
#### 后端影响:
- ✅ 新增API端点: `/api/agent/optimized-status`
- ✅ 新增API端点: `/api/agent/critical-status` 
- ✅ 新增API端点: `/api/agent/view-config`
- ✅ 扩展数据模型: `agent_models.py`
- ✅ 新增业务逻辑服务: `agent_service.py`
- ✅ 新增合规性检查工具: `compliance_checker.py`
- ❌ 不修改现有API端点: `/api/agent/status` (保持向后兼容)

#### 前端影响:
- ✅ 重构设备健康页面: `index.html`
- ✅ 新增优化视图逻辑: `optimized-view.js`
- ✅ 新增Agent状态组件: `components/agent-status/`
- ✅ 修改CSS样式: `device-health.css`
- ✅ 新增组件样式: `components/agent-status.css`
- ❌ 不删除现有功能，仅增强
...
**选择理由:**
1. **功能变更规模**: 中等规模，涉及多个新API端点和前端组件
2. **用户影响**: 界面布局和交互方式有显著变化
3. **安全要求**: 涉及SOLAS合规性，需要谨慎验证
4. **回滚需求**: 需要快速回滚能力，确保航行安全
5. **测试验证**: 需要并行运行新旧版本进行A/B测试
...
|--------|----------|------|
| 新增完整页面 | ✅ | 新增优化视图组��� |
| API接口签名变更 | ✅ | 新增API端点，但保持向后兼容 |
| 数据库schema迁移 | ❌ | 无数据库变更 |
| 核心逻辑变更 | ✅ | 新增分级数据过滤和合规性检查 |
| 用户界面重大变更 | ✅ | 界面布局和交互方式优化 |
...
├─────────────────────────────────────────────────────────────┤
│ 根据URL路径和Cookie分流:                                    │
│  - /device-health?version=v1 (蓝色版本)                    │
│  - /device-health?version=v2 (绿色版本)                    │
│  - 默认: 90% v1, 10% v2 (灰度发布)                         │
└──────────────────────────────┬──────────────────────────────┘
                               │
        ┌──────────────────────┼──────────────────────┐
...
# 部署后端代码
cp -r /path/to/new/backend/* src/backend/
# 部署前端代码  
cp -r /path/to/new/frontend/* src/frontend/

# 3. 配置环境变量
echo "APP_VERSION=v1.1.0" > .env.green
echo "DEPLOYMENT_COLOR=green" >> .env.green
```
...
python -m src.backend.main --port 8001 --env green &
...
http {
    ...
    map $cookie_version $frontend_upstream {
    ...
    server {
        ...
        location /api/ {
            ...
            proxy_set_header X-Deployment-Color $cookie_version;
        }
        ...
        location /device-health {
            ...
            add_header Set-Cookie "version=$cookie_version; Path=/; Max-Age=3600";
...
# 1. 功能测试
# 测试蓝色版本
curl "http://localhost/device-health?version=v1"
# 测试绿色版本
curl "http://localhost/device-health?version=v2"

# 2. API测试
curl "http://localhost/api/agent/optimized-status?view_level=compact"
curl "http://localhost/api/agent/critical-status"

# 3. 合规性验证
# 验证关键信息始终可见
python scripts/verify_compliance.py --version=v2

# 4. 性能测试
ab -n 1000 -c 10 "http://localhost/device-health?version=v2"
```

#### 阶段5: 灰度发布 (逐步扩大)
1. **第1天**: 10%流量到绿色版本 (内部测试用户)
2. **第2天**: 25%流量到绿色版本 (扩展测试范围)
3. **第3天**: 50%流量到绿色版本 (平衡测试)
4. **第4天**: 75%流量到绿色版本 (接近全量)
5. **第5天**: 100%流量到绿色版本 (全量发布)

#### 阶段6: 监控与观察
```bash
# 监控指标
# 1. 错误率监控
tail -f /var/log/poseidonx/green/error.log

# 2. 性能监控
watch -n 5 "curl -s http://localhost:8001/health | jq '.response_time'"
...
```
src/backend/api/agent/optimized_status.py
src/backend/api/agent/view_config.py
src/backend/services/agent_service.py
src/backend/utils/compliance_checker.py
src/backend/models/agent_models.py (扩展)
```

#### 前端新增:
```
src/frontend/pages/device-health/optimized-view.js
src/frontend/components/agent-status/
├── agent-status.html
├── agent-status.js
└── agent-status.css
src/frontend/css/components/agent-status.css
src/frontend/utils/view-manager.js
```
...
#### 后端修改:
```
src/backend/api/agent/__init__.py (注册新路由)
src/backend/tests/test_agent_api.py (添加新测试)
```

#### 前端修改:
```
src/frontend/pages/device-health/index.html (重构)
src/frontend/pages/device-health/device-health.js (集成优化视图)
src/frontend/css/device-health.css (扩展样式)
```
...
### 5.1 自动回滚条件
当以下任一条件触发时，自动回滚到蓝色版本:
1. **错误率 > 5%** (持续5分钟)
2. **响应时间 > 2秒** (P95，持续10分钟)
3. **关键API不可用** (健康检查失败)
4. **合规性检查失败** (关键信息不可见)
...
# 2. 更新Nginx配置，100%流量到蓝色版本
cp deployment/nginx/poseidonx-blue-only.conf /etc/nginx/conf.d/poseidonx.conf
nginx -s reload

# 3. 清理绿色环境文件
rm -rf /opt/poseidonx/green

# 4. 发送通知
curl -X POST -H "Content-Type: application/json" \
  -d '{"text":"已回滚到蓝色版本(v1.0.0)"}' \
  $SLACK_WEBHOOK_URL

echo "回滚完成，所有流量已切换到蓝色版本"
```
...
function getDeploymentVersion() {
    ...
    if (urlParams.has('version')) {
        ...
        document.cookie = `version=${version}; path=/; max-age=3600`;
        return version;
    }
    ...
    const version = random < 0.1 ? 'v2' : 'v1';
    document.cookie = `version=${version}; path=/; max-age=3600`;
    return version;
}
```
...
```python
# 后端分流逻辑示例
def should_use_green_version(user_id: str) -> bool:
    """决定用户是否使用绿色版本"""
    # 1. 内部测试用户
    ...
    if user_hash < 10:  # 10%用户
        return True
    
    # 3. 特定用户组
    user_group = get_user_group(user_id)
    if user_group in EARLY_ADOPTER_GROUPS:
        return True
...
```html
<!-- 反馈组件 -->
<div class="feedback-widget" id="new-ui-feedback">
    <h4>新界面反馈</h4>
    <p>您正在使用优化后的Agent状态界面，请提供反馈:</p>
    ...
    <textarea placeholder="其他建议..." rows="3"></textarea>
    <button class="submit-feedback">提交反馈</button>
    
    <div class="switch-version">
        <button class="switch-to-old">切换回旧版本</button>
    </div>
</div>
```
...
| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|----------|
| 新API性能问题 | 中 | 高 | 蓝绿部署，快速回滚 |
| 界面兼容性问题 | 中 | 中 | 渐进式灰度发布 |
| 合规性违规 | 低 | 高 | 实时合规性监控 |
| 用户不接受新界面 | 中 | 低 | 提供版本切换选项 |
| 数据不一致 | 低 | 中 | 共享数据源，无数据迁移 |
...
```bash
# 自动触发回滚
if [ $(curl -s http://localhost:8001/health | jq '.response_time') -gt 2000 ]; then
...
def monitor_compliance():
    while True:
        response = requests.get('http://localhost:8001/api/agent/critical-status')
        data = response.json()
        ...
        if missing:
            alert_slack(f"合规性违规: 缺失字段 {missing}")
            trigger_rollback()
...
### 部署类型: **蓝绿部署**
- **理由**: 中等规模功能变更，需要确保航行安全和用户体验
- **优势**: 零停机部署，快速回滚，A/B测试能力

### 关键决策点:
1. **保持向后兼容**: 不修改现有API，新增优化API
2. **渐进式发布**: 从10%流量开始，逐步扩大
3. **强监控**: 实时监控性能和合规性
4. **快速回滚**: 自动化回滚机制
...
**部署负责人**: DevOps工程师  
**部署时间窗口**: 业务低峰期 (02:00-04:00 UTC)  
**沟通渠道**: Slack #deployments, Email通知  
**批准人**: 技术负责人、船长(合规确认)
