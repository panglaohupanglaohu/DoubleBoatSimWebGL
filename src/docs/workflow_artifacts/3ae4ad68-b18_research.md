...
**核心需求**: 让货船(cargo ship)以双体船(catamaran)为圆心做圆周运动。
...
根据项目文件结构，以下文件与3D场景和船舶动画相关:

| 文件路径 | 相关性 |
|---------|--------|
| `src/frontend/digital-twin/main.js` | 3D场景主入口，可能包含船舶加载逻辑 |
| `src/frontend/digital-twin/demo.js` | 演示场景，可能包含动画 |
| `src/frontend/digital-twin/PoseidonX.js` | 核心3D引擎 |
| `src/frontend/digital-twin/NavigationMonitor.js` | 导航监控，可能包含船舶位置 |
| `src/frontend/digital-twin/index.js` | 场景初始化 |
| `src/frontend/digital-twin/DataAggregator.js` | 数据聚合 |
| `src/frontend/digital-twin/waves.js` | 波浪效果 |
| `src/frontend/digital-twin/weather-controls.js` | 天气控制 |
| `src/frontend/digital-twin/layer1-interface/DigitalTwinMap.js` | 数字孪生地图 |
| `src/frontend/GLB_20251223141542.glb` | 3D模型文件 |
| `src/frontend/digital-twin/layer2-agents/NavigatorAgent.js` | 导航Agent |
| `src/frontend/digital-twin/layer2-agents/AgentOrchestrator.js` | Agent编排器 |

### 2.2 现有3D场景分析

从 `darwin-ratchet.js` 的 HERITAGE 记录中可以看到:
- `wpc-attitude-v1`: 穿浪双体船姿态控制 (水翼/T-Foil 主动姿态反馈)
- `ais-iceberg-merge-v1`: AIS 列表聚合本地威胁 (包含3D场景中的冰山和货船目标)
- `ocean-shader-v1`: 海洋GPU着色器
- `icebergs-sss-v1`: 冰山次表面散射着色
...
### 3.1 方案一: 在 main.js 中添加动画逻辑

**可行性**: ⭐⭐⭐⭐⭐
**复杂度**: 低

`main.js` 是3D场景的主入口，很可能包含 Three.js 场景的初始化、动画循环和对象管理。

**需要修改的文件**:
1. `src/frontend/digital-twin/main.js` - 添加货船绕双体船旋转的动画逻辑
...
function animate() {
    ...
    // 获取双体船位置
    const catamaranPos = catamaran.position;
    
    // 计算货船绕双体船旋转
    const radius = 50; // 旋转半径
    const speed = 0.01; // 旋转速度
    const angle = time * speed;
    
    cargoShip.position.x = catamaranPos.x + radius * Math.cos(angle);
    cargoShip.position.z = catamaranPos.z + radius * Math.sin(angle);
    cargoShip.position.y = catamaranPos.y; // 保持同一水平面
    
    // 让货船朝向运动方向
    cargoShip.lookAt(catamaranPos);
    
    renderer.render(scene, camera);
}
```

### 3.2 方案二: 在 PoseidonX.js 中添加船舶运动系统

**可行性**: ⭐⭐⭐⭐
**复杂度**: 中

`PoseidonX.js` 是核心引擎，可能包含船舶管理系统。

**需要修改的文件**:
1. `src/frontend/digital-twin/PoseidonX.js` - 添加船舶运动API
2. `src/frontend/digital-twin/main.js` - 调用运动API

### 3.3 方案三: 在 NavigationMonitor.js 中添加导航动画

**可行性**: ⭐⭐⭐
**复杂度**: 中高

`NavigationMonitor.js` 负责导航监控，可能包含船舶位置更新逻辑。

**需要修改的文件**:
1. `src/frontend/digital-twin/NavigationMonitor.js` - 添加圆周运动逻辑
2. `src/frontend/digital-twin/main.js` - 集成导航监控

## 4. 推荐方案: 方案一 (main.js)

### 4.1 理由

1. **最低侵入性**: 不修改核心引擎，只在主循环中添加动画
2. **易于实现**: 纯数学计算，不依赖复杂系统
3. **易于测试**: 可以快速验证效果
4. **符合现有架构**: main.js 已经是动画循环的入口

### 4.2 需要修改的文件

| 文件 | 修改内容 | 行号估计 |
|------|---------|---------|
| `src/frontend/digital-twin/main.js` | 添加货船绕双体船旋转动画 | 约50-80行 |

### 4.3 具体修改点

**文件**: `src/frontend/digital-twin/main.js`

**修改1**: 在场景初始化后获取双体船和货船的引用
```javascript
// 在 init() 或 setupScene() 函数中
const catamaran = scene.getObjectByName('catamaran') || scene.getObjectByProperty('type', 'Catamaran');
const cargoShip = scene.getObjectByName('cargo_ship') || scene.getObjectByProperty('type', 'CargoShip');
```

**修改2**: 在动画循环中添加旋转逻辑
```javascript
// 在 animate() 或 render() 函数中
if (catamaran && cargoShip) {
    const radius = 80; // 旋转半径 (单位: 米)
    const angularSpeed = 0.005; // 角速度 (弧度/帧)
    const angle = performance.now() * angularSpeed / 1000;
...
1. **对象引用**: 需要确认双体船和货船在Three.js场景中的具体名称或标签
2. **坐标系**: 需要确认场景使用的坐标系 (Y-up 或 Z-up)
3. **性能**: 圆周运动计算量很小，不会影响性能
4. **碰撞检测**: 如果场景有碰撞检测，需要确保货船不会与其他物体碰撞

## 5. 备选方案: 通过Agent系统实现

### 5.1 利用现有Agent架构

从 `execution_team_manager.py` 可以看到，系统有 Navigation Agent 负责航行控制。可以通过Agent系统来实现:

**需要修改的文件**:
1. `src/frontend/digital-twin/layer2-agents/NavigatorAgent.js` - 添加圆周运动指令
2. `src/frontend/digital-twin/layer2-agents/AgentOrchestrator.js` - 编排运动任务

### 5.2 通过API控制

从 `agent_team_api.py` 可以看到，有任务分配API:
- `POST /api/v1/agent-teams/build/assign` - 分配任务给Build团队

可以通过API分配任务给PM，让PM分解任务给开发人员实现。
...
2. **长期方案** (系统集成):
   - 通过Agent系统分配任务
   - 使用 `POST /api/v1/agent-teams/build/assign` API
   - 让PM分解任务，开发人员实现
...
通过API分配任务:
```bash
POST /api/v1/agent-teams/build/assign
{
    "agent_id": "build_pm",
    "task": "让货船以双体船为圆心动起来 - 请在main.js中添加货船绕双体船旋转的动画逻辑，旋转半径80米，速度0.005弧度/帧，顺时针方向"
}
```
