...
我是 PoseidonX 系统的项目经理 (PM)。现在，我将对“让货船以双体船为圆心动起来”这一任务进行详细的分解和规划，并分配给 Build 团队。

---

### **任务分解与规划文档**

**项目名称:** PoseidonX - 数字孪生动态场景
**任务ID:** TASK-BUILD-2024-001
**任务标题:** 实现货船以双体船为圆心的圆周运动
**指派人:** Build 团队 (build_pm)
**优先级:** P0 (最高)
**状态:** 规划中
...
**目标:** 在 PoseidonX 数字孪生场景中，实现一艘或多艘货船以双体船（本船）为圆心，在水平面上进行持续的圆周运动。此功能是验证动态目标跟踪、态势感知算法以及未来自主避碰逻辑的基础。

**核心需求:**
1.  货船围绕双体船（坐标原点）做匀速圆周运动。
2.  运动轨迹平滑，无抖动。
3.  货船的朝向（船首向）应始终与运动方向（切线方向）保持一致。
4.  运动参数（如半径、角速度）应可配置。
...
*   **实现位置:** 前端 `Three.js` 场景。后端负责提供初始数据和配置，但实时动画循环必须在前端完成，以保证流畅性。
*   **关键文件:**
    *   `src/frontend/digital-twin/main.js`: 主场景初始化、动画循环 (`animate` 函数)。
    *   `src/frontend/digital-twin/PoseidonX.js`: 核心场景管理类，很可能包含对 3D 对象的引用和更新逻辑。
    *   `src/frontend/digital-twin/layer1-interface/DigitalTwinMap.js`: 2D 地图视图，可能需要同步更新货船位置。
    *   `src/frontend/js/AIoTMesh.js`: 可能包含 3D 模型加载和管理的通用逻辑。
*   **核心算法:**
    *   圆周运动方程:
        *   `x = centerX + radius * cos(angle)`
        *   `z = centerZ + radius * sin(angle)`
    *   船首向计算: 船首向角 = `atan2(dz, dx)`，其中 `(dx, dz)` 是速度向量，即切线方向。
*   **依赖关系:**
    *   货船 3D 模型 (`GLB_20251223141542.glb`) 已存在于项目中。
    *   双体船模型及其在场景中的位置（作为圆心）。
    *   现有的动画循环框架。
...
| 风险 | 可能性 | 影响 | 缓解措施 |
| :--- | :--- | :--- | :--- |
| **模型加载失败** | 低 | 高 | 确保 `GLB_20251223141542.glb` 文件路径正确且未损坏。添加加载失败的错误处理和用户提示。 |
| **动画循环冲突** | 中 | 中 | 审查 `main.js` 或 `PoseidonX.js` 中的 `animate` 函数，确保新逻辑以非侵入方式集成，避免覆盖或阻塞现有动画。 |
| **性能问题** | 低 | 中 | 圆周运动计算量很小，性能风险低。但如果同时有大量货船，需考虑使用 `BufferGeometry` 或实例化渲染。 |
| **坐标系统不一致** | 中 | 高 | 明确双体船在 Three.js 场景中的世界坐标。确认货船初始位置和运动半径的单位与场景单位一致。 |
| **与后端状态同步** | 低 | 低 | 此任务为纯前端动画，后端无需实时更新。但需确保前端初始化的货船位置与后端逻辑（如 AIS 数据）不冲突。 |
...
*   **任务 1.1: 代码审查**
    *   **负责人:** 研究员/架构师
    *   **内容:** 审查 `src/frontend/digital-twin/main.js` 和 `PoseidonX.js`，定位：
        1.  场景初始化入口。
        2.  3D 对象（特别是货船）的加载和管理方式。
        3.  主动画循环 (`animate` 或 `requestAnimationFrame` 回调) 的结构。
        4.  双体船对象的引用和其世界坐标。
    *   **产出:** 一份简短的代码分析报告，指出需要修改或扩展的具体函数和代码行。

*   **任务 1.2: 模型验证**
    *   **负责人:** 研究员
    *   **内容:** 确认 `src/frontend/GLB_20251223141542.glb` 模型可以正常加载，并获取其默认朝向（通常为 Z 轴正方向或 X 轴正方向）。
    *   **产出:** 确认模型可用，并记录其默认朝向。

#### 阶段 2: 核心功能开发 (预估工时: 6小时)

*   **任务 2.1: 创建货船管理模块**
    *   **负责人:** 开发者
    *   **内容:** 在 `PoseidonX.js` 或创建一个新的模块（如 `src/frontend/digital-twin/CargoShipManager.js`）中，实现一个 `CargoShipManager` 类。
    *   **功能:**
        *   `addCargoShip(model, radius, angularSpeed)`: 加载货船模型，设置其初始位置，并存储运动参数。
        *   `update(deltaTime)`: 在每一帧被调用，根据 `deltaTime` 更新所有货船的角度 `angle += angularSpeed * deltaTime`，然后计算并更新其位置和朝向。
        *   `getShips()`: 返回所有货船对象的列表，供其他模块（如 2D 地图）使用。
    *   **产出:** 功能完整的 `CargoShipManager` 类。

*   **任务 2.2: 集成到主循环**
    *   **负责人:** 开发者
    *   **内容:** 在 `main.js` 或 `PoseidonX.js` 的 `animate` 函数中，实例化 `CargoShipManager`，并在每一帧调用其 `update` 方法。
    *   **产出:** 货船开始在 3D 场景中做圆周运动。

*   **任务 2.3: 实现船首向同步**
    *   **负责人:** 开发者
    *   **内容:** 在 `CargoShipManager.update()` 中，计算货船当前速度向量的方向（切线方向），并使用 `quaternion.setFromUnitVectors` 或 `lookAt` 方法将货船的朝向设置为该方向。确保旋转是平滑的，避免突变。
    *   **产出:** 货船在运动时，船首始终指向其前进方向。

#### 阶段 3: 配置与优化 (预估工时: 4小时)

*   **任务 3.1: 参数可配置化**
    *   **负责人:** 开发者
    *   **内容:** 将圆周运动的半径 (`radius`) 和角速度 (`angularSpeed`) 从硬编码改为可配置参数。可以从以下来源读取：
        1.  **UI 控件:** 在 `index.html` 或相关面板中添加滑块或输入框，允许用户实时调整。
        2.  **URL 参数:** 支持通过 URL 查询参数（如 `?radius=100&speed=0.5`）进行初始化配置。
        3.  **后端 API:** 从 `agent_team_api.py` 或类似的后端配置接口获取。
    *   **产出:** 运动参数可通过 UI 或外部配置动态调整。

*   **任务 3.2: 2D 地图同步**
    *   **负责人:** 开发者
    *   **内容:** 审查 `DigitalTwinMap.js`，找到更新 2D 地图上 AIS 目标位置的方法。将 `CargoShipManager` 中的货船位置和朝向数据传递给地图模块，使其在 2D 地图上也同步运动。
    *   **产出:** 3D 场景和 2D 地图上的货船位置保持同步。

*   **任务 3.3: 代码清理与注释**
    *   **负责人:** 开发者
    *   **内容:** 清理调试代码，为所有新函数和关键逻辑添加清晰的 JSDoc 注释。
    *   **产出:** 干净、可维护的代码。
...
*   **任务 4.2: 集成测试**
    *   **负责人:** 测试员/开发者
    *   **内容:** 确保新功能与现有系统（如 AR-CAS Pro、AIS 列表、天气系统）无冲突。
    *   **产出:** 集成测试报告。
...
1.  **代码变更:** 修改或新增的源代码文件（如 `PoseidonX.js`, `CargoShipManager.js`, `main.js` 等）。
2.  **配置文档:** 说明如何配置运动参数（UI 或 API）。
3.  **测试报告:** 功能测试和集成测试的结果。
...
**PM 备注:** 此任务为后续所有动态目标交互功能（如避碰、跟踪）的基石。请 Build 团队 PM 确认收到此任务分解，并指派具体开发人员。如有任何技术疑问或发现新的风险，请及时与我同步。
