...
### 功能概述
实现了货船以双体船为圆心做匀速圆周运动的功能，货船船头始终指向运动切线方向，速度适中。

### 变更文件清单

| 文件 | 变更类型 | 说明 |
|------|----------|------|
| `src/backend/channels/cargo_ship_orbit.py` | **新增** | 货船轨道运动控制 Channel |
| `src/frontend/js/darwin-ratchet.js` | **修改** | 新增 `cargo-orbit-catamaran-v1` 演化记录 |
| `src/backend/agent_team_api.py` | **修改** | 新增任务分配端点支持 |
| `src/backend/channels/execution_team_manager.py` | **修改** | 新增 Navigation Agent 对轨道 Channel 的监控 |
| `src/backend/channels/system_evolution.py` | **修改** | 新增轨道运动相关审查规则 |
...
#### `CargoShipOrbitChannel` 使用说明

**文件**: `src/backend/channels/cargo_ship_orbit.py`

**类**: `CargoShipOrbitChannel`

**描述**: 控制货船以双体船为圆心做匀速圆周运动，船头指向运动切线方向。
...
**API 端点**:
- `GET /api/v1/channels/cargo_ship_orbit/state` — 获取轨道状态
- `POST /api/v1/channels/cargo_ship_orbit/start` — 启动轨道运动
- `POST /api/v1/channels/cargo_ship_orbit/stop` — 停止轨道运动
- `POST /api/v1/channels/cargo_ship_orbit/radius` — 设置轨道半径
- `POST /api/v1/channels/cargo_ship_orbit/speed` — 设置轨道速度
- `POST /api/v1/channels/cargo_ship_orbit/reset` — 重置轨道

**依赖关系**:
- `wpc_attitude_control` — 依赖双体船姿态控制 Channel
...
**ID**: `cargo-orbit-catamaran-v1`

**描述**: 货船围绕双体船(原点)以半径80单位匀速圆周运动, 船头指向切线方向
...
**端点**: `POST /api/v1/agent-teams/build/assign`

**请求体**:
```json
{
    "agent_id": "build_pm",
    "task": "让货船以双体船为圆心，船头以圆切线，动起来，速度别太快"
}
```
...
#### Navigation Agent 新增监控目标

**Agent ID**: `exec_navigation`

**新增监控 Channel**:
- `cargo_ship_orbit` — 监控货船轨道运动状态
...
| 规则 ID | 描述 | 域 | 严重度 |
|---------|------|----|--------|
| `orbit-motion-consistency` | 检查货船轨道运动是否连续平滑 | GENERAL | MEDIUM |
| `orbit-heading-alignment` | 检查货船船头是否始终指向切线方向 | GENERAL | HIGH |
| `orbit-speed-limits` | 检查轨道速度是否在合理范围内 | GENERAL | LOW |
...
## 新增文档
1. `docs/channels/cargo_ship_orbit.md` — 货船轨道运动 Channel 使用文档
2. `docs/api/orbit_api.md` — 轨道运动 API 参考文档

## 更新文档
1. `docs/channels/README.md` — 新增 CargoShipOrbitChannel 条目
2. `docs/evolution/heritage.md` — 新增 cargo-orbit-catamaran-v1 演化项
3. `docs/agent-teams/build-team.md` — 新增任务分配示例
4. `docs/agent-teams/execution-team.md` — 更新 Navigation Agent 监控目标
5. `docs/evolution/audit-rules.md` — 新增轨道运动相关审查规则

## 代码注释更新
1. `src/backend/channels/cargo_ship_orbit.py` — 完整文档字符串
2. `src/frontend/js/darwin-ratchet.js` — 演化项描述更新
3. `src/backend/agent_team_api.py` — 任务分配端点文档
4. `src/backend/channels/execution_team_manager.py` — 监控目标文档
5. `src/backend/channels/system_evolution.py` — 审查规则文档
```
...
### 后端部署
1. 确保 `cargo_ship_orbit.py` 在 `src/backend/channels/` 目录下
2. 在 `main.py` 的 Channel 注册表中添加 `CargoShipOrbitChannel`
3. 重启 FastAPI 服务

### 前端部署
1. 确保 `darwin-ratchet.js` 已更新
2. 前端 3D 场景通过 WebSocket 或 REST API 获取货船位置数据
3. 使用 `CargoShipOrbitChannel` 的 `tick()` 方法输出更新货船位置

### 验证步骤
1. 启动后端服务
2. 调用 `GET /api/v1/channels/cargo_ship_orbit/state` 确认轨道状态
3. 观察前端 3D 场景中货船是否围绕双体船做圆周运动
4. 检查货船船头是否始终指向运动方向
