...
📝 提示词:
  ...
  ## ⚠️ 代码输出格式 (硬性规则，违反则代码不会被采纳)
  你的代码将被自动提取，保存到管线共享工作区，最终自动应用到项目代码库。
  
  ### 规则 A · Fence 必须顶格 (0 缩进)
  代码块的开闭 ``` 必须从行首开始，**前面不能有空格或制表符**。
  在引用前序步骤内容时也不要复述任何 ``` 块——会被解析器误识别。
  
  ### 规则 B · 路径标注
  每个代码块必须在开 fence 同一行标注目标文件路径，格式严格如下：
  ...
  ### 规则 C · 大文件最小化变更 (重要)
  如果目标文件**已存在且大于 200 行**（在 📂 项目上下文中你能看到原文）：
    - **禁止**输出整文件「从零重写」的版本——这会丢失项目里所有未修改代码
    - 改为输出一个**新建的辅助模块**（如 `src/frontend/digital-twin/cargo_orbit.js`），把新逻辑封装其中
    - 然后仅在需要 hook 的旧文件里输出**很小的 patch 片段**（用文字描述+几行 diff，不要整文件）
    - 系统有 shrink-replace 防护：新文件 < 旧文件 50% 时会自动拒绝覆盖
  
  ### 规则 D · 完整性
  - 路径必须是 📂 项目上下文 中列出的**实际文件路径**
  - 新文件输出**完整内容**；旧大文件按规则 C 处理
  - 不要使用 `...existing code...` 或 `// 省略`
  - 只输出真正需要变更的文件，不要复述未变文件
...
1. **前端模块**: `src/frontend/digital-twin/cargo_orbit.js` - 货船圆周运动逻辑
2. **后端Channel**: `src/backend/channels/cargo_orbit_channel.py` - 位置上报Channel

## 前端模块

```javascript // src/frontend/digital-twin/cargo_orbit.js
/**
 * cargo_orbit.js - 货船圆周运动模块
 * 
 * 创建一个货船 Mesh 并让它围绕双体船以500米半径、每秒0.5度角速度做圆周运动。
 * 独立模块，不修改 main.js 或 PoseidonX.js。
 * 
 * @version 1.0.0
 * @date 2026-03-12
 */
 ...
 * @param {THREE.Object3D} catamaranObject - 双体船对象（圆心参考）
 * @param {Object} options - 配置选项
 * @param {number} options.radius - 圆周半径（米，默认500）
 * @param {number} options.angularSpeed - 角速度（度/秒，默认0.5）
 * @param {number} options.initialAngle - 初始角度（度，默认0）
 * @param {string} options.cargoColor - 货船颜色（默认0x8B4513）
 * @param {number} options.cargoScale - 货船缩放（默认1.0）
 * @param {Function} options.onPositionUpdate - 位置更新回调 (lat, lon, course, speed) => void
 * @returns {Object} 控制对象 { cargoMesh, start, stop, getState, setAngle }
 */
export function setupCargoOrbit(scene, catamaranObject, options = {}) {
  // 默认配置
  const config = {
    radius: options.radius || 500,
    angularSpeed: options.angularSpeed || 0.5, // 度/秒
    initialAngle: options.initialAngle || 0,
    cargoColor: options.cargoColor || 0x8B4513,
    cargoScale: options.cargoScale || 1.0,
    onPositionUpdate: options.onPositionUpdate || null,
  };

  // 状态
  let currentAngle = config.initialAngle; // 当前角度（度）
  let isRunning = false;
  let animationId = null;
  let lastTimestamp = null;

  // 创建货船 Mesh
  const cargoMesh = createCargoShip(config.cargoColor, config.cargoScale);
  scene.add(cargoMesh);

  // 初始位置
  updatePosition(cargoMesh, catamaranObject, currentAngle, config.radius);

  /**
   * 创建货船几何体
   * @param {number} color - 颜色
   * @param {number} scale - 缩放
   * @returns {THREE.Mesh} 货船 Mesh
   */
  function createCargoShip(color, scale) {
    // 使用 BoxGeometry 组合成简单的货船形状
    const group = new THREE.Group();

    // 船体（主箱体）
    const hullGeo = new THREE.BoxGeometry(30 * scale, 8 * scale, 12 * scale);
    ...
    // 上层建筑（驾驶室）
    const cabinGeo = new THREE.BoxGeometry(10 * scale, 4 * scale, 8 * scale);
    ...
    cabin.position.set(5 * scale, 6 * scale, 0);
    group.add(cabin);

    // 烟囱
    const chimneyGeo = new THREE.CylinderGeometry(1.5 * scale, 2 * scale, 5 * scale, 8);
    const chimneyMat = new THREE.MeshPhongMaterial({
      color: 0x333333,
      flatShading: false,
    });
    const chimney = new THREE.Mesh(chimneyGeo, chimneyMat);
    chimney.position.set(8 * scale, 8 * scale, 0);
    group.add(chimney);

    // 甲板货物（集装箱）
    const containerColors = [0xFF4444, 0x44FF44, 0x4444FF, 0xFFFF44, 0xFF44FF];
    for (let i = 0; i < 4; i++) {
      const containerGeo = new THREE.BoxGeometry(4 * scale, 3 * scale, 4 * scale);
    ...
    wireframe.position.copy(hull.position);
    group.add(wireframe);
  ...
  /**
   * 更新货船位置
   * @param {THREE.Object3D} mesh - 货船对象
   * @param {THREE.Object3D} center - 圆心对象（双体船）
   * @param {number} angleDeg - 当前角度（度）
   * @param {number} radius - 半径
   */
  function updatePosition(mesh, center, angleDeg, radius) {
    const angleRad = (angleDeg * Math.PI) / 180;
    const centerPos = center.position;

    // 计算圆周位置
    const x = centerPos.x + radius * Math.cos(angleRad);
    const z = centerPos.z + radius * Math.sin(angleRad);

    mesh.position.set(x, centerPos.y, z);
    ...
    // 触发位置更新回调
    if (config.onPositionUpdate) {
      // 计算经纬度（基于双体船位置 + 偏移）
      const lat = centerPos.z + radius * Math.sin(angleRad);
      const lon = centerPos.x + radius * Math.cos(angleRad);
      const course = ((angleDeg + 90) % 360); // 航向 = 角度 + 90度（切线方向）
      const speed = (config.angularSpeed * Math.PI / 180) * radius; // 线速度 = 角速度 * 半径
  ...
  function animate(timestamp) {
    ...
    // 更新角度
    currentAngle += config.angularSpeed * deltaTime;
    if (currentAngle >= 360) {
      currentAngle -= 360;
    }
  ...
  function start() {
    ...
    lastTimestamp = null;
    animationId = requestAnimationFrame(animate);
    console.log('[CargoOrbit] 货船圆周运动已启动');
  }
  ...
  function stop() {
    ...
    lastTimestamp = null;
    console.log('[CargoOrbit] 货船圆周运动已停止');
  }
 ...
 * @returns {Object} 控制对象
 */
export function createAndStartCargoOrbit(scene, catamaranObject, options = {}) {
  const orbit = setupCargoOrbit(scene, catamaranObject, options);
  orbit.start();
  return orbit;
}

// 默认导出
export default setupCargoOrbit;
```

## 后端 Channel

```python // src/backend/channels/cargo_orbit_channel.py
"""
cargo_orbit_channel.py - 货船圆周运动 Channel

提供 CargoOrbitChannel(MarineChannel) 上报当前货船位置 (lat, lon, course, speed)。
与前端 cargo_orbit.js 配合，通过 WebSocket 接收货船位置数据并广播给订阅者。
...
class CargoOrbitChannel(MarineChannel):
    ...
    接收前端货船位置数据，并广播给所有订阅者。
    数据格式: { lat, lon, course, speed, angle, timestamp }
    """
    
    def __init__(self, channel_id: str = "cargo_orbit", **kwargs):
        """
        初始化 CargoOrbitChannel
        
        Args:
            channel_id: Channel 标识符 (默认: "cargo_orbit")
            **kwargs: 其他参数传递给 MarineChannel
        """
        super().__init__(channel_id=channel_id, **kwargs)
        ...
        # 更新间隔（秒）
        self._update_interval = 1.0  # 1秒更新一次
        
        # 后台任务
        self._update_task: Optional[asyncio.Task] = None
        
        logger.info(f"[CargoOrbitChannel] 初始化完成, channel_id={channel_id}")
    
    async def start(self) -> None:
        """启动 Channel"""
        await super().start()
        logger.info("[CargoOrbitChannel] 已启动")
    ...
    async def stop(self) -> None:
        ...
        await super().stop()
        logger.info("[CargoOrbitChannel] 已停止")
    ...
    async def handle_message(self, message: Dict[str, Any]) -> None:
        ...
        Args:
            message: 消息字典，包含货船位置数据
        """
        msg_type = message.get("type", "")
        
        if msg_type == "cargo_position_update":
            await self._handle_position_update(message.get("data", {}))
        elif msg_type == "cargo_orbit_control":
            await self._handle_control_command(message.get("data", {}))
        else:
            logger.debug(f"[CargoOrbitChannel] 未知消息类型: {msg_type}")
    ...
    async def _handle_position_update(self, data: Dict[str, Any]) -> None:
        ...
        Args:
            data: 位置数据 { lat, lon, course, speed, angle, timestamp }
        """
        # 更新状态
        self._cargo_state.update({
            "lat": data.get("lat", self._cargo_state["lat"]),
            "lon": data.get("lon", self._cargo_state["lon"]),
            "course": data.get("course", self._cargo_state["course"]),
            "speed": data.get("speed", self._cargo_state["speed"]),
            "angle": data.get("angle", self._cargo_state["angle"]),
            "is_moving": True,
        ...
        await self.broadcast({
            ...
            "data": self._cargo_state,
            "timestamp": int(time.time() * 1000),
        })
        
        logger.debug(f"[CargoOrbitChannel] 位置更新: lat={self._cargo_state['lat']:.4f}, "
                     f"lon={self._cargo_state['lon']:.4f}, "
                     f"course={self._cargo_state['course']:.1f}°, "
                     f"speed={self._cargo_state['speed']:.2f} m/s")
    ...
    async def _handle_control_command(self, data: Dict[str, Any]) -> None:
        ...
        Args:
            data: 控制命令 { command: "start"|"stop"|"set_angle", angle?: number }
        """
        command = data.get("command", "")
        
        if command == "start":
            self._cargo_state["is_moving"] = True
            logger.info("[CargoOrbitChannel] 货船运动已启动")
        elif command == "stop":
            self._cargo_state["is_moving"] = False
            logger.info("[CargoOrbitChannel] 货船运动已停止")
        elif command == "set_angle":
            angle = data.get("angle", 0)
            self._cargo_state["angle"] = angle % 360
            logger.info(f"[CargoOrbitChannel] 设置角度: {self._cargo_state['angle']:.1f}°")
        else:
            logger.warning(f"[CargoOrbitChannel] 未知控制命令: {command}")
        
        # 广播状态更新
        await self.broadcast({
            "type": "cargo_orbit_status",
            "channel": self.channel_id,
            "data": self._cargo_state,
            "timestamp": int(time.time() * 1000),
        })
    
    async def get_cargo_position(self) -> Dict[str, Any]:
        """
        获取当前货船位置
...
# 工厂函数
def create_cargo_orbit_channel(**kwargs) -> CargoOrbitChannel:
    """
    创建 CargoOrbitChannel 实例
    
    Args:
        **kwargs: 传递给 CargoOrbitChannel 的参数
    
    Returns:
        CargoOrbitChannel 实例
    """
    return CargoOrbitChannel(**kwargs)
```
...
import { setupCargoOrbit } from './digital-twin/cargo_orbit.js';

// 获取场景和双体船对象
const scene = ...; // 你的 Three.js 场景
const catamaran = ...; // 双体船对象

// 设置货船圆周运动
const cargoOrbit = setupCargoOrbit(scene, catamaran, {
  radius: 500,
  angularSpeed: 0.5,
  initialAngle: 0,
  cargoColor: 0x8B4513,
  onPositionUpdate: (pos) => {
    // 可选：通过 WebSocket 发送位置到后端
    console.log('货船位置:', pos);
  }
});
...
在 `src/backend/register_channels.py` 中注册：

```python
from src.backend.channels.cargo_orbit_channel import CargoOrbitChannel

# 注册 Channel
channels.register(CargoOrbitChannel(channel_id="cargo_orbit"))
```
