...
## 📋 任务概述
将货船围绕双体船的绕圈轨迹从圆形（半径80单位）改为椭圆形（长轴120单位，短轴60单位）。

## 🔍 变更分析

### 前序步骤产出
根据前序步骤（pm_decompose → research → architecture → develop → test），该任务**已经在前序步骤中完整实现**。Developer 和 Tester 均确认代码已到位并通过验证。

### 实际代码��更
经过对项目文件的审查，**未发现实际的代码变更**。前序步骤的 Developer 和 Tester 均判定该任务已在 `darwin-ratchet.js` 的 HERITAGE 记录中作为历史演化项存在，无需额外修改。

### 当前状态
- `darwin-ratchet.js` 中已有记录 `cargo-orbit-catamaran-v1`：货船以双体船为圆心做半径80单位匀速圆周运动
- **未找到椭圆轨迹的实际实现代码**

## ⚠️ 问题识别

前序步骤的 Developer 和 Tester 存在误判：
1. `darwin-ratchet.js` 中的 HERITAGE 记录是**历史记录**，不是实际功能代码
2. 货船绕圈的实际实现代码在 `src/frontend/digital-twin/` 目录下的 3D 场景管理文件中
3. 需要实际修改运动逻辑代码才能实现椭圆轨迹
...
| 文件 | 变更类型 | 说明 |
|------|----------|------|
| `src/frontend/digital-twin/main.js` | 修改 | 货船运动逻辑，将圆形轨迹改为椭圆 |
| `src/frontend/js/darwin-ratchet.js` | 修改 | 更新 HERITAGE 记录，添加椭圆轨迹演化项 |
| `src/backend/agent_team_api.py` | 新增 API | 添加椭圆参数配置端点（可选） |
...
git checkout -- src/frontend/digital-twin/main.js
git checkout -- src/frontend/js/darwin-ratchet.js
```
...
需要先找到货船运动控制的实际代码。根据项目结构，可能在以下文件中：

```bash
# 搜索货船运动相关代码
grep -r "cargo\|orbit\|绕圈\|圆周\|circle\|ellipse" src/frontend/digital-twin/ --include="*.js"
```

### 步骤 2：修改运动逻辑

在找到的货船运动控制代码中，将圆形轨迹参数：
```javascript
// 当前：圆形轨迹
const radius = 80;
...
```javascript
// 改为：椭圆轨迹
const semiMajorAxis = 120;  // 长轴半径
const semiMinorAxis = 60;   // 短轴半径
const x = semiMajorAxis * Math.cos(angle);
const z = semiMinorAxis * Math.sin(angle);
```

### 步骤 3：更新 Darwin Ratchet 记录

在 `darwin-ratchet.js` 中添加新的演化记录：
```javascript
{ id: 'cargo-orbit-ellipse-v1', title: '货船绕圈轨迹改为椭圆', category: 'scene', description: '长轴120单位(X轴), 短轴60单位(Z轴), 匀速角速度' }
```
...
for angle in [0, math.pi/4, math.pi/2, math.pi]:
    ...
    z = 60 * math.sin(angle)
    print(f'angle={angle:.2f}: x={x:.1f}, z={z:.1f}')
"
```
...
deployment:
  ...
  affected_services:
    - frontend-3d-scene
  files:
    - src/frontend/digital-twin/main.js (modify)
    - src/frontend/js/darwin-ratchet.js (modify)
  rollback:
    command: git checkout -- src/frontend/digital-twin/main.js src/frontend/js/darwin-ratchet.js
    estimated_time: 30s
  verification:
    - 货船沿椭圆轨迹运动
    - 长轴120单位（X轴方向）
...
由于前序步骤的 Developer 和 Tester 均判定"无需修改"，实际代码变更尚未执行。建议：

1. **立即执行实际代码修改**：找到货船运动控制代码并修改为椭圆轨迹
2. **更新 Darwin Ratchet**：添加椭圆轨迹的演化记录
3. **重新运行 QA 验证**：确保椭圆轨迹正确实现
