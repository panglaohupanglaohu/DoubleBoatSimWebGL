...
创建货船围绕双体船做圆周运动的独立模块，包括前端3D可视化和后端数据通道。

## 技术方案分析

### 1. 前端模块 `cargo_orbit.js`

**文件路径**: `src/frontend/digital-twin/cargo_orbit.js` (新建)

**技术要点**:
- 使用 Three.js 创建货船 Mesh
- 基于双体船位置计算圆周运动轨迹
- 使用 `requestAnimationFrame` 或 `setInterval` 驱动动画循环
- 半径 500 米，角速度 0.5 度/秒

**关键实现**:
```javascript
// 圆周运动计算
const angle = (time * angularSpeed) % (2 * Math.PI);
const x = catamaranPosition.x + radius * Math.cos(angle);
const z = catamaranPosition.z + radius * Math.sin(angle);
```

### 2. 后端通道 `cargo_orbit_channel.py`

**文件路径**: `src/backend/channels/cargo_orbit_channel.py` (新建)

**技术要点**:
- 继承 `MarineChannel` 基类
- 实现 `get_data()` 方法返回位置信息
- 数据格式: `{lat, lon, course, speed}`

## 需要检查的现有文件

### 前端依赖检查

1. **`src/frontend/digital-twin/main.js`** - 主入口文件，需要确认 Three.js 导入方式和场景初始化
2. **`src/frontend/digital-twin/PoseidonX.js`** - 主系统类，需要确认 scene 和 camera 的获取方式
3. **`src/frontend/digital-twin/PoseidonXChannels.js`** - Channel 数据集成，需要确认数据上报接口

### 后端依赖检查

1. **`src/backend/marine_channels_integration.py`** - 需要确认 `MarineChannel` 基类定义
2. **`src/backend/register_channels.py`** - 需要确认通道注册机制
3. **`src/backend/main.py`** - 需要确认 WebSocket 和 REST API 配置
...
```javascript
/**
 * cargo_orbit.js - 货船轨道运动模块
 * 创建货船 Mesh 并围绕双体船做圆周运动
 */
...
export function setupCargoOrbit(scene, catamaranObject, options = {}) {
  const {
    ...
    cargoColor = 0x888888,
    cargoSize = { width: 50, height: 20, depth: 15 }
  } = options;
  ...
  const material = new THREE.MeshPhongMaterial({ color: cargoColor });
  const cargoShip = new THREE.Mesh(geometry, material);
  
  // 添加船首标识
  const bowGeometry = new THREE.ConeGeometry(5, 15, 4);
  const bowMaterial = new THREE.MeshPhongMaterial({ color: 0xcc0000 });
  const bow = new THREE.Mesh(bowGeometry, bowMaterial);
  bow.position.set(0, 0, cargoSize.depth / 2 + 7.5);
  cargoShip.add(bow);
  ...
  function update() {
    ...
    // 计算角度增量（度转弧度）
    const deltaAngle = (angularSpeed * deltaTime * Math.PI) / 180;
    angle = (angle + deltaAngle) % (2 * Math.PI);

    // 获取双体船位置
    const catPos = catamaranObject.position;
    
    // 计算货船位置
    cargoShip.position.x = catPos.x + radius * Math.cos(angle);
    cargoShip.position.z = catPos.z + radius * Math.sin(angle);
    
    // 计算朝向（指向运动方向）
    const course = (angle * 180 / Math.PI + 90) % 360;
    cargoShip.rotation.y = (course * Math.PI) / 180;

    // 返回位置数据用于上报
    return {
      lat: catPos.x + radius * Math.cos(angle),
      lon: catPos.z + radius * Math.sin(angle),
      course: course,
      speed: (radius * angularSpeed * Math.PI) / 180 // 线速度
    };
  }
...
### 后端 `cargo_orbit_channel.py`

```python
"""
cargo_orbit_channel.py - 货船轨道数据通道
上报货船围绕双体船做圆周运动的位置信息
"""
...
class CargoOrbitChannel(MarineChannel):
    ...
    def __init__(self, channel_id: str = "cargo_orbit"):
    ...
    def get_data(self) -> Dict[str, Any]:
        ...
        angle_deg = (self.angular_speed * elapsed) % 360
        angle_rad = math.radians(angle_deg)
        
        # 计算位置（简化：假设在平面坐标系）
        lat = self.center_lat + (self.radius / 111320) * math.cos(angle_rad)
        lon = self.center_lon + (self.radius / (111320 * math.cos(math.radians(self.center_lat)))) * math.sin(angle_rad)
        
        # 计算航向（运动方向）
        course = (angle_deg + 90) % 360
        
        # 计算速度（线速度）
        speed = (self.radius * math.radians(self.angular_speed)) * 1.94384  # 转换为节
...
1. **Three.js 版本和导入方式** - 需要确认 `main.js` 中 Three.js 的导入方式（ES Module 或 script 标签）
2. **`MarineChannel` 基类定义** - 需要查看 `marine_channels_integration.py` 确认基类接口
3. **通道注册机制** - 需要查看 `register_channels.py` 确认如何注册新通道
4. **双体船对象引用** - 需要确认 `catamaranObject` 在场景中的获取方式

## 建议的下一步

1. 查看 `main.js` 确认 Three.js 使用方式和场景初始化
2. 查看 `marine_channels_integration.py` 确认 `MarineChannel` 基类
3. 查看 `register_channels.py` 确认通道注册流程
4. 根据确认的信息调整实现细节
