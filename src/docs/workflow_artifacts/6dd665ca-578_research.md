...
# 技术调研报告：货船轨道遥测 HUD 与后端 Channel

## 1. 任务概述

在 `src/frontend/digital-twin/main.js` 中为 cargo ship 的圆周运动添加 wabi-sabi 风格的 HUD overlay，显示当前角度和距离双体船的距离。同时新建 `src/backend/channels/cargo_orbit_telemetry.py`，继承 MarineChannel，通过 process_event 上报 cargo 当前 lat/lon。
...
- **第 1-15 行**: 导入 Three.js 和相关模块
- **第 18-50 行**: `state` 对象包含 `cargoShip` 属性（第 33 行），用于存储货船 3D 对象
- **第 72 行**: `createCargoShip()` 函数被调用，创建货船
- **第 74 行**: `createCargoOrbitHUD()` 函数被调用（**注意：代码中已存在此调用，但函数实现未在提供的代码片段中显示**）
- **第 100-105 行**: `animate()` 函数被调用，这是动画循环入口

**关键发现**：`createCargoOrbitHUD()` 已经在 `init()` 函数中被调用（第 74 行），但该函数的实现不在提供的代码片段中。这意味着：
1. 要么该函数在 main.js 的后续部分已实现
2. 要么该函数尚未实现，需要添加

### 2.2 后端 cargo_ship_orbit.py 分析

已存在完整的 `CargoShipOrbitChannel` 类，实现了：
- 货船绕双体船的圆周运动控制
- `tick()` 方法计算货船新位置
- 支持启动/停止/配置轨道参数
- 坐标转换逻辑（场景坐标 → 地理坐标）

### 2.3 后端 cargo_orbit_telemetry.py 分析

**文件已存在**（在项目上下文中显示），包含完整的 `CargoOrbitTelemetryChannel` 类：
- 继承 `MarineChannel`
- 实现 `process_event` 方法处理 `cargo_orbit_telemetry` 事件
- 将场景坐标 (x, z) 转换为地理坐标 (lat, lon)
- 包含遥测历史记录功能
...
| 文件路径 | 修改内容 |
|---------|---------|
| `src/frontend/digital-twin/main.js` | 实现 `createCargoOrbitHUD()` 函数，创建 wabi-sabi 风格 HUD overlay |

### 3.2 后端文件

| 文件路径 | 修改内容 |
|---------|---------|
| `src/backend/channels/cargo_orbit_telemetry.py` | **文件已存在**，但需要确认是否完整实现 |
| `src/backend/register_channels.py` | 注册新的 `CargoOrbitTelemetryChannel` |

## 4. 实现方案

### 4.1 前端 HUD 实现方案

在 `main.js` 中实现 `createCargoOrbitHUD()` 函数：

```javascript
function createCargoOrbitHUD() {
    // 创建 wabi-sabi 风格 HUD 容器
    ...
    angleRow.style.cssText = `display: flex; justify-content: space-between; margin-bottom: 8px;`;
    angleRow.innerHTML = `
        <span style="color: #8a7a6a; font-size: 11px; letter-spacing: 1px;">方位角</span>
        <span id="cargo-angle" style="color: #e8d8c0; font-size: 16px; font-weight: bold;">0.0°</span>
    `;
    hudContainer.appendChild(angleRow);

    // 距离显示
    const distRow = document.createElement('div');
    distRow.style.cssText = `display: flex; justify-content: space-between; margin-bottom: 8px;`;
    distRow.innerHTML = `
        <span style="color: #8a7a6a; font-size: 11px; letter-spacing: 1px;">距離</span>
        <span id="cargo-distance" style="color: #e8d8c0; font-size: 16px; font-weight: bold;">0.0 m</span>
    `;
    hudContainer.appendChild(distRow);
    ...
    divider.style.cssText = `
        ...
        margin: 10px 0;
    `;
    hudContainer.appendChild(divider);

    // 坐标显示
    const coordRow = document.createElement('div');
    coordRow.style.cssText = `display: flex; justify-content: space-between; margin-bottom: 6px;`;
    coordRow.innerHTML = `
        <span style="color: #8a7a6a; font-size: 10px; letter-spacing: 1px;">LAT</span>
        <span id="cargo-lat" style="color: #b0a090; font-size: 12px;">--</span>
    `;
    hudContainer.appendChild(coordRow);

    const lonRow = document.createElement('div');
    lonRow.style.cssText = `display: flex; justify-content: space-between;`;
    lonRow.innerHTML = `
        <span style="color: #8a7a6a; font-size: 10px; letter-spacing: 1px;">LON</span>
        <span id="cargo-lon" style="color: #b0a090; font-size: 12px;">--</span>
    `;
    hudContainer.appendChild(lonRow);
    ...
    state.cargoOrbitHUD = {
        ...
        angleEl: document.getElementById('cargo-angle'),
        distanceEl: document.getElementById('cargo-distance'),
        latEl: document.getElementById('cargo-lat'),
        lonEl: document.getElementById('cargo-lon'),
    };
}
```
...
function updateCargoOrbit() {
    if (!state.cargoShip || !state.boatMesh) return;

    // 获取双体船位置
    const catPos = state.boatMesh.position;

    // 计算货船轨道位置
    const radius = 80;
    const speed = 0.005; // rad/frame
    state.cargoOrbitAngle = (state.cargoOrbitAngle || 0) + speed;

    const angleRad = state.cargoOrbitAngle;
    const x = catPos.x + radius * Math.cos(angleRad);
    const z = catPos.z + radius * Math.sin(angleRad);

    state.cargoShip.position.set(x, catPos.y, z);
    state.cargoShip.lookAt(catPos.x, catPos.y, catPos.z);

    // 存储当前状态供 HUD 使用
    state.cargoOrbitState = {
        angleDeg: (state.cargoOrbitAngle * 180 / Math.PI) % 360,
        distance: radius,
        x: x,
        z: z,
    };
...
function updateCargoOrbitHUD() {
    if (!state.cargoOrbitHUD || !state.cargoOrbitState) return;

    const { angleDeg, distance, x, z } = state.cargoOrbitState;
    const hud = state.cargoOrbitHUD;

    hud.angleEl.textContent = `${angleDeg.toFixed(1)}°`;
    hud.distanceEl.textContent = `${distance.toFixed(1)} m`;

    // 坐标转换（与后端一致）
    const ORIGIN_LAT = 31.2304;
    const ORIGIN_LON = 121.4737;
    const SCENE_TO_DEG = 0.0001;
    const lat = ORIGIN_LAT + z * SCENE_TO_DEG;
    const lon = ORIGIN_LON + x * SCENE_TO_DEG;

    hud.latEl.textContent = `${lat.toFixed(6)}°N`;
    hud.lonEl.textContent = `${lon.toFixed(6)}°E`;
}
```
...
# 在 channel_registry 中添加
channel_registry.register(CargoOrbitTelemetryChannel())
```
...
1. **前端 HUD**: 使用纯 HTML/CSS 创建 overlay，不依赖 Three.js，实现简单
2. **后端 Channel**: 已有完整的 `CargoOrbitTelemetryChannel` 实现，只需注册
3. **坐标转换**: 前后端使用相同的转换逻辑，确保一致性

### 5.2 风险点

1. **main.js 代码片段不完整**: 提供的 main.js 只到第 100 行左右，`createCargoOrbitHUD()` 函数可能在后续代码中已实现
2. **动画循环**: 需要确认 `animate()` 函数的完整实现，确保正确插入更新逻辑
3. **状态管理**: 需要确保 `state.cargoShip` 和 `state.boatMesh` 在 HUD 更新时已初始化

### 5.3 建议

1. **先检查 main.js 完整内容**: 确认 `createCargoOrbitHUD()` 是否已实现
2. **使用已有的 cargo_ship_orbit.py**: 后端轨道控制逻辑已完整，前端应与之同步
3. **添加 WebSocket 通信**: 考虑将 HUD 数据通过 WebSocket 发送到后端，实现前后端数据同步

## 6. 总结

| 组件 | 状态 | 操作 |
|------|------|------|
| 前端 HUD (`createCargoOrbitHUD`) | 未实现 | 需要实现 wabi-sabi 风格 overlay |
| 前端动画更新 (`updateCargoOrbit`) | 未实现 | 需要添加轨道位置计算 |
| 前端 HUD 更新 (`updateCargoOrbitHUD`) | 未实现 | 需要添加 HUD 数据更新 |
| 后端 `cargo_orbit_telemetry.py` | 已存在 | 需要确认完整性并注册 |
| 后端 `register_channels.py` | 需要修改 | 注册新 Channel |
