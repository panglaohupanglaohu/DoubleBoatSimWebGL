...
# 技术方案设计 — 货船轨道遥测 HUD 与后端 Channel

## 1. 概述

本方案为 PoseidonX 数字孪生系统增加货船轨道遥测功能，包含：
- **前端**: 在 `main.js` 中添加 wabi-sabi 风格的 HUD overlay，显示货船当前角度和距离双体船的距离
- **后端**: 新建 `cargo_orbit_telemetry.py` Channel，将场景坐标转换为地理坐标并上报
...
│                                                             │
│  main.js                                                     │
│  ├── createCargoShip() ─── 创建货船 3D 模型                  │
│  ├── animate() ─────────── 每帧更新货船位置                   │
│  ├── createCargoOrbitHUD() ── 创建 wabi-sabi HUD overlay     │
│  └── updateCargoOrbitHUD() ── 更新 HUD 数据                  │
│                                                             │
│  HUD Overlay (HTML Element)                                  │
│  ├── 角度指示器 (罗盘风格)                                    │
│  ├── 距离显示 (米)                                           │
│  └── wabi-sabi 视觉风格 (手写字体、水墨质感、不对称布局)       │
└──────────────────────┬──────────────────────────────────────┘
                       │ WebSocket / REST API
                       ▼
┌─────────────────────────────────────────────────────────────┐
│                     后端 (FastAPI)                           │
│                                                             │
│  channels/cargo_orbit_telemetry.py                           │
│  ├── CargoOrbitTelemetryChannel                             │
│  │   ├── process_event() ─── 处理遥测上报事件                │
│  │   ├── _scene_to_geo() ─── 场景坐标→地理坐标转换           │
│  │   └── get_latest_telemetry() ── 获取最新遥测数据          │
│  └── 坐标转换常量                                            │
└─────────────────────────────────────────────────────────────┘
```
...
#### 3.1.1 创建函数: `createCargoOrbitHUD()`

在 `main.js` 的 `init()` 函数中已调用 `createCargoOrbitHUD()`，需要实现该函数。

**位置**: 在 `main.js` 中 `createCargoShip()` 和 `createIcebergs()` 之后

**功能**:
1. 创建 HTML div 元素作为 HUD 容器
2. 应用 wabi-sabi 风格 CSS
3. 显示货船当前角度 (0-360°)
4. 显示货船距离双体船的距离 (米)
5. 每帧更新数据

#### 3.1.2 更新函数: `updateCargoOrbitHUD()`

在 `animate()` 循环中调用，从 `state.cargoShip` 获取位置信息。

**数据来源**:
- `state.cargoShip.position` — 货船当前 3D 位置
- 双体船位置: `state.boatMesh.position` (或原点 `(0, 0, 0)`)
...
#### 3.1.3 wabi-sabi 风格设计

**视觉元素**:
- 背景: 半透明水墨质感 (使用 CSS 渐变和模糊)
- 字体: 手写风格 (Google Fonts: `Yuji Syuku` 或 `Klee One`)
- 边框: 不规则、不对称的边框 (使用 `clip-path` 或 SVG)
- 颜色: 墨色 (#2c3e50)、赭石 (#8B4513)、宣纸色 (#F5F0E1)
- 装饰: 水墨圆点、毛笔笔触分隔线
...
#cargo-orbit-hud {
    ...
    font-family: 'Klee One', 'Yuji Syuku', serif;
    color: #2c3e50;
    z-index: 1000;
...
#### 3.2.1 文件: `src/backend/channels/cargo_orbit_telemetry.py`

**类**: `CargoOrbitTelemetryChannel`
...
**坐标转换**:
- 原点: 31.2304°N, 121.4737°E (上海港外海)
- 转换因子: 1 场景单位 ≈ 0.0001 度 (约 11 米)

**数据流**:
```
前端 (每帧) → WebSocket → 后端 process_event() → 坐标转换 → 存储/上报
```
...
animate() 循环
    │
    ├── 更新 cargoShip.position (圆周运动)
    │
    ├── updateCargoOrbitHUD()
    │   ├── 计算角度和距离
    │   └── 更新 DOM 元素
    │
    └── (可选) 通过 WebSocket 发送遥测数据到后端
        └── process_event({type: "cargo_orbit_telemetry", x, z, angle_deg, distance})
```
...
| 文件 | 修改内容 |
|------|----------|
| `src/frontend/digital-twin/main.js` | 实现 `createCargoOrbitHUD()` 和 `updateCargoOrbitHUD()` 函数 |

### 4.2 需要新建的文件

| 文件 | 说明 |
|------|------|
| `src/backend/channels/cargo_orbit_telemetry.py` | 货船轨道遥测 Channel (已存在，需确认完整性) |
...
function createCargoOrbitHUD() {
    // 创建 DOM 元素
    // 应用 wabi-sabi 样式
    // 存储到 state.cargoOrbitHUD
}

// 更新 HUD (每帧调用)
function updateCargoOrbitHUD() {
    const cargo = state.cargoShip;
    const cat = state.boatMesh;
    if (!cargo || !cat) return;
    
    const dx = cargo.position.x - cat.position.x;
    const dz = cargo.position.z - cat.position.z;
    const distance = Math.sqrt(dx*dx + dz*dz);
    const angleDeg = (Math.atan2(dz, dx) * 180 / Math.PI + 360) % 360;
    
    // 更新 DOM
    state.cargoOrbitHUD.angleEl.textContent = angleDeg.toFixed(1) + '°';
    state.cargoOrbitHUD.distanceEl.textContent = distance.toFixed(1) + 'm';
}
```
...
event = {
    ...
    "distance": float,   # 距双体船距离 (场景单位)
    "heading_deg": float # 货船朝向 (度)
}
...
1. 在 `main.js` 中找到 `createCargoOrbitHUD()` 函数位置
2. 实现 HUD 创建逻辑:
   - 创建 div 容器
   - 添加角度和距离显示元素
   - 应用 wabi-sabi 样式
...
1. 在 `animate()` 函数中找到货船位置更新代码
2. 添加 `updateCargoOrbitHUD()` 调用
3. 实现角度和距离计算

### 步骤 3: 确认后端 Channel

1. 检查 `cargo_orbit_telemetry.py` 是否完整
2. 确保 `MarineChannel` 基类正确导入
3. 验证坐标转换逻辑

### 步骤 4: 集成测试

1. 启动前端，确认 HUD 显示
2. 观察货船圆周运动时 HUD 数据更新
3. 测试后端 Channel 的事件处理

## 7. 注意事项

1. **性能**: HUD 更新在动画循环中，避免 DOM 操作过于频繁
2. **兼容性**: 确保 wabi-sabi 字体在浏览器中可用 (使用 Google Fonts CDN)
3. **响应式**: HUD 位置应适应不同屏幕尺寸
4. **可访问性**: 添加适当的 ARIA 标签

## 8. 扩展建议

1. 添加货船轨迹历史显示 (在 HUD 或 3D 场景中)
2. 支持 HUD 拖拽和缩放
3. 添加更多遥测数据 (速度、航向、ETA)
