...
让货船(cargo ship)以双体船为圆心做圆周轨道运动，半径80m，角速度0.015 rad/帧。需要修改 `src/frontend/digital-twin/main.js` 文件。

## 文件分析

### 目标文件: `src/frontend/digital-twin/main.js`

根据项目上下文，该文件是数字孪生系统的主入口文件。需要分析其现有结构，找到货船(cargo ship)和双体船(catamaran)的3D对象引用，然后在动画循环中添加圆周运动逻辑。

### 相关文件分析

1. **`src/frontend/digital-twin/main.js`** - 主入口文件，包含场景初始化、动画循环
2. **`src/frontend/digital-twin/PoseidonXIntegration.js`** - 集成模块，包含shipController引用
3. **`src/frontend/digital-twin/demo.js`** - 可能包含演示逻辑
4. **`src/frontend/digital-twin/index.js`** - 可能包含索引逻辑

## 技术方案

### 方案概述

在 `main.js` 的动画循环中，为货船(cargo ship)添加圆周运动逻辑：
- 圆心：双体船(catamaran)的位置
- 半径：80米
- 角速度：0.015 rad/帧
- 使用参数方程：`x = centerX + radius * cos(angle)`, `z = centerZ + radius * sin(angle)`
...
需要在 `main.js` 中找到以下关键部分：

1. **货船对象的引用** - 需要找到 `cargoShip` 或类似命名的3D对象变量
2. **双体船对象的引用** - 需要找到 `catamaran` 或类似命名的3D对象变量
3. **动画循环** - 找到 `animate()` 或 `update()` 函数

### 实现代码（伪代码）

```javascript
// 在动画循环中添加
let cargoAngle = 0; // 初始角度

function animate() {
    // 获取双体船位置作为圆心
    const centerX = catamaran.position.x;
    const centerZ = catamaran.position.z;
    
    // 更新货船位置
    cargoAngle += 0.015; // 角速度 0.015 rad/帧
    cargoShip.position.x = centerX + 80 * Math.cos(cargoAngle);
    cargoShip.position.z = centerZ + 80 * Math.sin(cargoAngle);
    
    // 让货船朝向运动方向（可选）
    cargoShip.lookAt(centerX, cargoShip.position.y, centerZ);
...
1. **技术可行性**：✅ 完全可行。Three.js 支持直接修改对象位置属性
2. **性能影响**：✅ 极低。仅需每帧计算两个三角函数
3. **兼容性**：✅ 不破坏现有功能，仅添加新行为
4. **验证方式**：✅ 刷新页面后观察货船是否做圆周运动

## 风险与注意事项

1. 需要确认 `main.js` 中货船和双体船的变量命名
2. 如果货船是GLB模型加载的，需要确保 `position` 属性可写
3. 角速度0.015 rad/帧在60fps下约为0.9 rad/s，约51.6°/s，视觉上会明显可见

## 结论

任务可行，仅需修改 `src/frontend/digital-twin/main.js` 一个文件，在动画循环中添加圆周运动逻辑即可。
