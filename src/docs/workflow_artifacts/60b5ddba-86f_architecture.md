...
📝 提示词:
  ...
  **子任务拆解:**
    ...
    - *状态：** 规划中
    - *负责人：** [待分配 - Build团队PM]
    - **碰撞检测：** 实时计算本船与目标船（货船）之间的最近会遇点 (CPA) 和到达最近会遇点时间 (TCPA)。
    - **避让决策：** 当检测到碰撞风险（CPA < 安全距离 且 TCPA < 时间阈值）时，自动生成并执行避让动作（如调整航向/航速）。
    - **可视化预警：** 在前端界面上，对存在碰撞风险的目标进行高亮显示，并提供风险等级提示。
...
好的，作为 PoseidonX 系统的架构师 (Architect)，我将基于前序步骤的产出（PM分解和技术调研），为Build团队设计一份详细、可执行的技术方案。
...
### 1.1. 目标
为PoseidonX系统增加碰撞检测与避让功能，实时监控本船（双体船）与目标船（货船）的碰撞风险，并在必要时提供可视化预警和避让建议，确保航行安全。

### 1.2. 核心原则
- **实时性：** 核心计算（CPA/TCPA）必须在毫秒级完成。
- **渐进性：** 采用分阶段实施策略，先实现核心功能，再逐步完善。
- **可扩展性：** 架构设计需支持未来集成更复杂的COLREGs规则引擎。
- **用户友好：** 预警信息清晰、直观，不干扰正常操作。

## 2. 系统架构

本方案采用前后端分离架构，后端负责核心计算，前端负责数据展示和用户交互。
...
#### 3.1.1. 新增文件：`src/backend/app/services/collision_service.py`

**功能：** 实现核心碰撞检测和避让逻辑。

**核心函数：**

1.  **`calculate_cpa_tcpa(own_ship: ShipState, target_ship: ShipState) -> Tuple[float, float]`**
    -   **输入：** 本船和目标船的 `ShipState` 对象（包含经纬度、航向、航速）。
    -   **算法：** 使用标准海事CPA/TCPA计算公式。
        -   将经纬度转换为笛卡尔坐标（或使用`geopy`库）。
        -   计算相对速度向量。
        -   计算最近会遇点的时间和距离。
    -   **输出：** `(cpa_distance_nm, tcpa_minutes)`。

2.  **`assess_collision_risk(cpa: float, tcpa: float) -> str`**
    -   **逻辑：**
        -   `cpa < 1.0` 且 `tcpa < 15.0` -> `"high"`
        -   `cpa < 2.0` 且 `tcpa < 30.0` -> `"medium"`
        -   其他 -> `"low"`
    -   **输出：** 风险等级字符串。

3.  **`generate_avoidance_suggestion(own_ship: ShipState, target_ship: ShipState, risk_level: str) -> str`**
    -   **逻辑（Phase 1）：** 如果风险为 `"high"`，建议本船向右转向10度（符合COLREGs基本规则）。
    -   **输出：** 字符串，如 `"建议：向右转向至147.9°"`。

#### 3.1.2. 新增文件：`src/backend/app/api/collision.py`
...
```json
{
  ...
  "cpa": 0.5,
  "tcpa": 8.2,
  "suggestion": "建议：向右转向至147.9°"
}
```

#### 3.1.3. 修改文件：`src/backend/app/schemas.py`
...
#### 3.1.4. 修改文件：`src/backend/app/main.py`
...
#### 3.2.1. 新增文件：`src/frontend/js/collision.js`

**功能：** 管理碰撞检测逻辑和UI更新。

**核心函数：**

1.  **`checkCollision(ownShip, targetShip)`**
    -   调用后端API `POST /api/v1/collision/check`。
    -   使用 `fetch` 或 `axios` 发送请求。
    -   返回 `CollisionCheckResponse` 对象。

2.  **`updateUI(collisionData)`**
    -   根据 `collisionData.collision_risk` 更新预警面板的显示/隐藏和样式。
    -   更新面板中的文本内容（MMSI、CPA、TCPA、建议）。
    -   触发地图组件的 `highlightTarget` 方法。

3.  **`startPeriodicCheck(interval)`**
    -   使用 `setInterval` 定期执行碰撞检测。
    -   从全局状态或地图组件获取本船和目标船的最新状态。

#### 3.2.2. 修改文件：`src/frontend/js/map.js`

**修改点：** 在 `renderTargets` 或类似函数中，为每个目标船添加一个 `riskLevel` 属性。

**逻辑：**
```javascript
// 假设 collisionData 包含 riskLevel
function renderTargets(targets, collisionData) {
    targets.forEach(target => {
        const riskLevel = collisionData[target.mmsi]?.collision_risk || 'low';
        // 根据 riskLevel 设置绘制颜色
        let color = 'green';
        ...
        // 绘制目标船，使用 color 变量
        drawShip(target, color);
    });
}
```

#### 3.2.3. 修改文件：`src/frontend/index.html`
...
#### 3.2.4. 修改文件：`src/frontend/css/style.css`
...
### 4.1. Sprint 1: 核心功能实现 (3天)

1.  **后端开发：**
    -   创建 `collision_service.py`，实现CPA/TCPA计算和风险等级评估。
    -   创建 `collision.py` API端点。
    -   修改 `schemas.py` 和 `main.py`。
2.  **前端开发：**
    -   创建 `collision.js`，实现API调用和UI更新逻辑。
    -   修改 `index.html` 和 `style.css`，添加预警面板。
3.  **测试：**
    -   使用模拟数据测试API端点的正确性。
    -   验证前端能否正确显示预警信息。

### 4.2. Sprint 2: 避让逻辑与地图集成 (2天)

1.  **后端开发：**
    -   在 `collision_service.py` 中实现 `generate_avoidance_suggestion` 函数。
2.  **前端开发：**
    -   修改 `map.js`，根据风险等级高亮显示目标船。
    -   在预警面板中显示避让建议。
3.  **测试：**
    -   模拟碰撞场景，验证避让建议是否合理。
    -   验证地图高亮效果。

### 4.3. Sprint 3: 优化与COLREGs集成 (2天)

1.  **后端开发：**
    -   评估并引入COLREGs规则引擎（如`colregs`库）。
    -   根据会遇场景（交叉、对遇、追越）生成更精确的避让建议。
2.  **前端开发：**
    -   优化预警面板交互（如用户确认、手动调整）。
    -   添加历史碰撞事件记录。
3.  **测试：**
...
| 风险 | 可能性 | 影响 | 缓解措施 |
| :--- | :--- | :--- | :--- |
| AIS数据延迟或错误 | 中 | 高 | 实现数据滤波（卡尔曼滤波）；在前端显示数据置信度；与AIS数据源团队沟通。 |
| COLREGs规则复杂 | 高 | 中 | 分阶段实施；Phase 1仅实现基本规则；后续迭代逐步完善。 |
| 前端性能问题 | 低 | 中 | 优化Canvas渲染；使用Web Worker进行数据处理。 |

## 6. 交付物清单

-   `src/backend/app/services/collision_service.py`
-   `src/backend/app/api/collision.py`
-   `src/backend/app/schemas.py` (修改)
-   `src/backend/app/main.py` (修改)
-   `src/frontend/js/collision.js`
-   `src/frontend/index.html` (修改)
-   `src/frontend/css/style.css` (修改)
-   `src/frontend/js/map.js` (修改)
-   单元测试报告
-   集成测试报告

---

**请Build团队PM根据此技术方案，分配具体负责人并开始执行。如有任何技术疑问，请及时与我沟通。**
