...
好的，Deployer 收到任务。作为 PoseidonX 系统的 DevOps 部署工程师，我将基于前序所有步骤（PM分解、技术调研、架构设计、开发、测试）的产出，对本次变更进行分析并制定部署策略。

---

## 部署报告: TASK-2025-AR-001

### 1. 变更分析

本次开发任务为在 `AR CAS Pro` 菜单中集成雷达图，涉及前后端多个文件的修改和新增。

- **后端变更:**
    - **新增文件:** `src/backend/app/utils/geo_utils.py` (地理计算工具)
    - **修改文件:**
        - `src/backend/app/routers/ais_data.py` (新增 WebSocket 端点)
        - `src/backend/app/services/ais_data_service.py` (新增数据过滤函数)
        - `src/backend/app/services/collision_risk_service.py` (修改风险计算逻辑)
- **前端变更:**
    - **修改文件:**
        - `src/frontend/js/radar_chart.js` (完善雷达图渲染组件)
        - `src/frontend/js/ar_cas_pro.js` (新增 WebSocket 连接与数据更新逻辑)
        - `src/frontend/pages/ar_cas_pro.html` (添加雷达图容器)

**影响范围评估:**
- **API 签名变更:** 是。新增了 WebSocket 端点 `/ws/radar/{mmsi_list}`。
- **核心 Channel 逻辑变更:** 否。AIS 数据服务逻辑未发生根本性改变，只是新增了过滤和计算功能。
- **新增/大幅修改 HTML 页面:** 是。`ar_cas_pro.html` 页面被修改，新增了雷达图容器。
- **整体评估:** 这是一个**较大功能变更**，涉及前后端协同工作，且新增了 API 端点。
...
**理由:**
1.  **API 签名变更:** 新增了 WebSocket 端点，属于接口变更，需要平滑过渡。
2.  **前端页面修改:** `ar_cas_pro.html` 的修改可能影响现有用户的使用体验，蓝绿部署可以快速回滚。
3.  **降低风险:** 蓝绿部署可以让我们在“绿环境”中验证新功能，确认无误后再将流量切换到新版本，最大限度地降低对线上用户的影响。

### 3. 部署清单

| 步骤 | 操作 | 详细说明 |
| :--- | :--- | :--- |
| **1. 环境准备** | 部署新版本到“绿环境” | 将本次变更的所有代码部署到与生产环境（蓝环境）隔离的“绿环境”中。 |
| **2. 数据库/配置** | 检查配置 | 确认“绿环境”的后端配置（如 WebSocket 端口、AIS 数据源地址）与“蓝环境”一致。 |
| **3. 后端部署** | 重启后端服务 | 在“绿环境”中重启 FastAPI 服务，使新的 WebSocket 端点和逻辑生效。 |
| **4. 前端部署** | 更新静态文件 | 将修改后的 `radar_chart.js`、`ar_cas_pro.js` 和 `ar_cas_pro.html` 部署到“绿环境”的 Web 服务器。 |
| **5. 功能验证** | 执行冒烟测试 | 在“绿环境”中访问 `ar_cas_pro` 页面，验证雷达图是否正常显示、数据是否每2秒刷新、碰撞风险提示是否生效。 |
| **6. 流量切换** | 切换负载均衡器 | 将负载均衡器的流量从“蓝环境”切换到“绿环境”。 |
| **7. 监控与观察** | 监控系统指标 | 密切监控新环境的错误率、响应时间、WebSocket 连接数等指标，持续观察 15-30 分钟。 |
| **8. 清理** | 保留“蓝环境” | 保留“蓝环境”作为回滚目标，不要立即销毁。 |

### 4. 回滚方案

如果新版本（绿环境）出现严重问题，执行以下回滚操作：

1.  **立即切换:** 将负载均衡器的流量从“绿环境”**立即切回**“蓝环境”。
2.  **问题定位:** 在“绿环境”中定位并修复问题。
3.  **重新部署:** 修复后，重复上述部署流程，将修复后的版本部署到“绿环境”并进行验证。
4.  **再次切换:** 验证通过后，再次将流量切换到“绿环境”。

**回滚影响:** 回滚期间，用户将短暂（秒级）回到旧版本界面，雷达图功能将暂时不可用，但系统核心功能不受影响。

### 5. 影响文件清单

| 文件路径 | 操作类型 | 说明 |
| :--- | :--- | :--- |
| `src/backend/app/utils/geo_utils.py` | **新增** | 新增地理计算工具模块 |
| `src/backend/app/routers/ais_data.py` | **修改** | 新增 WebSocket 端点 |
| `src/backend/app/services/ais_data_service.py` | **修改** | 新增数据过滤函数 |
| `src/backend/app/services/collision_risk_service.py` | **修改** | 修改风险计算逻辑 |
| `src/frontend/js/radar_chart.js` | **修改** | 完善雷达图渲染组件 |
| `src/frontend/js/ar_cas_pro.js` | **修改** | 新增 WebSocket 连接逻辑 |
| `src/frontend/pages/ar_cas_pro.html` | **修改** | 添加雷达图容器 |

---

**结论:** 本次变更为较大功能变更，建议采用**蓝绿部署**策略。部署清单和回滚方案已准备就绪，可以开始执行部署。
