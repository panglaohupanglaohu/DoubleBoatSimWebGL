...
### 任务概述
让货船以双体船为圆心动起来 — 在 3D 场景中实现货船围绕双体船做圆周运动。

### 前序步骤产出分析

| 步骤 | Agent | 产出 |
|------|-------|------|
| pm_decompose | PM | 任务分解：3D 场景动画、轨道计算、控制面板 |
| research | Researcher | 技术调研：Three.js 动画、轨道数学、性能优化 |
| architecture | Architect | 架构设计：CargoShipOrbitChannel、轨道控制器 |
| develop | Developer | 代码实现：新增 `cargo-ship-orbit.js`、修改 `digital-twin.html` |

### 变更分析

#### 新增文件
1. **`src/frontend/js/cargo-ship-orbit.js`** — 货船轨道动画核心逻辑
   - 大小: ~15KB
   - 功能: 轨道计算、动画循环、速度控制、暂停/恢复

#### 修改文件
1. **`src/frontend/digital-twin.html`** — 数字孪生主页面
   - 变更: 引入 cargo-ship-orbit.js、添加轨道控制 UI
   - 影响范围: 页面加载、3D 场景初始化
...
|---------|---------|
| 新增 HTML 页面 | ❌ 否 |
| 大幅修改 HTML 页面 | ✅ 是 (digital-twin.html 新增轨道控制) |
| API 签名变更 | ❌ 否 |
| 核心 Channel 逻辑变更 | ❌ 否 |
| 新增 JS 文件 | ✅ 是 (cargo-ship-orbit.js) |

**结论: 蓝绿部署** — 涉及前端页面修改和新 JS 文件，需要蓝绿部署确保平滑切换。
...
#### 步骤 1: 准备新版本 (Blue → Green)

```bash
# 1. 备份当前版本
cp src/frontend/digital-twin.html src/frontend/digital-twin.html.bak.$(date +%Y%m%d_%H%M%S)

# 2. 部署新 JS 文件
cp storage/pipeline_runs/<task_id>/04_develop/code/cargo-ship-orbit.js src/frontend/js/cargo-ship-orbit.js

# 3. 部署修改后的 HTML
cp storage/pipeline_runs/<task_id>/04_develop/code/digital-twin.html src/frontend/digital-twin.html
```
...
```bash
# 1. 检查 JS 文件完整性
ls -la src/frontend/js/cargo-ship-orbit.js
# 预期: 文件存在且大小 > 5KB

# 2. 检查 HTML 引用
grep -n "cargo-ship-orbit" src/frontend/digital-twin.html
# 预期: 找到 script 引用

# 3. 检查 HTML 语法
python3 -c "
import html.parser
with open('src/frontend/digital-twin.html', 'r') as f:
    content = f.read()
...
```bash
# 1. 检查页面加载
curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/digital-twin.html
# 预期: 200

# 2. 检查 JS 加载
curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/js/cargo-ship-orbit.js
# 预期: 200

# 3. 检查控制台无错误
# 打开浏览器开发者工具，确认无 404/500 错误
```
...
| 文件 | 操作 | 风险等级 | 说明 |
|------|------|---------|------|
| `src/frontend/js/cargo-ship-orbit.js` | **新增** | 🟢 低 | 独立 JS 模块，不影响现有功能 |
| `src/frontend/digital-twin.html` | **修改** | 🟡 中 | 新增 script 引用和 UI 控件 |

### 依赖关系
```
digital-twin.html
  └── cargo-ship-orbit.js (新增)
  └── digital-twin/main.js (已有)
  └── digital-twin/PoseidonX.js (已有)
```
...
```bash
# 1. 立即回滚 HTML
cp src/frontend/digital-twin.html.bak.$(date +%Y%m%d_%H%M%S) src/frontend/digital-twin.html

# 2. 删除新 JS 文件
rm src/frontend/js/cargo-ship-orbit.js
...
### 部署后验证清单
- [ ] digital-twin.html 加载正常
- [ ] cargo-ship-orbit.js 加载正常
- [ ] 3D 场景正常渲染
- [ ] 货船围绕双体船做圆周运动
- [ ] 轨道控制 UI 正常工作
- [ ] 速度调节功能正常
...
### 性能考虑
- 货船轨道动画使用 `requestAnimationFrame`，与现有 3D 渲染循环兼容
- 轨道计算使用 `Math.sin/cos`，性能开销极小
- 建议监控 FPS，确保动画不影响整体场景性能
