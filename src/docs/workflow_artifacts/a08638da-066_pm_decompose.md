...
| 子任务 | 描述 | 优先级 | 预估工时 |
|--------|------|--------|----------|
| 2.1.1 确定椭圆参数 | 定义椭圆的长轴半径、短轴半径、旋转角度 | P0 | 0.5h |
| 2.1.2 确定运动逻辑 | 定义匀速运动还是变速运动（开普勒定律） | P1 | 0.5h |
| 2.1.3 确定配置方式 | 决定椭圆参数是硬编码还是可配置 | P1 | 0.5h |
...
| 子任务 | 描述 | 依赖 | 预估工时 |
|--------|------|------|----------|
| 2.2.1 定位轨迹控制代码 | 查找货船运动控制的源代码位置 | 无 | 1h |
| 2.2.2 分析现有圆形轨迹实现 | 理解当前圆周运动的数学实现 | 2.2.1 | 1h |
| 2.2.3 确认 Three.js 版本和 API | 确保使用的 Three.js API 兼容 | 2.2.1 | 0.5h |
...
| 子任务 | 描述 | 依赖 | 预估工时 |
|--------|------|------|----------|
| 2.3.1 修改轨迹计算逻辑 | 将圆形参数方程改为椭圆参数方程 | 2.2.2 | 2h |
| 2.3.2 调整船头朝向 | 确保船头始终指向椭圆切线方向 | 2.3.1 | 1h |
| 2.3.3 添加椭圆参数配置接口 | 创建后端 API 用于动态调整椭圆参数 | 2.1.3 | 2h |
| 2.3.4 前端配置面板 | 在 UI 中添加椭圆参数调节控件 | 2.3.3 | 1.5h |

**数学公式**：
- 椭圆参数方程：`x = a * cos(θ)`, `z = b * sin(θ)`
- 切线方向向量：`(-a * sin(θ), 0, b * cos(θ))`
- 其中 a = 长轴半径，b = 短轴半径，θ = 当前角度

### 2.4 测试验证阶段

| 子任务 | 描述 | 依赖 | 预估工时 |
|--------|------|------|----------|
| 2.4.1 视觉验证 | 确认货船沿椭圆轨迹运动，船头指向正确 | 2.3.1, 2.3.2 | 1h |
| 2.4.2 边界测试 | 测试极端椭圆参数（a >> b 或 a ≈ b） | 2.3.1 | 0.5h |
| 2.4.3 性能测试 | 确保椭圆轨迹计算不影响帧率 | 2.3.1 | 0.5h |
| 2.4.4 配置接口测试 | 测试后端 API 参数调整功能 | 2.3.3 | 1h |

### 2.5 文档与部署

| 子任务 | 描述 | 依赖 | 预估工时 |
|--------|------|------|----------|
| 2.5.1 更新 Darwin Ratchet 记录 | 记录椭圆轨迹作为新的演化项 | 2.3.1 | 0.5h |
| 2.5.2 编写技术文档 | 记录椭圆参数说明和配置方法 | 2.3.3 | 1h |
| 2.5.3 代码审查 | 提交 PR 进行代码审查 | 2.3.x | 1h |
...
2.1.2 运动逻辑确定
    ↓
2.2.1 定位轨迹控制代码 ───→ 2.2.2 分析现有实现 ───→ 2.2.3 确认 Three.js API
                                    ↓
2.1.3 配置方式确定 ──────→ 2.3.1 修改轨迹计算逻辑
                                    ↓
                            2.3.2 调整船头朝向
                                    ↓
                            2.3.3 后端配置 API ───→ 2.3.4 前端配置面板
                                    ↓
                            2.4.x 测试验证
                                    ↓
                            2.5.x 文档与部署
...
| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|----------|
| 货船运动代码分散在多个文件 | 中 | 高 | 先全局搜索 `cargo`、`orbit`、`circle` 等关键词定位 |
| 椭圆轨迹导致船头朝向异常 | 中 | 中 | 使用切线向量公式，在开发环境中实时调试 |
| 椭圆参数硬编码后难以调整 | 低 | 中 | 优先实现后端 API 配置接口 |
| 椭圆轨迹与碰撞检测冲突 | 低 | 高 | 保持原有碰撞检测逻辑，仅修改运动轨迹 |
| 性能下降（大量三角函数计算） | 低 | 低 | 使用预计算或缓存角度值 |
...
```bash
# 搜索货船相关代码
grep -r "cargo" --include="*.js" --include="*.html" src/frontend/

# 搜索圆周运动相关代码
grep -r "orbit\|circle\|circular\|radius" --include="*.js" src/frontend/

# 搜索双体船相关代码
grep -r "catamaran\|双体船" --include="*.js" --include="*.html" src/frontend/
```

### 5.2 可能涉及的源文件

根据项目结构分析，以下文件可能包含货船运动控制逻辑：

| 文件路径 | 可能性 | 说明 |
|----------|--------|------|
| `src/frontend/digital-twin/DigitalTwinMap.js` | 高 | 数字孪生地图，可能包含目标运动逻辑 |
| `src/frontend/digital-twin/PoseidonX.js` | 中 | 核心系统，可能管理所有实体运动 |
| `src/frontend/digital-twin/main.js` | 中 | 主入口，可能初始化场景和实体 |
| `src/frontend/digital-twin/demo.js` | 低 | 演示文件，可能包含示例代码 |
| `src/frontend/worldmonitor-map.html` | 中 | 世界监控地图，可能包含货船显示 |
| `src/frontend/navigation-v3.html` | 中 | 导航页面，可能包含船舶运动 |

### 5.3 关键代码模式

在现有圆形轨迹中，预期会找到类似以下模式的代码：
...
// 船头指向切线方向
cargo.lookAt(centerX, 0, centerZ); // 或使用切线向量
```
...
// 椭圆轨迹（目标）
const angle = time * angularSpeed;
const x = centerX + a * Math.cos(angle);  // a = 长轴半径
const z = centerZ + b * Math.sin(angle);  // b = 短轴半径
cargo.position.set(x, 0, z);
// 船头指向椭圆切线方向
const tangentX = -a * Math.sin(angle);
const tangentZ = b * Math.cos(angle);
cargo.lookAt(x + tangentX, 0, z + tangentZ);
```
...
如果需要通过后端配置椭圆参数，建议在 `agent_team_api.py` 中添加：
...
class EllipseConfig(BaseModel):
    center_x: float = 0.0
    center_z: float = 0.0
    semi_major_axis: float = 120.0  # 长轴半径
    semi_minor_axis: float = 60.0   # 短轴半径
    rotation_angle: float = 0.0     # 椭圆旋转角度（弧度）
    angular_speed: float = 0.01     # 角速度

# API 端点
@router.get("/cargo/orbit/config")
async def get_cargo_orbit_config():
    """获取货船椭圆轨迹配置"""
    return _cargo_orbit_config

@router.post("/cargo/orbit/config")
async def set_cargo_orbit_config(config: EllipseConfig):
    """设置货船椭圆轨迹配置"""
    global _cargo_orbit_config
    _cargo_orbit_config = config
...
| 交付物 | 格式 | 负责人 |
|--------|------|--------|
| 修改后的货船运动控制代码 | JavaScript | 开发者 |
| 椭圆参数配置 API（可选） | Python | 后端开发者 |
| 前端配置面板（可选） | HTML/JS | 前端开发者 |
| 更新后的 Darwin Ratchet 记录 | JavaScript | 开发者 |
| 技术文档 | Markdown | 开发者 |
| 测试报告 | Markdown | QA |
...
1. **立即执行**：搜索定位货船运动控制代码（2.2.1）
2. **并行进行**：确定椭圆参数和运动逻辑（2.1.1, 2.1.2）
3. **准备环境**：确保 Three.js 开发环境可用，准备调试工具
