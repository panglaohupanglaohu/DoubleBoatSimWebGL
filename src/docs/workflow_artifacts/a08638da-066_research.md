...
**目标**: 将货船围绕双体船的绕圈轨迹从圆形改为椭圆形

**当前状态**: 根据 `darwin-ratchet.js` 中的 `cargo-orbit-catamaran-v1` 条目:
> "货船围绕双体船(原点)以半径80单位匀速圆周运动, 船头指向切线方向"

## 2. 需要修改的文件

### 2.1 核心3D场景文件

根据项目结构，货船运动逻辑最可能位于以下文件中:

| 文件路径 | 可能性 | 理由 |
|---------|--------|------|
| `src/frontend/digital-twin/main.js` | **高** | 主场景入口，包含3D对象管理 |
| `src/frontend/digital-twin/NavigationMonitor.js` | **中** | 导航监控，可能包含运动逻辑 |
| `src/frontend/digital-twin/PoseidonX.js` | **中** | 系统核心，可能管理3D对象 |
| `src/frontend/digital-twin/demo.js` | **低** | 演示文件，不包含核心逻辑 |

### 2.2 需要修改的达尔文棘轮记录

| 文件路径 | 修改内容 |
|---------|---------|
| `src/frontend/js/darwin-ratchet.js` | 更新 `cargo-orbit-catamaran-v1` 的描述 |

## 3. 技术方案分析

### 3.1 椭圆轨迹数学公式

将圆形轨迹 `(x, z) = (R*cos(θ), R*sin(θ))` 改为椭圆轨迹:
...
### 4.1 修改 `src/frontend/digital-twin/main.js` (假设文件)
...
cargoShip.rotation.y = -angle + Math.PI/2; // 指向切线方向
```

**修改后代码**:
```javascript
// 货船椭圆运动
const semiMajorAxis = 120;  // 半长轴
const semiMinorAxis = 60;   // 半短轴
cargoShip.position.x = semiMajorAxis * Math.cos(angle);
cargoShip.position.z = semiMinorAxis * Math.sin(angle);

// 椭圆切线方向
const dx = -semiMajorAxis * Math.sin(angle);
const dz = semiMinorAxis * Math.cos(angle);
cargoShip.rotation.y = Math.atan2(dx, dz); // 指向切线方向
```

### 4.2 修改 `src/frontend/js/darwin-ratchet.js`
...
| 评估项 | 状态 | 说明 |
|-------|------|------|
| 技术可行性 | ✅ 可行 | 仅需修改数学公式，无架构变更 |
| 影响范围 | ✅ 小 | 只影响货船运动逻辑 |
| 向后兼容 | ✅ 兼容 | 圆形是椭圆的特例(a=b=R) |
| 视觉效果 | ✅ 更丰富 | 椭圆轨迹更自然，模拟绕岛或绕航 |
...
const cargoOrbitConfig = {
    type: 'ellipse',  // 'circle' | 'ellipse'
    semiMajorAxis: 120,
    semiMinorAxis: 60,
    angularSpeed: 0.005  // 弧度/帧
};
```

### 6.2 验证方法

1. **视觉验证**: 在3D场景中观察货船轨迹是否为椭圆
2. **数学验证**: 打印位置坐标，验证满足椭圆方程 `(x/a)² + (z/b)² = 1`
3. **方向验证**: 验证船头始终指向椭圆切线方向

## 7. 总结

**需要修改的文件**:
1. `src/frontend/digital-twin/main.js` (或包含货船运动逻辑的实际文件)
2. `src/frontend/js/darwin-ratchet.js` (更新描述)

**不需要修改的文件**:
- 后端文件 (`src/backend/`) — 纯前端视觉修改
- 其他前端文件 — 不影响其他功能

**风险**: 低。仅涉及数学公式替换，不改变系统架构。
