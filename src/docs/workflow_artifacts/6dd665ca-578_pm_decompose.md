...
好的，作为 PoseidonX 系统的项目经理 (PM)，我已经分析了“复杂任务测试V4”的需求和项目上下文。以下是结构化的任务分解文档，旨在为后续的开发工作提供清晰的指导。
...
本任务旨在为数字孪生场景中的货船（`cargoShip`）添加两个核心功能：
1.  **前端 (Frontend):** 在 `src/frontend/digital-twin/main.js` 中，为货船的圆周运动创建一个 **wabi-sabi 风格的 HUD 覆盖层**，实时显示货船当前的角度和与双体船的距离。
2.  **后端 (Backend):** 新建 `src/backend/channels/cargo_orbit_telemetry.py` 文件，创建一个继承自 `MarineChannel` 的 `CargoOrbitTelemetryChannel` 类，通过 `process_event` 方法接收并上报货船的实时经纬度 (`lat/lon`) 数据。

### 2. 任务分解与子步骤

#### 阶段 1: 分���与设计 (预计耗时: 1小时)

**目标:** 明确技术方案，识别依赖和风险，为开发提供蓝图。

-   **1.1 前端 HUD 设计分析**
    -   **输入:** `src/frontend/digital-twin/main.js` 文件，特别是 `createCargoShip()` 和 `animate()` 函数。
    -   **任务:**
        -   分析 `createCargoShip()` 函数，确定 `cargoShip` 对象是如何被创建和存储的（例如，`state.cargoShip`）。
        -   分析 `animate()` 函数，找到货船位置更新的逻辑，确定获取货船当前角度和距离双体船距离的计算点。
        -   设计 HUD 的 HTML 结构和 CSS 样式，确保其符合“wabi-sabi”风格（例如，使用不完美的字体、自然的纹理、不对称的布局、柔和的色彩和留白）。
        -   确定 HUD 的定位方式（例如，固定在屏幕角落或跟随货船在3D空间中的位��）。
    -   **产出:** HUD 的 HTML/CSS 设计草图或伪代码。

-   **1.2 后端 Channel 设计分析**
    -   **输入:** `src/backend/channels/cargo_ship_orbit.py` 和 `src/backend/channels/marine_base.py` 文件。
    -   **任务:**
        -   分析 `CargoShipOrbitChannel` 的 `tick()` 方法，了解货船位置 (`x`, `z`) 和角度 (`angle_deg`) 是如何计算的。
        -   分析 `MarineChannel` 基类，明确 `process_event` 方法的签名和预期行为。
        -   设计 `CargoOrbitTelemetryChannel` 的 `process_event` 方法，定义其接收的事件格式（例如，`{"type": "cargo_orbit_telemetry", "x": ..., "z": ..., "angle_deg": ..., "distance": ...}`）。
        -   设计坐标转换逻辑：将场景坐标 `(x, z)` 转换为地理坐标 `(lat, lon)`。需要定义场景原点对应的地理坐标和转换比例。
    -   **产出:** `CargoOrbitTelemetryChannel` 类的接口定义和坐标转换算法。

-   **1.3 依赖与风险识别**
    -   **依赖:**
        -   前端 HUD 依赖于 `main.js` 中 `animate()` 循环的正确执行和货船位置数据的可用性。
        -   后端 `CargoOrbitTelemetryChannel` 依赖于 `CargoShipOrbitChannel` 提供货船的位置和角度数据。
        -   前后端数据交互依赖于现有的 WebSocket 或 API 通道。
    -   **风险:**
        -   **低风险:** HUD 样式不符合“wabi-sabi”风格。可通过代码审查和迭代解决。
        -   **中风险:** 前后端数据格式不匹配，导致后端无法正确解析前端发送的遥测数据。需要在开发前明确数据契约。
        -   **低风险:** 坐标转换参数（原点、比例）不准确，导致地理坐标偏差。可通过校准或配置文件解决。

#### 阶段 2: 后端开发 (预计耗时: 2小时)

**目标:** 创建并实现 `CargoOrbitTelemetryChannel`。

-   **2.1 创建文件**
    -   **操作:** 在 `src/backend/channels/` 目录下创建新文件 `cargo_orbit_telemetry.py`。

-   **2.2 实现 `CargoOrbitTelemetryChannel` 类**
    -   **操作:**
        -   导入必要的模块：`logging`, `math`, `datetime`, `typing` 以及 `channels.marine_base`。
        -   定义类 `CargoOrbitTelemetryChannel(MarineChannel)`。
        -   设置类属性：`name`, `description`, `version`, `priority`。
        -   实现 `__init__` 方法：初始化状态，设置坐标转换常量（原点经纬度、场景单位转度数的比例）。
        -   实现 `initialize()` 和 `shutdown()` 方法。
        -   实现 `process_event()` 方法：
            -   解析 `event` 字典，提取 `type` 字段。
            -   如果 `type` 是 `"cargo_orbit_telemetry"`，则提取 `x`, `z`, `angle_deg`, `distance` 等字段。
            -   调用坐标转换函数，将 `(x, z)` 转换为 `(lat, lon)`。
            -   记录或上报转换后的数据（例如，存储到内部状态或通过日志输出）。
            -   返回处理结果字典。
        -   实现一个辅助函数 `_scene_to_geo(x, z)` 用于坐标转换。
    -   **参考:** `src/backend/channels/cargo_orbit_telemetry.py` 文件（系统已提供完整实现，开发者需确保其与项目集成）。

-   **2.3 集成与注册**
    -   **操作:**
        -   在 `src/backend/register_channels.py` 或类似配置文件中，注册 `CargoOrbitTelemetryChannel`，使其在系统启动时被加载。
        -   确保 `CargoShipOrbitChannel` 在 `tick()` 方法中或通过其他机制，能够向 `CargoOrbitTelemetryChannel` 发送 `"cargo_orbit_telemetry"` 类型的事件。

#### 阶段 3: 前端开发 (预计耗时: 3小时)

**目标:** 在 `main.js` 中创建并集成 wabi-sabi 风格的 HUD。

-   **3.1 创建 HUD HTML 元素**
    -   **操作:** 在 `main.js` 的 `init()` 函数中，或创建一个新的函数（如 `createCargoOrbitHUD()`），动态创建一个 `div` 元素作为 HUD 容器。
    -   **样式设计 (wabi-sabi):**
        -   **字体:** 使用衬线字体或手写风格字体（如 `'Courier New', monospace` 或通过 Google Fonts 加载的 `'Kalam', cursive`）。
        -   **颜色:** 使用大地色系、墨色、宣纸白。例如，背景为半透明的米黄色 `rgba(245, 235, 220, 0.8)`，文字为深褐色 `#4a3b32`。
        -   **边框:** 使用不规则的、手绘风格的边框（可以通过 `border-image` 或 SVG 实现，或者简单使用 `box-shadow` 模拟）。
        -   **布局:** 不对称，留白充足。可以放在屏幕右下角或左下角。
        -   **内容:** 显示“角度”和“距离”两个数据项，并带有简单的标签，如 `方位: 45.2°` 和 `距离: 82.3m`。
    -   **参考:** 项目中的 `src/frontend/design-demo-wabisabi.html` 文件，可以借鉴其设计元素。

-   **3.2 集成数据更新逻辑**
    -   **操作:**
        -   在 `animate()` 函数中，找到更新货船位置的部分。
        -   计算货船相对于双体船（通常是原点 `(0, 0, 0)` 或 `state.boatMesh` 的位置）的角度和距离。
        -   获取 HUD 的 DOM 元素引用。
        -   在每一帧动画循环中，更新 HUD 元素内的文本内容，显示计算出的角度和距离。
        -   **示例代码片段 (在 `animate` 中):**
            ```javascript
            // 假设 cargoShip 位置已知
            ...
            const hudElement = document.getElementById('cargo-orbit-hud');
            if (hudElement) {
                hudElement.innerHTML = `
                    <div class="hud-row">方位: ${angleDeg.toFixed(1)}°</div>
                    <div class="hud-row">距离: ${distance.toFixed(1)}m</div>
                `;
            }
            ```
...
-   **3.3 发送遥测数据到后端**
    -   **操作:**
        -   在 `animate()` 函数中，当货船位置更新后，构造一个包含 `type`, `x`, `z`, `angle_deg`, `distance` 的数据对象。
        -   通过现有的 WebSocket 连接 (`state.ws`) 或 HTTP API 调用，将此数据对象发送到后端。
        -   **示例代码片段:**
            ```javascript
            if (state.ws && state.ws.readyState === WebSocket.OPEN) {
                state.ws.send(JSON.stringify({
                    type: 'cargo_orbit_telemetry',
                    x: cargoPos.x,
                    z: cargoPos.z,
                    angle_deg: angleDeg,
                    distance: distance
...
-   **4.1 单元测试**
    -   **后端:** 为 `CargoOrbitTelemetryChannel` 编写单元测试，验证 `process_event` 方法能正确解析事件并转换坐标。
    -   **前端:** 手动测试 HUD 是否在页面加载后出现，并且数据是否随货船运动而实时更新。
...
-   **4.2 集成测试**
    ...
    -   **验证:**
        -   货船是否按圆周运动。
        -   HUD 是否显示正确的角度和距离。
        -   后端日志是否打印出接收到的遥测数据和转换后的经纬度。
        -   检查 `CargoOrbitTelemetryChannel` 的状态 API 是否能返回最新的遥测数据。

-   **4.3 代码审查**
    -   **操作:** 提交 Pull Request，邀请团队成员审查代码，重点关注：
        -   HUD 的样式是否符合 wabi-sabi 风格。
        -   前后端数据契约是否一致。
        -   坐标转换逻辑是否正确。
        -   代码风格和项目规范。
...
| 风险描述 | 可能性 | 影响 | 缓解措施 |
| :--- | :--- | :--- | :--- |
| 前后端数据格式不匹配 | 中 | 高 | 在开发前（阶段1）明确定义数据契约（JSON Schema），并进行严格的单元测试。 |
| HUD 样式不符合 wabi-sabi 风格 | 低 | 中 | 提供明确的设计参考（如 `design-demo-wabisabi.html`），并在代码审查中重点关注。 |
| 坐标转换参数不准确 | 低 | 低 | 将原点经纬度和转换比例定义为可配置的常量，方便后期校准。 |
| 前端性能因 HUD 更新而下降 | 低 | 中 | 确保 HUD 更新逻辑高效，避免在每一帧进行复杂的 DOM 操作。可以使用 `requestAnimationFrame` 进行优化。 |

### 4. 交付物清单

1.  **后端文件:** `src/backend/channels/cargo_orbit_telemetry.py` (已实现)
2.  **前端文件:** `src/frontend/digital-twin/main.js` (修改版，包含 HUD 创建和数据发送逻辑)
3.  **配置文件:** `src/backend/register_channels.py` (修改版，注册新 Channel)
4.  **测试报告:** 单元测试和集成测试结果。

### 5. 后续建议

-   **数据持久化:** 考虑将遥测历史数据存储到数据库（如 InfluxDB 或 TimescaleDB），以便进行回放和分析。
-   **HUD 增强:** 为 HUD 添加更多信息，如货船航向、速度，或显示其运动轨迹。
-   **用户交互:** 允许用户通过点击 HUD 或场景中的货船来查看更多详细信息。
-   **动态配置:** 将坐标转换参数和 HUD 样式通过配置文件或 API 进行管理，提高系统的灵活性。
