...
好的，作为 PoseidonX 系统的架构师 (Architect)，我已经仔细审阅了任务描述、项目上下文、前序步骤的产出以及所有提供的源文件。

基于现有代码库的分析，我发现一个关键问题：后端 `CargoShipOrbitChannel` 已经实现了货船绕双体船做圆周运动的完整逻辑，但前端 3D 场景 (`digital-twin.html` 及其相关 JS 文件) 并未消费这个 Channel 的数据来驱动货船模型运动。

因此，当前任务的核心不是“从零开发”，而是“集成与适配”。我将基于此设计技术方案。
...
本方案旨在实现货船在 3D 数字孪生场景中，以双体船为圆心，船头指向运动切线方向，进行低速匀速圆周运动。

**核心策略：**
- **复用后端逻辑**：利用已存在的 `CargoShipOrbitChannel` 作为运动数据源。
- **前端集成**：修改前端 3D 场景的核心逻辑，定期从后端拉取货船位置和朝向数据，并应用到 Three.js 场景中的货船模型上。
- **速度控制**：通过调整后端的 `speed_deg_per_sec` 参数来控制视觉上的“快慢”，确保运动平滑且不突兀。

### 2. 需要修改的文件

| 文件路径 | 修改类型 | 修改内容摘要 |
| :--- | :--- | :--- |
| `src/backend/channels/cargo_ship_orbit.py` | **修改** | 调整默认 `radius` 和 `speed_deg_per_sec` 参数，使其更适合前端 3D 场景的视觉比例。 |
| `src/frontend/digital-twin/PoseidonX.js` | **修改** | 在核心循环中增加对 `CargoShipOrbitChannel` 数据的轮询逻辑，并更新货船模型的位置和旋转。 |
| `src/frontend/digital-twin/main.js` | **修改** | 在场景初始化时，获取货船模型的引用，并传递给 `PoseidonX` 核心逻辑。 |

### 3. 详细技术方案

#### 3.1 后端参数调整 (`cargo_ship_orbit.py`)

当前 `CargoShipOrbitChannel` 的默认参数是为真实世界米制单位设计的（半径500米，角速度0.5度/秒）。前端 3D 场景使用的是 Three.js 单位，通常 1 单位 ≈ 1 米，但场景中的模型比例可能不同。为了在视觉上获得更好的效果，需要调整参数。

**修改内容：**
- **减小轨道半径**：将 `radius` 从 `500.0` 调整为 `80.0`，使其与 `darwin-ratchet.js` 中描述的“半径80单位”一致。
- **降低角速度**���将 `speed_deg_per_sec` 从 `0.5` 调整为 `0.2`，使货船运动更慢、更平稳，符合“速度别太快”的要求。

```python
# 在 CargoShipOrbitChannel.__init__ 方法中
self.orbit_config = OrbitConfig(
    radius=orbit_cfg.get("radius", 80.0),          # 修改默认值
    speed_deg_per_sec=orbit_cfg.get("speed_deg_per_sec", 0.2), # 修改默认值
    initial_angle_deg=orbit_cfg.get("initial_angle_deg", 0.0),
    height_offset=orbit_cfg.get("height_offset", 0.0),
    enabled=orbit_cfg.get("enabled", True),
    auto_start=orbit_cfg.get("auto_start", True),
...
#### 3.2 前端核心逻辑修改 (`PoseidonX.js`)

`PoseidonX.js` 是数字孪生的核心控制器，负责管理所有 Channel 和 3D 场景的更新循环。我们需要在这里添加一个定时器，定期从后端 API 获取货船状态，并更新场景中的货船模型。

**修改内容：**

1.  **新增方法 `_fetchCargoShipState()`**：
    -   使用 `fetch` API 调用后端端点 `/api/v1/channels/cargo_ship_orbit/state`（需要后端新增此 API，见下文）。
    -   解析返回的 JSON 数据，提取 `cargo_position` 和 `cargo_heading`。

2.  **新增方法 `_updateCargoShip(position, heading)`**：
    -   获取场景中货船模型的引用（通过 `this.cargoShipMesh`）。
    -   使用 `position` 更新模型的位置 (`mesh.position.set(x, y, z)`)。
    -   使用 `heading` 更新模型的旋转。由于 Three.js 使用弧度，需要将角度转换为弧度，并应用到模型的 Y 轴旋转上 (`mesh.rotation.y = THREE.MathUtils.degToRad(heading)`)。

3.  **修改 `animate()` 或 `update()` 循环**：
    -   在循环中，每隔一定时间（例如 1 秒）调用 `_fetchCargoShipState()`。
    -   为了平滑运动，可以在两次拉取之间使用线性插值（Lerp），但这会增加复杂度。初期可以先实现“跳转”式更新，如果感觉卡顿再优化。

**代码示例 (PoseidonX.js 新增逻辑):**

```javascript
// 在 PoseidonX 类的 constructor 或 init 方法中
this.cargoShipMesh = null; // 用于存储货船模型的引用
this.cargoShipState = { position: {x:0, y:0, z:0}, heading: 0 };
this.lastCargoShipUpdate = 0;
this.cargoShipUpdateInterval = 1000; // 毫秒

// 新增方法：获取货船状态
async _fetchCargoShipState() {
    try {
        const response = await fetch('/api/v1/channels/cargo_ship_orbit/state');
        if (!response.ok) return;
        const data = await response.json();
        this.cargoShipState.position = data.cargo_position;
        this.cargoShipState.heading = data.cargo_heading;
    } catch (error) {
        console.warn('[PoseidonX] Failed to fetch cargo ship state:', error);
    }
}

// 新增方法：更新货船模型
_updateCargoShip() {
    if (!this.cargoShipMesh) return;
    const pos = this.cargoShipState.position;
    this.cargoShipMesh.position.set(pos.x, pos.y, pos.z);
    // 船头朝向切线方向，heading 是 Y 轴旋转角度
    this.cargoShipMesh.rotation.y = THREE.MathUtils.degToRad(this.cargoShipState.heading);
}

// 修改 animate 或 update 方法
update(time, delta) {
    // ... 其他更新逻辑 ...

    // 定时拉取货船状态
    if (time - this.lastCargoShipUpdate > this.cargoShipUpdateInterval) {
        this._fetchCargoShipState();
        this.lastCargoShipUpdate = time;
    }
    // 更新货船模型位置
    this._updateCargoShip();
...
`main.js` 负责初始化 3D 场景和加载模型。我们需要在这里找到货船模型，并将其引用传递给 `PoseidonX` 实例。

**修改内容：**

1.  **找到货船模型**：在模型加载完成的回调函数中，通过模型名称或遍历子节点找到货船模型。
2.  **传递引用**：将找到的模型赋值给 `poseidonX.cargoShipMesh`。

**代码示例 (main.js 新增逻辑):**

```javascript
// 假设模型加载完成的回调函数是 onModelLoaded
function onModelLoaded(gltf) {
    const scene = gltf.scene;
    // 遍历场景，找到名为 'cargo_ship' 或类似名称的模型
    scene.traverse((child) => {
        if (child.isMesh && child.name.includes('cargo')) {
            poseidonX.cargoShipMesh = child;
            console.log('[Main] Cargo ship mesh found:', child.name);
        }
    });
    // ... 其他初始化逻辑 ...
}
...
#### 3.4 新增后端 API (`agent_team_api.py` 或新建文件)

为了让前端获取货船状态，需要新增一个 API 端点。这个端点可以放在 `agent_team_api.py` 中，或者为了职责清晰，新建一个 `cargo_ship_api.py` 文件。

**方案：在 `agent_team_api.py` 中新增端点**

```python
# 在 agent_team_api.py 中新增

# 全局引用 CargoShipOrbitChannel
_cargo_ship_channel = None

def set_cargo_ship_channel(channel):
    global _cargo_ship_channel
    _cargo_ship_channel = channel

@router.get("/channels/cargo_ship_orbit/state")
async def get_cargo_ship_orbit_state():
    """获取货船轨道运动状态。"""
    if not _cargo_ship_channel:
        raise HTTPException(503, "Cargo ship orbit channel not initialized")
    return _cargo_ship_channel.get_orbit_state()
```
...
from channels.cargo_ship_orbit import CargoShipOrbitChannel
from agent_team_api import set_cargo_ship_channel

@app.on_event("startup")
async def startup_event():
    # ... 初始化其他组件 ...
    cargo_channel = CargoShipOrbitChannel()
    cargo_channel.initialize()
    set_cargo_ship_channel(cargo_channel)
    # ... 注册 channel 到 registry ...
```
...
-   **URL**: `GET /api/v1/channels/cargo_ship_orbit/state`
-   **描述**: 获取货船当前的轨道运动状态。
-   **响应格式 (JSON)**:
...
1.  **后端参数调整**：
    -   打开 `src/backend/channels/cargo_ship_orbit.py`。
    -   找到 `OrbitConfig` 的实例化代码，将 `radius` 默认值改为 `80.0`，`speed_deg_per_sec` 默认值改为 `0.2`。

2.  **后端 API 新增**：
    -   打开 `src/backend/agent_team_api.py`。
    -   在文件末尾或合适位置，添加 `set_cargo_ship_channel` 函数和 `get_cargo_ship_orbit_state` 路由。
    -   打开 `src/backend/main.py`。
    -   在 `startup` 事件中，实例化 `CargoShipOrbitChannel`，调用 `initialize()`，然后通过 `set_cargo_ship_channel` 注入。

3.  **前端模型引用**：
    -   打开 `src/frontend/digital-twin/main.js`。
    -   在模型加载完成的回调中，遍历场景找到货船模型，并将其赋值给 `poseidonX.cargoShipMesh`。

4.  **前端核心逻辑**：
    -   打开 `src/frontend/digital-twin/PoseidonX.js`。
    -   在 `constructor` 或 `init` 方法中，初始化 `cargoShipMesh` 和相关状态变量。
    -   实现 `_fetchCargoShipState` 和 `_updateCargoShip` 方法。
    -   在 `update` 或 `animate` 循环中，调用上述两个方法。

5.  **测试**：
    -   启动后端服务 (`uvicorn src.backend.main:app --reload`)。
    -   访问 `http://localhost:8000/api/v1/channels/cargo_ship_orbit/state`，确认能返回正确的 JSON 数据。
    -   打开前端页面 (`digital-twin.html`)，观察货船是否开始围绕双体船做圆周运动，且船头始终指向运动方向。
