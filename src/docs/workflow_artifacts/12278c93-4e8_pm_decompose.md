...
好的，项目经理。我已经分析了任务需求、项目上下文和相关文件。

这是一个非常明确的任务：**让货船以双体船为圆心动起来**。这意味着我们需要在3D场景中实现一个动画效果，其中一艘或多艘货船围绕一艘双体船（很可能是我们系统中的“穿浪双体船”）进行圆周运动或轨道运动。

以下是为 **build团队** 分配的任务分解和规划文档。

---

## 任务分解与规划: 货船绕双体船圆周运动

**任务ID:** TASK-BUILD-2024-001
**任务标题:** 实现货船以双体船为圆心的环绕动画
**分配给:** Build 团队 (前端/3D 开发)
**优先级:** 高
**状态:** 待开始

### 1. 任务概述

在 PoseidonX 系统的 3D 数字孪生场景中，实现一艘或多艘货船围绕穿浪双体船进行持续的圆周运动。此功能旨在模拟港口、锚地或编队航行场景，增强场景的动态真实感和视觉冲击力。

### 2. 需求分析

1.  **核心逻辑**: 货船的位置必须实时计算，使其始终围绕一个动态的圆心（双体船）旋转。
2.  **圆心对象**: 圆心是场景中的“穿浪双体船”。双体船本身可能静止或运动，货船的环绕运动必须以此为参考系。
3.  **运动对象**: 场景中已存在的货船模型（`GLB_20251223141542.glb` 或场景中其他货船）。
4.  **运动参数**:
    - **轨道半径**: 可配置，例如 200 米。
    - **旋转速度**: 可配置，例如 每 60 秒完成一圈。
    - **旋转方向**: 顺时针或逆时针，可配置。
    - **船首朝向**: 货船在运动时，其船首应始终指向其运动方向（切线方向），或者指向圆心（向心），或者保持与双体船平行。**建议默认指向运动切线方向**，模拟正常航行。
5.  **性能**: 动画应平滑运行，不造成明显的帧率下降。
6.  **集成**: 此功能应作为场景的一个可开关或可配置的模块，最好能集成到现有的控制面板或场景管理逻辑中。

### 3. 技术分析与依赖

- **核心依赖**:
    - **3D 引擎**: 项目使用 Three.js (从 `digital-twin` 目录下的 JS 文件推断)。
    - **场景管理**: `src/frontend/digital-twin/main.js` 或 `src/frontend/digital-twin/index.js` 很可能是场景的入口和主循环。
    - **对象引用**: 需要获取双体船和货船的 Three.js 对象引用。
- **技术风险**:
    - **对象查找**: 如何可靠地获取场景中的“双体船”和“货船”对象？可能需要通过名称、标签或特定属性来查找。
    - **坐标系统**: 货船的运动需要在世界坐标系下计算，确保与双体船的位置变化同步。
    - **现有动画冲突**: 需要确保新的环绕动画不与货船或双体船已有的其他动画（如航行、摇摆）冲突。

### 4. 任务分解 (子任务)

| 子任务 ID | 子任务名称 | 描述 | 负责人 | 预估工时 |
| :--- | :--- | :--- | :--- | :--- |
| **TASK-1** | **场景对象分析** | 分析 `main.js` 或 `index.js`，确定场景中双体船和货船的 Three.js 对象是如何创建和引用的。找到获取这些对象引用的最佳方式。 | 前端开发 | 2h |
| **TASK-2** | **核心动画逻辑开发** | 创建一个新的模块（例如 `src/frontend/digital-twin/orbital-animation.js`），实现货船绕双体船圆周运动的数学计算和更新逻辑。 | 前端开发 | 4h |
| **TASK-3** | **集成到主循环** | 将 `TASK-2` 开发的动画逻辑集成到 Three.js 的 `requestAnimationFrame` 主循环中，确保每帧更新货船位置。 | 前端开发 | 2h |
| **TASK-4** | **参数化与配置** | 将轨道半径、速度、方向等参数暴露为可配置项。可以创建一个简单的控制接口（如 HTML 滑块或 JS 对象），方便调试和后续集成。 | 前端开发 | 2h |
| **TASK-5** | **船首朝向修正** | 实现货船在圆周运动中的船首朝向逻辑。默认应指向运动方向的切线。 | 前端开发 | 2h |
| **TASK-6** | **测试与调试** | 在场景中测试动画效果，调整参数以达到最佳视觉效果。检查性能，确保无报错。 | 前端开发 | 2h |
| **TASK-7** | **代码审查与合并** | 提交代码，进行代码审查，并合并到主分支。 | 前端开发 | 1h |

### 5. 技术实现方案 (草案)

#### 5.1. 核心算法

在每一帧（`requestAnimationFrame`）中，执行以下步骤：

1.  **获取圆心位置**: 获取双体船对象 (`catamaran`) 的 `position` 属性。
2.  **计算新位置**:
    - 维护一个当前角度 `currentAngle`。
    - 每帧增加角度增量: `angleDelta = (2 * Math.PI / orbitPeriod) * deltaTime`。
    - 计算货船新位置:
        ```javascript
        const radius = 200; // 轨道半径
        const newX = catamaran.position.x + radius * Math.cos(currentAngle);
        const newZ = catamaran.position.z + radius * Math.sin(currentAngle);
        cargoShip.position.set(newX, catamaran.position.y, newZ);
        ```
3.  **更新船首朝向**:
    - 计算货船运动方向向量: `direction = new THREE.Vector3(-Math.sin(currentAngle), 0, Math.cos(currentAngle)).normalize();`
    - 使用 `cargoShip.lookAt()` 或 `quaternion` 让货船朝向该方向。
4.  **循环**: 当 `currentAngle` 超过 `2 * Math.PI` 时，重置为 0。

#### 5.2. 代码结构建议

创建一个新文件 `src/frontend/digital-twin/orbital-animation.js`。

```javascript
// src/frontend/digital-twin/orbital-animation.js

import * as THREE from 'three';

class OrbitalAnimation {
    constructor(scene, catamaran, cargoShip, config = {}) {
        this.scene = scene;
        this.catamaran = catamaran;
        this.cargoShip = cargoShip;
        this.radius = config.radius || 200;
        this.speed = config.speed || 0.1; // 弧度/秒
        this.direction = config.direction || 1; // 1: 逆时针, -1: 顺时针
        this.currentAngle = 0;
        this.enabled = true;
    }

    update(deltaTime) {
        if (!this.enabled || !this.catamaran || !this.cargoShip) return;

        // 1. 更新角度
        this.currentAngle += this.direction * this.speed * deltaTime;

        // 2. 计算新位置
        const center = this.catamaran.position;
        const newX = center.x + this.radius * Math.cos(this.currentAngle);
        const newZ = center.z + this.radius * Math.sin(this.currentAngle);
        this.cargoShip.position.set(newX, center.y, newZ);
        ...
        this.cargoShip.quaternion.setFromUnitVectors(
...
#### 5.3. 集成点 (`main.js` 或 `index.js`)

```javascript
// 在 main.js 或 index.js 中
import OrbitalAnimation from './orbital-animation.js';

// ... 场景初始化代码 ...

// 假设 catamaran 和 cargoShip 是已经加载好的 Three.js 对象
const orbitalAnim = new OrbitalAnimation(scene, catamaran, cargoShip, {
    radius: 250,
    speed: 0.05,
    direction: -1 // 顺时针
});
...
1.  货船能够平滑地围绕双体船进行圆周运动。
2.  当双体船移动时，货船能跟随其新位置继续环绕。
3.  货船的船首指向其运动方向。
4.  轨道半径、速度和方向可以通过配置文件调整。
5.  动画性能良好，无明显卡顿。
6.  代码已提交并通过审查。

### 7. 备注

- 如果场景中有多艘货船，可以创建多个 `OrbitalAnimation` 实例，并配置不同的半径和初始角度，形成更丰富的编队效果。
- 后续可以考虑将此功能集成到 `WeatherEffects.js` 或类似的场景特效管理模块中，通过 UI 开关控制。

---

**请 Build 团队负责人确认任务分解，并开始分配子任务。如有任何疑问，请随时与我沟通。**
