...
**现有资产分析：**
- 后端已有 `CargoShipOrbitChannel`（`src/backend/channels/cargo_ship_orbit.py`）实现了完整的轨道运动控制逻辑
- 前端 Darwin Ratchet 已记录该特性（`cargo-orbit-catamaran-v1`）
- 需要将后端计算的位置数据传递给前端 3D 场景进行渲染

## 2. 技术方案可行性评估

### 2.1 后端实现（已完成 ✅）

`CargoShipOrbitChannel` 已实现：
- 轨道运动计算（`tick()` 方法）
- 船头切线方向计算（`heading_deg = (current_angle + 90°) % 360°`）
- 速度控制（`speed_deg_per_sec` 参数）
- 半径配置（`radius` 参数）
- 双体船位置跟踪（`update_catamaran_position()`）
- REST API 接口（通过 `agent_team_api.py` 的 `/evolution/items` 端点）

### 2.2 前端实现（需要开发）

需要修改的 3D 场景文件来接收并渲染货船位置。

## 3. 需要修改的文件

### 3.1 前端 3D 场景文件

根据项目上下文，以下文件可能包含 3D 场景渲染逻辑：

| 文件路径 | 分析 |
|---------|------|
| `src/frontend/digital-twin/main.js` | 主场景初始化，可能包含 3D 对象管理 |
| `src/frontend/digital-twin/index.js` | 入口文件 |
| `src/frontend/digital-twin/demo.js` | 演示场景 |
| `src/frontend/digital-twin/PoseidonX.js` | PoseidonX 核心类 |
| `src/frontend/digital-twin/PoseidonXIntegration.js` | 集成逻辑 |
| `src/frontend/digital-twin/DataAggregator.js` | 数据聚合器 |

### 3.2 具体修改点

#### 文件 1: `src/frontend/digital-twin/main.js`

**修改内容：**
1. 在场景初始化时加载货船 3D 模型（`GLB_20251223141542.glb`）
2. 创建 `CargoShipOrbitController` 类管理货船运动
3. 在 `animate()` 循环中更新货船位置和朝向
...
loader.load('/GLB_20251223141542.glb', (gltf) => {
    cargoShip.add(gltf.scene);
    scene.add(cargoShip);
});
...
function animate() {
    ...
    // 更新货船位置
    const angle = (Date.now() * 0.0005) % (2 * Math.PI); // 0.5°/s ≈ 0.0087 rad/s
    const radius = 80; // 单位
...
#### 文件 2: `src/frontend/digital-twin/PoseidonX.js`

**修改内容：**
1. 添加 `cargoShipOrbit` 配置项
2. 在初始化时创建货船对象
3. 在更新循环中调用轨道计算

#### 文件 3: `src/frontend/digital-twin/DataAggregator.js`
...
class CargoShipOrbit {
    constructor(scene, radius = 80, speed = 0.5) {
        this.radius = radius;
        this.speed = speed; // 度/秒
        this.angle = 0;
    ...
    update(deltaTime) {
        ...
        this.group.position.x = this.radius * Math.cos(rad);
        this.group.position.z = this.radius * Math.sin(rad);
        
        // 朝向（切线方向）
        this.group.rotation.y = rad + Math.PI / 2;
    }
}
```

### 4.2 方案 B：后端驱动（与现有架构集成）

**优点：** 与后端 `CargoShipOrbitChannel` 集成，状态同步
**缺点：** 需要 API 调用，有网络延迟

```javascript
// 在 DataAggregator.js 中添加
async function fetchCargoShipPosition() {
    const response = await fetch('/api/v1/agent-teams/evolution/items/cargo-orbit-catamaran-v1');
    const data = await response.json();
    return data.cargo_position;
}
```
...
| 参数 | 建议值 | 说明 |
|------|--------|------|
| 半径 | 80 单位 | 与 Darwin 记录一致 |
| 角速度 | 0.5°/s | 约 0.7 单位/s 线速度 |
| 完成一圈 | 720 秒 (12 分钟) | 视觉上缓慢但可见运动 |
...
1. **立即实施方案 A**（纯前端）：
   ...
   - 添加货船轨道运动控制器
   - 使用 `GLB_20251223141542.glb` 作为货船模型

2. **后续集成方案 B**：
   - 通过 `DataAggregator.js` 连接后端 `CargoShipOrbitChannel`
   - 实现前后端状态同步

## 7. 风险与注意事项

1. **模型加载**：确保 `GLB_20251223141542.glb` 是货船模型
2. **坐标系**：Three.js 使用 Y-up，需要确认模型朝向
3. **性能**：轨道计算简单，不会影响帧率
4. **双体船位置**：如果双体船移动，需要动态更新圆心位置

## 8. 结论

**可行性：高** ✅

后端 `CargoShipOrbitChannel` 已提供完整的轨道运动控制逻辑，前端只需在 3D 场景中实现渲染即可。建议采用方案 A（纯前端实现）快速交付，后续再与后端集成。
