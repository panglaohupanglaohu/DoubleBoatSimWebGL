...
# 技术方案设计 — Cargo Orbit Telemetry & Wabi-Sabi HUD

## 1. 概述

本方案为货船圆周运动系统增加两个功能：
1. **前端**: 在 `main.js` 中添加 wabi-sabi 风格的 HUD，显示货船当前角度和距离双体船的距离
2. **后端**: 新建 `cargo_orbit_telemetry.py` Channel，通过 `process_event` 上报货船经纬度

## 2. 技术调研结论

### 2.1 现有系统分析

- **`CargoShipOrbitChannel`** (`cargo_ship_orbit.py`): 已实现完整的轨道运动控制，包含 `tick()` 方法计算货船位置 (x, y, z 坐标)，`get_cargo_position()` 返回当前位置
- **`MarineChannel`** 基类: 提供 `process_event()` 接口，支持事件驱动
- **`main.js`**: 前端主入口，包含 Three.js 3D 场景渲染逻辑，已有 `cargoShip` 对象管理货船运动
- **坐标系统**: 前端使用 Three.js 场景坐标 (x, z 平面)，后端需要转换为经纬度

### 2.2 关键技术决策

| 决策项 | 选择 | 理由 |
|--------|------|------|
| 经纬度转换 | 以双体船位置为参考点，使用等距圆柱投影近似 | 场景范围小（半径80m），误差可忽略 |
| HUD 实现 | CSS2DRenderer + 原生 DOM | 轻量、易实现 wabi-sabi 风格，无需额外库 |
| 通信方式 | WebSocket 事件 | 与现有 PoseidonX 架构一致 |
| 数据上报频率 | 每帧更新 | 轨道运动连续，需要实时跟踪 |

## 3. 详细设计

### 3.1 后端: `cargo_orbit_telemetry.py`

**文件路径**: `src/backend/channels/cargo_orbit_telemetry.py`

```python
# -*- coding: utf-8 -*-
"""
Cargo Orbit Telemetry Channel - 货船轨道遥测数据上报

监听 CargoShipOrbitChannel 的轨道状态，将货船位置转换为经纬度
并通过 process_event 上报。

坐标转换模型:
- 以双体船位置为原点 (lat0, lon0)
- 场景坐标 (x, z) → 经纬度偏移:
  - dlat = z / EARTH_RADIUS_M * (180/π)
  - dlon = x / (EARTH_RADIUS_M * cos(lat0_rad)) * (180/π)
"""
...
# 地球参数
EARTH_RADIUS_M = 6371000.0  # 地球平均半径 (米)
...
class CargoOrbitTelemetryChannel(MarineChannel):
    ...
    将 CargoShipOrbitChannel 计算的货船位置转换为经纬度，
    通过 process_event 上报给系统其他组件。
    """
    
    name = "cargo_orbit_telemetry"
    description = "货船轨道遥测数据上报 (经纬度)"
    version = "1.0.0"
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 双体船参考位置 (经纬度)
        self._catamaran_lat: float = self.config.get("catamaran_lat", 22.2969)   # 香港南丫岛附近
        self._catamaran_lon: float = self.config.get("catamaran_lon", 114.1719)
        ...
        logger.info("📡 CargoOrbitTelemetryChannel initialized at (%.4f, %.4f)",
                     self._catamaran_lat, self._catamaran_lon)
    
    def initialize(self) -> bool:
        """初始化遥测通道。"""
        self._initialized = True
        self._set_health(ChannelStatus.OK, "货船遥测通道就绪")
        return True
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "reference_position": {
                "lat": self._catamaran_lat,
                "lon": self._catamaran_lon,
            },
        }
    ...
    def process_event(self, event: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        支持的事件类型:
          - "cargo_orbit_position": 接收货船位置并上报经纬度
          - "set_reference": 设置双体船参考位置
          - "get_telemetry": 获取最新遥测数据
        ...
        if event_type == "cargo_orbit_position":
            return self._handle_position_report(event)
        elif event_type == "set_reference":
            return self._handle_set_reference(event)
        elif event_type == "get_telemetry":
            return {"status": "ok", "telemetry": self._last_telemetry}
        
        return {"status": "ignored", "reason": f"unknown event type: {event_type}"}
    
    def _handle_position_report(self, event: Dict[str, Any]) -> Dict[str, Any]:
        """
        处理货船位置上报事件。
        ...
        Args:
            event: 包含 cargo_position 字段的事件
            
        Returns:
            包含经纬度的遥测数据
        """
        cargo_pos = event.get("cargo_position", {})
        x = cargo_pos.get("x", 0.0)
        z = cargo_pos.get("z", 0.0)
        heading = event.get("cargo_heading", 0.0)
        angle_deg = event.get("angle_deg", 0.0)
        
        # 场景坐标 → 经纬度偏移
        # 使用等距圆柱投影近似 (适用于小范围)
        lat_rad = self._catamaran_lat * DEG_TO_RAD
        
        # 纬度偏移: 北/南方向 (z 轴)
        dlat = z / EARTH_RADIUS_M * RAD_TO_DEG
        
        # 经度偏移: 东/西方向 (x 轴)，需乘以 cos(lat) 修正
        dlon = x / (EARTH_RADIUS_M * math.cos(lat_rad)) * RAD_TO_DEG
        
        cargo_lat = self._catamaran_lat + dlat
        cargo_lon = self._catamaran_lon + dlon
        
        # 构建遥测数据
        self._last_telemetry = {
            "timestamp": datetime.now().isoformat(),
            "cargo_lat": round(cargo_lat, 6),
            "cargo_lon": round(cargo_lon, 6),
            "cargo_heading": round(heading, 1),
            "orbit_angle_deg": round(angle_deg, 1),
            "scene_position": {
                "x": round(x, 2),
                "z": round(z, 2),
            },
        ...
        logger.debug("📡 Cargo telemetry: (%.6f, %.6f) heading=%.1f°",
                     cargo_lat, cargo_lon, heading)
    ...
    def _handle_set_reference(self, event: Dict[str, Any]) -> Dict[str, Any]:
        """
        设置双体船参考位置。
        
        Args:
            event: 包含 lat, lon 字段的事件
        ...
        if lat is not None:
            self._catamaran_lat = float(lat)
        if lon is not None:
            self._catamaran_lon = float(lon)
        
        logger.info("📡 Reference position updated: (%.6f, %.6f)",
                     self._catamaran_lat, self._catamaran_lon)
...
__all__ = ["CargoOrbitTelemetryChannel"]
```

### 3.2 前端: `main.js` 修改

**文件路径**: `src/frontend/digital-twin/main.js`

#### 3.2.1 新增 Wabi-Sabi HUD 组件

在文件末尾添加以下代码：

```javascript
// ============================================================
// Wabi-Sabi HUD for Cargo Ship Orbit
// ============================================================

/**
 * 创建 wabi-sabi 风格的 HUD 显示货船轨道信息。
 * 
 * 设计理念:
 * - 手写感字体 (使用系统衬线字体模拟毛笔效果)
 * - 不完美的边框 (使用 SVG 滤镜模拟墨迹边缘)
 * - 留白与不对称布局
 * - 自然材质配色 (墨色、宣纸色、朱砂红点缀)
 */
function createWabiSabiHUD() {
    // 创建容器
    ...
    container.style.cssText = `
        ...
        /* 模拟宣纸纹理 */
        background-image: 
            radial-gradient(circle at 20% 30%, rgba(200, 180, 150, 0.15) 0%, transparent 50%),
            radial-gradient(circle at 80% 70%, rgba(180, 160, 130, 0.1) 0%, transparent 40%);
        /* 墨迹边缘效果 */
    ...
    // 角度显示
    const angleRow = createDataRow('方位角', '0.0°', 'angle-value');
    container.appendChild(angleRow);

    // 距离显示
    const distRow = createDataRow('距離', '0.0 m', 'distance-value');
    container.appendChild(distRow);

    // 轨道圈数 (额外信息)
    const orbitRow = createDataRow('軌道圈數', '0', 'orbit-count');
    ...
    statusText.id = 'orbit-status-text';
 ...
 * @param {string} valueId - 值元素的 ID
 * @returns {HTMLElement} 行容器
 */
function createDataRow(label, value, valueId) {
 ...
 * 更新 HUD 显示数据。
 * 
 * @param {Object} data - 轨道数据
 * @param {number} data.angle - 当前角度 (度)
 * @param {number} data.distance - 距离双体船的距离 (场景单位)
 * @param {number} data.totalOrbits - 已完成轨道圈数
 * @param {boolean} data.isRunning - 是否运行中
 */
function updateWabiSabiHUD(data) {
    ...
    const statusDot = document.getElementById('orbit-status-dot');
    const statusText = document.getElementById('orbit-status-text');

    if (angleEl) {
        angleEl.textContent = `${data.angle.toFixed(1)}°`;
    }
    if (distEl) {
        distEl.textContent = `${data.distance.toFixed(1)} m`;
    }
    if (orbitEl) {
        orbitEl.textContent = data.totalOrbits.toFixed(1);
    }
    if (statusDot && statusText) {
        if (data.isRunning) {
            statusDot.style.background = '#C04040';
            statusText.textContent = '運行中';
        } else {
            statusDot.style.background = '#8B7355';
...
#### 3.2.2 在动画循环中集成 HUD 更新

找到 `main.js` 中的动画循环（通常是 `animate()` 或 `render()` 函数），在货船位置更新后添加：

```javascript
// 在动画循环中，货船位置更新后添加：
// 假设 cargoShip 对象包含轨道状态信息

// 初始化 HUD (仅在首次调用时创建)
if (!window._cargoOrbitHUD) {
    window._cargoOrbitHUD = createWabiSabiHUD();
}

// 计算货船到双体船的距离
// 假设 catamaran 是双体船对象，cargoShip 是货船对象
const dx = cargoShip.position.x - catamaran.position.x;
const dz = cargoShip.position.z - catamaran.position.z;
const distance = Math.sqrt(dx * dx + dz * dz);

// 获取当前角度 (从 cargoShip 的轨道状态中获取)
const angle = cargoShip.orbitAngle || 0; // 假设 orbitAngle 属性存在

// 更新 HUD
updateWabiSabiHUD({
    angle: angle,
    distance: distance,
    totalOrbits: cargoShip.totalOrbits || 0,
    isRunning: cargoShip.orbitRunning !== false,
});

// 可选: 通过 WebSocket 向后端发送遥测数据
if (window.poseidonSocket && window.poseidonSocket.readyState === WebSocket.OPEN) {
    window.poseidonSocket.send(JSON.stringify({
        type: 'cargo_orbit_position',
        cargo_position: {
            x: cargoShip.position.x,
            z: cargoShip.position.z,
        },
        cargo_heading: cargoShip.rotation.y || 0,
        angle_deg: angle,
    }));
}
```
...
**文件路径**: `src/backend/register_channels.py`

在 `register_channels.py` 中添加新 Channel 的注册：

```python
# 在 register_channels.py 的 channel 注册部分添加
from channels.cargo_orbit_telemetry import CargoOrbitTelemetryChannel

# 注册遥测通道
registry.register(
    CargoOrbitTelemetryChannel,
    config={
        "catamaran_lat": 22.2969,
        "catamaran_lon": 114.1719,
    }
)
```
...
| 事件类型 | 方向 | 描述 | 请求格式 | 响应格式 |
|---------|------|------|---------|---------|
| `cargo_orbit_position` | 前端→后端 | 上报货船位置 | `{type, cargo_position: {x,z}, cargo_heading, angle_deg}` | `{status, telemetry: {cargo_lat, cargo_lon, ...}}` |
| `set_reference` | 前端→后端 | 设置参考位置 | `{type, lat, lon}` | `{status, reference_position}` |
| `get_telemetry` | 前端→后端 | 获取最新遥测 | `{type}` | `{status, telemetry}` |
...
{
    angle: number,        // 当前角度 (度)
    distance: number,     // 距离双体船 (场景单位)
    totalOrbits: number,  // 已完成轨道圈数
...
### 步骤 1: 创建后端 Channel
- 在 `src/backend/channels/` 目录下创建 `cargo_orbit_telemetry.py`
- 实现 `CargoOrbitTelemetryChannel` 类
- 实现坐标转换逻辑

### 步骤 2: 注册新 Channel
- 在 `register_channels.py` 中添加注册代码

### 步骤 3: 修改前端 main.js
- 添加 `createWabiSabiHUD()` 函数
- 添加 `updateWabiSabiHUD()` 函数
- 在动画循环中集成 HUD 更新

### 步骤 4: 测试验证
- 启动后端服务，确认 Channel 注册成功
- 启动前端，确认 HUD 显示正确
- 验证经纬度转换准确性

## 6. 注意事项

1. **坐标转换精度**: 由于场景范围小（半径80m），等距圆柱投影的误差可忽略不计
2. **性能**: HUD 使用原生 DOM，性能开销极小
3. **兼容性**: Wabi-sabi 风格使用标准 CSS，兼容所有现代浏览器
4. **扩展性**: 如需更精确的经纬度转换，可替换为 UTM 投影或直接使用 GIS 库
