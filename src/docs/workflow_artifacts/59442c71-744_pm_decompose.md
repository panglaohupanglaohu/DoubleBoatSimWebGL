...
好的，项目经理。我已经分析了任务需求和项目上下文，以下是针对“让货船以双体船为圆心，船头以圆切线，动起来，速度别太快”这一任务的详细分解和规划文档。

---

## 任务分解与规划文档

**项目名称:** PoseidonX - 货船轨道运动功能
**任务发起人:** 系统指令 / 用户
**项目经理:** PoseidonX PM
**目标团队:** Build Team (构建团队)
**日期:** 2024-05-24

### 1. 任务概述

**核心目标:** 在 PoseidonX 数字孪生系统的 3D 场景中，实现一艘货船围绕双体船（位于场景原点）做匀速圆周运动。货船的船头必须始终指向其运动方向的切线，并且运动速度需要控制在“不太快”的视觉舒适范围内。

**成功标准:**
1.  货船在 3D 场景中可见，并围绕原点（双体船）做圆周运动。
2.  货船的船头始终指向其运��轨迹的切线方向。
3.  运动速度平缓、视觉上舒适。
4.  功能可通过系统现有的“达尔文棘轮”机制进行记录和追踪。

### 2. 技术分析与依赖识别

根据项目上下文，我们已有部分基础工作，但核心的前端3D实现逻辑尚未完成。

- **现有资产:**
    - **后端逻辑 (`src/backend/channels/cargo_ship_orbit.py`):** 已存在一个完整的 `CargoShipOrbitChannel`，它包含了轨道运动的所有数学计算（位置、朝向、速度控制）。这是一个重要的后端资产，但**它并未与前端3D场景连接**。
    - **达尔文棘轮记录 (`src/frontend/js/darwin-ratchet.js`):** 系统中已有一条名为 `cargo-orbit-catamaran-v1` 的记录，表明该功能已被“规划”或“记录”，但尚未实现。
    - **3D 场景:** 项目包含多个 3D 场景文件（如 `GLB_20251223141542.glb`）和相关的 JavaScript 模块（如 `src/frontend/digital-twin/` 下的文件），表明前端具备 3D 渲染能力。

- **核心依赖与风险:**
    1.  **前端 3D 引擎:** 需要确定当前 3D 场景使用的是哪个库（很可能是 Three.js）。这是实现货船运动和朝向控制的基础。
    2.  **数据流:** 后端 `CargoShipOrbitChannel` 计算出的位置数据需要实时推送到前端。需要确认现有的 WebSocket 或轮询机制是否支持，或者需要新建一个。
    3.  **货船模型:** 需要确认场景中是否已加载了货船的 3D 模型（`.glb` 文件）。如果没有，需要加载一个。
    4.  **双体船模型:** 需要确认双体船模型位于场景原点 `(0, 0, 0)`，或者获取其实际位置作为轨道圆心。

### 3. 任务分解 (WBS)

我将此任务分解为以下可执行的子任务，并分配给 Build Team 的相应成员。

---

#### 任务 1: [前端开发] 实现货船轨道运动核心逻辑

- **负责人:** Build Team - 前端开发工程师
- **描述:** 在前端 3D 场景中，编写核心逻辑，使货船模型围绕原点做圆周运动，并实时更新其朝向。
- **详细步骤:**
    1.  **定位 3D 场景入口:** 找到负责渲染主 3D 场景的 JavaScript 文件（例如 `src/frontend/digital-twin/main.js` 或 `index.js`）。
    2.  **获取或加载货船模型:**
        - 检查场景中是否已存在名为 `cargo_ship` 或类似名称的 3D 对象。
        - 如果没有，从 `src/frontend/GLB_20251223141542.glb` 或其他资源中加载货船模型，并添加到场景中。
    3.  **实现圆周运动逻辑:**
        - 在场景的动画循环（`requestAnimationFrame`）中，编写代码。
        - 定义轨道参数：半径 `R` (例如 80 单位)，角速度 `ω` (例如 0.01 弧度/帧，或基于时间增量计算)。
        - 每帧更新货船位置：
          - `x = R * cos(θ)`
          - `z = R * sin(θ)`
          - `θ += ω * deltaTime`
        - **速度控制:** 调整 `ω` 的值，使其视觉上“不太快”。建议初始值设为 `0.005` 弧度/帧，并根据视觉效果微调。
    4.  **实现船头朝向切线:**
        - 计算货船运动方向向量：`direction = (-sin(θ), 0, cos(θ))`。
        - 使用 Three.js 的 `Object3D.lookAt()` 方法或四元数，让货船模型朝向 `position + direction` 点。
        - **关键:** 确保货船模型的“前方”是其自身的 +Z 或 -Z 轴，必要时进行旋转调整。
    5.  **代码集成与测试:**
        - 将代码集成到主场景循环中。
        - 在浏览器中打开场景，观察货船是否按预期运动。

- **交付物:** 修改后的前端 JavaScript 文件，包含货船轨道运动的完整逻辑。

---

#### 任务 2: [前端/后端集成] 连接后端数据通道 (可选，但推荐)

- **负责人:** Build Team - 全栈/集成工程师
- **描述:** 将后端的 `CargoShipOrbitChannel` 计算出的数据推送到前端，使运动逻辑可由后端控制。
- **详细步骤:**
    1.  **分析后端 Channel:** 确认 `CargoShipOrbitChannel` 的 `tick()` 方法输出的数据格式（`cargo_position`, `cargo_heading`）。
    2.  **建立数据通道:**
        - 检查项目是否已有 WebSocket 或 SSE 端点用于实时数据推送。
        - 如果没有，在 `src/backend/main.py` 或 `api_extensions.py` 中创建一个新的 WebSocket 端点，用于广播 `CargoShipOrbitChannel` 的状态。
    3.  **前端订阅数据:**
        - 在前端主场景文件中，编写代码连接到后端的 WebSocket。
        - 接收 `cargo_position` 和 `cargo_heading` 数据。
    4.  **更新前端逻辑:**
        - 修改任务 1 中的代码，使其不再自行计算位置，而是直接使用从后端接收到的 `cargo_position` 和 `cargo_heading` 数据来更新货船模型。
    5.  **端到端测试:** 启动后端服务，确认前端能正确接收数据并驱动货船运动。

- **交付物:** 后端 WebSocket 端点代码 + 修改后的前端数据订阅和模型更新代码。

---

#### 任务 3: [QA/测试] 功能验证与调优

- **负责人:** Build Team - 测试工程师
- **描述:** 验证货船轨道运动功能是否符合所有成功标准，并对速度等参数进行调优。
- **详细步骤:**
    1.  **功能验证:**
        - 货船是否围绕原点（双体船）做圆周运动？
        - 船头是否始终指向切线方向？
        - 运动是否平滑、无抖动？
    2.  **速度调优:**
        - 与项目经理和设计师确认“速度别太快”的具体标准。
        - 调整角速度 `ω` 参数，直到视觉上令人满意。
    3.  **边界情况测试:**
        - 测试当浏览器窗口大小变化时，运动是否正常。
        - 测试在低帧率下，运动是否依然平滑（使用 `deltaTime` 修正）。
    4.  **记录测试结果:** 将测试结果和最终参数记录在案。

- **交付物:** 测试报告，包含验证结果和最终参数配置。
...
- **负责人:** Build Team - PM (我)
- **描述:** 在功能开发完成后，更新系统中的“达尔文棘轮”记录，标记该功能为已实现。
- **详细步骤:**
    1.  在浏览器控制台中，调用 `Darwin.record()` 方法。
    2.  更新 `cargo-orbit-catamaran-v1` 记录的 `fitness` 状态为 `pass`。
    3.  或者，如果该记录不存在，创建一个新的记录，标题为“货船轨道运动功能实现”。
- **交付物:** 系统内一条状态为 `pass` 的达尔文棘轮记录。

### 4. 任务分配与优先级

| 任务 ID | 任务名称 | 负责人 | 优先级 | 预估工时 | 依赖 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| T1 | 实现货船轨道运动核心逻辑 | 前端开发工程师 | **P0** | 4小时 | 无 |
| T2 | 连接后端数据通道 | 全栈工程师 | P1 | 3小时 | T1 (可选) |
| T3 | 功能验证与调优 | 测试工程师 | **P0** | 2小时 | T1 |
| T4 | 更新达尔文棘轮记录 | PM | P2 | 0.5小时 | T3 |

**建议执行顺序:** T1 → T3 → T4。T2 作为增强选项，可在核心功能稳定后进行。

### 5. 风险与缓解措施

| 风险 | 可能性 | 影响 | 缓解措施 |
| :--- | :--- | :--- | :--- |
| 3D 场景使用的库不是 Three.js | 低 | 高 | 任务开始前，PM 需与开发确认当前使用的 3D 库。如果是其他库，调整 T1 中的 API 调用。 |
| 货船模型加载失败或不存在 | 中 | 高 | 开发需在项目资源中查找 `.glb` 文件。如果找不到，使用一个简单的立方体或球体作为临时替代，并立即向 PM 报告。 |
| 后端 WebSocket 实现复杂度过高 | 中 | 中 | 如果 T2 耗时过长，可以暂时放弃后端集成，让前端独立完成所有逻辑，��保核心功能按时交付。 |
| 速度参数难以达成共识 | 低 | 低 | PM 需提前与相关方（如设计师）沟通，设定一个可接受的速度范围，让开发在此范围内调整。 |

---

**项目经理备注:**

请 Build Team 的各位成员确认以上任务分解。**前端开发工程师**请立即开始 T1 任务，首先确认 3D 场景的技术栈。**测试工程师**请在 T1 完成后立即介入 T3。我将负责 T4 的收尾工作。
