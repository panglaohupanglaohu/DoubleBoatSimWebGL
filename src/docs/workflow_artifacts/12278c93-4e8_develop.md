...
## 任务理解
**让货船以双体船为圆心动起来** — 在3D场景中，货船应该围绕双体船（穿浪双体船）做圆周运动。

## 技术调研要点
1. 项目使用Three.js进行3D渲染
2. 双体船是场景中的主船（穿浪双体船）
3. 货船是场景中的另一艘船，需要围绕双体船做圆周运动
4. 需要修改前端3D场景的动画逻辑

## 架构设计方案

### 修改文件
1. **`src/frontend/digital-twin/main.js`** — 主场景初始化，包含3D对象管理
2. **`src/frontend/digital-twin/PoseidonX.js`** — PoseidonX核心类，管理场景对象

### 实现方案
在场景中找到一个代表双体船的对象（通常是主船），然后让货船对象围绕它做圆周运动。需要：
1. 识别双体船和货船的3D对象
2. 在动画循环中添加货船绕双体船旋转的逻辑
3. 设置合适的旋转半径和速度
...
根据架构设计，我需要查看具体的3D场景文件来确定如何实现。让我先查看关键文件。

```python // src/backend/channels/cargo_ship_orbit.py
# -*- coding: utf-8 -*-
"""
...
现在我需要修改前端的主场景文件，让货船在3D场景中实际运动起来。让我查看相关文件。

```javascript // src/frontend/digital-twin/main.js
/**
 * PoseidonX Digital Twin — Main Entry
 * 
 * 主场景初始化与动画循环。
 * 管理3D场景中的船舶对象，包括双体船和货船。
 */
...
const STATE = {
    ...
    // 船舶对象
    catamaran: null,       // 双体船 (主船)
    cargoShip: null,       // 货船
    
    // 货船轨道运动
    cargoOrbit: {
        enabled: true,
        radius: 500,           // 轨道半径 (场景单位)
        speed: 0.5,            // 角速度 (度/秒)
        angle: 0,              // 当前角度 (度)
        heightOffset: 0,       // 高度偏移
        catamaranPosition: { x: 0, y: 0, z: 0 },
    },
...
export function initScene(container) {
    ...
    STATE.scene.background = new THREE.Color(0x87CEEB); // 天空蓝
    ...
    STATE.camera.position.set(200, 150, 300);
    ...
    STATE.controls.maxPolarAngle = Math.PI / 2.2;
...
function setupLights() {
    ...
    // 半球光
    const hemiLight = new THREE.HemisphereLight(0x87CEEB, 0x3a6ea5, 0.8);
...
function setupOcean() {
    ...
    STATE.objects.push(ocean);
}

// ── 加载船舶模型 ──────────────────────────────────────────

export function loadCatamaran(gltfPath) {
    ...
    return new Promise((resolve, reject) => {
        loader.load(
            ...
            (gltf) => {
                ...
                STATE.catamaran = model;
                STATE.objects.push(model);
                
                console.log('[Main] Catamaran loaded');
                resolve(model);
            },
            (progress) => {
                // 加载进度
            ...
            (error) => {
                console.error('[Main] Failed to load catamaran:', error);
                reject(error);
            }
        );
    });
...
export function loadCargoShip(gltfPath) {
    const loader = new GLTFLoader();
    const dracoLoader = new DRACOLoader();
    dracoLoader.setDecoderPath('https://www.gstatic.com/draco/versioned/decoders/1.5.6/');
    loader.setDRACOLoader(dracoLoader);
    ...
    return new Promise((resolve, reject) => {
        loader.load(
            ...
            (gltf) => {
                ...
                // 初始位置: 在双体船右侧轨道上
                const initialAngle = STATE.cargoOrbit.angle;
                const radius = STATE.cargoOrbit.radius;
                model.position.set(
                ...
                STATE.scene.add(model);
                STATE.cargoShip = model;
                STATE.objects.push(model);
                
                console.log('[Main] Cargo ship loaded at initial position');
                resolve(model);
            },
            (progress) => {
                // 加载进度
            ...
            (error) => {
                console.error('[Main] Failed to load cargo ship:', error);
                reject(error);
            }
        );
    });
...
export function createSimpleCargoShip() {
    const group = new THREE.Group();
    
    // 船体
    const hullGeo = new THREE.BoxGeometry(40, 10, 15);
    ...
    // 上层建筑
    const cabinGeo = new THREE.BoxGeometry(10, 6, 8);
    const cabinMat = new THREE.MeshStandardMaterial({
        color: 0x8B4513,
        roughness: 0.8,
    });
    const cabin = new THREE.Mesh(cabinGeo, cabinMat);
    cabin.position.set(-8, 8, 0);
    cabin.castShadow = true;
    cabin.receiveShadow = true;
    group.add(cabin);
    
    // 烟囱
    const chimneyGeo = new THREE.CylinderGeometry(1.5, 2, 5);
    const chimneyMat = new THREE.MeshStandardMaterial({
        color: 0x333333,
        roughness: 0.9,
    });
    const chimney = new THREE.Mesh(chimneyGeo, chimneyMat);
    chimney.position.set(-8, 12, 0);
    chimney.castShadow = true;
    chimney.receiveShadow = true;
    group.add(chimney);
    
    // 集装箱 (甲板上)
    const containerColors = [0xcc3333, 0x3366cc, 0x33cc33, 0xffcc00];
    for (let i = 0; i < 6; i++) {
        const containerGeo = new THREE.BoxGeometry(6, 3, 4);
    ...
    STATE.cargoShip = group;
    STATE.objects.push(group);
    
    console.log('[Main] Simple cargo ship created');
    return group;
}

// ── 创建简易双体船 (当GLB不可用时) ────────────────────────

export function createSimpleCatamaran() {
    const group = new THREE.Group();
    
    // 主甲板
    const deckGeo = new THREE.BoxGeometry(60, 3, 25);
    ...
    // 左船体
    const leftHullGeo = new THREE.BoxGeometry(55, 5, 6);
    const hullMat = new THREE.MeshStandardMaterial({
        color: 0x34495e,
        roughness: 0.5,
        metalness: 0.4,
    });
    const leftHull = new THREE.Mesh(leftHullGeo, hullMat);
    leftHull.position.set(0, 0, -12);
    leftHull.castShadow = true;
    leftHull.receiveShadow = true;
    group.add(leftHull);
    
    // 右船体
    const rightHull = new THREE.Mesh(leftHullGeo.clone(), hullMat);
    rightHull.position.set(0, 0, 12);
    rightHull.castShadow = true;
    rightHull.receiveShadow = true;
    group.add(rightHull);
    
    // 驾驶台
    const bridgeGeo = new THREE.BoxGeometry(12, 8, 10);
    const bridgeMat = new THREE.MeshStandardMaterial({
        color: 0xecf0f1,
        roughness: 0.3,
    });
    const bridge = new THREE.Mesh(bridgeGeo, bridgeMat);
    bridge.position.set(-10, 8, 0);
    bridge.castShadow = true;
    bridge.receiveShadow = true;
    group.add(bridge);
    
    // 天线
    const antennaGeo = new THREE.CylinderGeometry(0.3, 0.3, 5);
    ...
    STATE.scene.add(group);
    STATE.catamaran = group;
    STATE.objects.push(group);
    
    console.log('[Main] Simple catamaran created');
    return group;
}

// ── 轨道控制 API ──────────────────────────────────────────

export function setCargoOrbitEnabled(enabled) {
    STATE.cargoOrbit.enabled = enabled;
    console.log('[Main] Cargo orbit', enabled ? 'enabled' : 'disabled');
}

export function setCargoOrbitRadius(radius) {
    if (radius <= 0) return;
    STATE.cargoOrbit.radius = radius;
    console.log('[Main] Cargo orbit radius set to', radius);
}

export function setCargoOrbitSpeed(speed) {
    if (speed <= 0) return;
    STATE.cargoOrbit.speed = speed;
    console.log('[Main] Cargo orbit speed set to', speed, '°/s');
}

export function setCargoOrbitAngle(angleDeg) {
    STATE.cargoOrbit.angle = angleDeg % 360;
}

export function getCargoOrbitState() {
    return { ...STATE.cargoOrbit };
}
...
function animate() {
    ...
    // 更新货船轨道运动
    updateCargoShipOrbit(delta, elapsed);
...
function updateCargoShipOrbit(delta, elapsed) {
    if (!STATE.cargoShip || !STATE.cargoOrbit.enabled) return;
    
    // 更新轨道角度
    STATE.cargoOrbit.angle = (STATE.cargoOrbit.angle + STATE.cargoOrbit.speed * delta) % 360;
    
    // 计算货船位置 (绕双体船旋转)
    const angleRad = STATE.cargoOrbit.angle * Math.PI / 180;
    const radius = STATE.cargoOrbit.radius;
    
    // 双体船位置 (如果双体船存在，使用其位置作为圆心)
    let centerX = 0;
    let centerZ = 0;
    let centerY = 0;
    
    if (STATE.catamaran) {
        centerX = STATE.catamaran.position.x;
        centerZ = STATE.catamaran.position.z;
        centerY = STATE.catamaran.position.y;
    }
    
    // 更新货船位置
    STATE.cargoShip.position.x = centerX + radius * Math.cos(angleRad);
    STATE.cargoShip.position.z = centerZ + radius * Math.sin(angleRad);
    STATE.cargoShip.position.y = centerY + STATE.cargoOrbit.heightOffset;
    
    // 货船朝向 (沿运动切线方向)
    // 切线方向 = 当前角度 + 90°
    const headingDeg = (STATE.cargoOrbit.angle + 90) % 360;
    STATE.cargoShip.rotation.y = headingDeg * Math.PI / 180;
    
    // 添加轻微的上下浮动 (模拟海浪)
    if (STATE.cargoShip.position.y === 0) {
        STATE.cargoShip.position.y = Math.sin(elapsed * 0.5) * 0.5;
    }
}
...
export function dispose() {
    ...
    console.log('[Main] Scene disposed');
}
...
export default {
    ...
    loadCatamaran,
    loadCargoShip,
    createSimpleCatamaran,
    createSimpleCargoShip,
    setCargoOrbitEnabled,
    setCargoOrbitRadius,
    setCargoOrbitSpeed,
    setCargoOrbitAngle,
    getCargoOrbitState,
...
现在我需要更新 `PoseidonX.js` 文件，集成货船轨道运动逻辑：

```javascript // src/frontend/digital-twin/PoseidonX.js
/**
 * PoseidonX — 核心系统类
 * 
 * 管理数字孪生场景的完整生命周期，包括船舶加载、轨道运动控制等。
 */
...
class PoseidonX {
    constructor(config = {}) {
        this.config = {
            catamaranModelPath: '/GLB_20251223141542.glb',
            cargoShipModelPath: null,  // 可选货船模型
            useSimpleModels: true,     // 默认使用简易模型
            ...config,
        };
