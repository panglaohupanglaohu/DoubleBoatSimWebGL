...
**任务名称**: 复杂任务测试V4 - 货船轨道遥测HUD与后端Channel实现

**目标**: 
1. 在 `src/frontend/digital-twin/main.js` 中为货船圆周运动添加 wabi-sabi 风格的 HUD overlay
2. 新建 `src/backend/channels/cargo_orbit_telemetry.py`，继承 MarineChannel，实现 process_event 上报 cargo 当前 lat/lon
...
- **文件**: `src/frontend/digital-twin/main.js`
- **关键点**:
  - `createCargoShip()` 函数创建货船
  - `animate()` 函数驱动圆周运动
  - 已有 `createCargoOrbitHUD()` 函数（需确认是否已实现）
  - 货船位置通过 `state.cargoShip` 对象访问

#### 1.1.2 实现 wabi-sabi HUD Overlay
- **设计原则**:
  - 使用 HTML/CSS 创建 overlay，而非 Canvas
  - wabi-sabi 风格：不完美、自然、质朴的美学
  - 显示内容：当前角度 (deg) + 距双体船距离 (m)
  - 半透明背景，柔和边缘，自然纹理

- **实现步骤**:
  1. 创建 HUD 容器元素 (div)
  2. 设计 wabi-sabi 风格 CSS（手写字体、自然色系、轻微不规则边框）
...
- **数据计算**:
  ...
  // 双体船位置 (原点)
  const catPos = { x: 0, z: 0 };
  // 距离
  const distance = Math.sqrt(
    (cargoPos.x - catPos.x) ** 2 + 
    (cargoPos.z - catPos.z) ** 2
  );
  // 角度 (度)
  const angle = Math.atan2(cargoPos.z - catPos.z, cargoPos.x - catPos.x) * 180 / Math.PI;
  ```

#### 1.1.3 集成到现有系统
- 确保 HUD 不遮挡其他 UI 元素
- 支持拖拽/缩放（参考 `makeDraggable` 函数）
- 响应式布局
...
- **基类**: `MarineChannel` (位于 `src/backend/channels/marine_base.py`)
- **依赖**: `cargo_ship_orbit` Channel (已存在)

#### 1.2.2 实现核心功能
- **坐标转换**: 场景坐标 (x, z) → 地理坐标 (lat, lon)
  - 原点: 31.2304°N, 121.4737°E (上海外海)
  - 转换因子: 1 场景单位 ≈ 0.0001 度 ≈ 11 米
- **process_event 方法**:
  - 接收 `cargo_orbit_telemetry` 类型事件
  - 解析 x, z, angle_deg, distance 字段
  - 转换为 lat/lon
  - 记录到历史缓存
  - 返回处理结果
...
| 风险 | 等级 | 缓解措施 |
|------|------|----------|
| `createCargoOrbitHUD()` 可能已存在 | 中 | 检查现有代码，若已存在则修改而非重写 |
| wabi-sabi 风格与现有 UI 冲突 | 低 | 使用 CSS scoping，避免全局样式污染 |
| 坐标转换精度不足 | 中 | 使用高精度浮点数，验证转换公式 |
| 后端 Channel 注册失败 | 高 | 确保继承正确的基类，实现所有抽象方法 |
| 前端与后端数据同步延迟 | 低 | 使用 WebSocket 实时推送，前端缓存最新数据 |

### 2.2 依赖关系

```
cargo_orbit_telemetry.py
  ├── marine_base.py (MarineChannel 基类)
  ├── cargo_ship_orbit.py (依赖的 Channel)
  └── register_channels.py (Channel 注册)

main.js (前端)
  ├── waves.js (波浪参数)
  ├── three.js (3D 渲染)
  └── cargo_ship_orbit (后端数据源)
```
...
### 3.1 阶段一：前端 HUD 实现 (预计 2 小时)

1. **分析现有代码** (30 分钟)
   - 阅读 `main.js` 中 `createCargoShip()` 和 `animate()` 函数
   - 确认 `createCargoOrbitHUD()` 是否已存在
   - 理解货船位置更新机制
...
### 3.2 阶段二：后端 Channel 实现 (预计 2 小时)

1. **创建 Channel 文件** (30 分钟)
   - 复制 `cargo_ship_orbit.py` 作为模板
   - 修改类名和元数据
   - 实现 `__init__`, `initialize`, `shutdown`, `get_status`
...
3. **实现 process_event** (1 小时)
   - 处理 `cargo_orbit_telemetry` 事件
   - 处理 `get_latest_telemetry` 事件
   - 实现历史记录缓存
   - 注册到 Channel 系统

### 3.3 阶段三：集成测试 (预计 1 小时)
...
### 4.1 前端 HUD 实现 (main.js)

```javascript
// 在 init() 函数中调用
function createCargoOrbitHUD() {
  // 检查是否已存在
  if (document.getElementById('cargo-orbit-hud')) return;
  ...
  hud.innerHTML = `
    <div style="font-size: 11px; letter-spacing: 2px; text-transform: uppercase; 
                color: #8a7a6a; margin-bottom: 8px; border-bottom: 1px solid rgba(180,160,140,0.2); padding-bottom: 6px;">
      ⚓ CARGO ORBIT
    </div>
    <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
      <span style="color: #a09080;">角度</span>
      <span id="cargo-angle" style="font-weight: bold; font-size: 18px;">0.0°</span>
    </div>
    <div style="display: flex; justify-content: space-between;">
      <span style="color: #a09080;">距离</span>
      <span id="cargo-distance" style="font-weight: bold; font-size: 18px;">0.0 m</span>
    </div>
  `;
...
function updateCargoOrbitHUD() {
  ...
  if (!angleEl || !distEl || !state.cargoShip) return;
  
  const cargoPos = state.cargoShip.position;
  const distance = Math.sqrt(cargoPos.x ** 2 + cargoPos.z ** 2);
  const angle = Math.atan2(cargoPos.z, cargoPos.x) * 180 / Math.PI;
...
### 4.2 后端 Channel 实现 (cargo_orbit_telemetry.py)
...
### 5.2 后端验收
- [ ] Channel 正确继承 MarineChannel
- [ ] `process_event` 正确处理 `cargo_orbit_telemetry` 事件
- [ ] 坐标转换精度在 0.0001° 以内
- [ ] 历史记录缓存正常工作
- [ ] Channel 状态查询返回正确信息
- [ ] 单元测试通过
...
1. **性能优化**: 使用 `requestAnimationFrame` 节流 HUD 更新频率
2. **国际化**: 支持多语言显示 (角度/距离标签)
3. **数据持久化**: 将遥测历史保存到数据库
4. **可视化增强**: 在 HUD 中添加迷你轨道图
5. **告警功能**: 当货船距离过近或过远时触发告警
