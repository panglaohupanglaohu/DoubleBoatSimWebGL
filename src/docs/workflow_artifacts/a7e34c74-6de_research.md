...
### 1.1 前端 `src/frontend/digital-twin/main.js`

**文件存在性**: ✅ 已确认存在

**关键代码分析**:
- 该文件是数字孪生前端的主入口
- 需要找到 cargo ship 圆周运动的实现部分
- 需要添加 wabi-sabi 风格的 HUD 覆盖层

**需要修改的点**:
1. 找到 cargo ship 位置更新逻辑（可能通过 WebSocket 或轮询从后端获取）
2. 添加 HUD 元素显示角度和距离
3. 实现 wabi-sabi 风格的 CSS 样式

### 1.2 后端 `src/backend/channels/cargo_ship_orbit.py`

**文件存在性**: ✅ 已确认存在

**关键代码分析**:
- `CargoShipOrbitChannel` 类实现了货船绕双体船的圆周运动
- `tick()` 方法计算货船位置（第 200-260 行）
- `process_event()` 方法处理事件（第 130-190 行）
- 货船位置存储在 `_cargo_ship_position` 字典中（x, y, z 坐标）
...
**需要新增**:
- 需要将 x/y/z 坐标转换为 lat/lon 坐标
- 需要创建新的 `CargoOrbitTelemetryChannel` 来上报遥测数据

### 1.3 后端 `src/backend/channels/marine_base.py`

**文件存在性**: 未在项目上下文中直接显示，但被其他文件引用

**关键分析**:
- `MarineChannel` 是基类，定义了 `process_event()` 接口
- `ChannelPriority` 和 `ChannelStatus` 是枚举类型
- 新 channel 需要继承 `MarineChannel` 并实现 `process_event()`

---

## 2. 技术方案设计

### 2.1 后端：新建 `cargo_orbit_telemetry.py`
...
class CargoOrbitTelemetryChannel(MarineChannel):
    ...
    从 CargoShipOrbitChannel 获取货船位置，
    将 x/y/z 坐标转换为 lat/lon 坐标，
    通过 process_event 上报遥测数据。
    """
    
    name = "cargo_orbit_telemetry"
    description = "货船轨道遥测数据上报"
    version = "1.0.0"
    priority = ChannelPriority.P3
    
    def __init__(self, config=None):
        super().__init__()
        # 参考点：双体船位置对应的经纬度
        self._reference_lat = config.get("reference_lat", 25.0)  # 默认纬度
        self._reference_lon = config.get("reference_lon", 121.0)  # 默认经度
        self._scale_factor = config.get("scale_factor", 0.001)  # 场景单位转经纬度系数
    ...
    def process_event(self, event: dict) -> dict:
        ...
        if event_type == "cargo_position_update":
            # 从 CargoShipOrbitChannel 接收位置更新
            position = event.get("position", {})
            x = position.get("x", 0.0)
            z = position.get("z", 0.0)
            
            # 转换为经纬度
            lat = self._reference_lat + z * self._scale_factor
            lon = self._reference_lon + x * self._scale_factor
            ...
            telemetry = {
                ...
                "distance_to_catamaran": event.get("distance", 0.0),
            }
            
            self._latest_telemetry = telemetry
            
            # 上报遥测数据（可发送到消息队列或日志）
            logger.info(f"🚢 Cargo telemetry: lat={lat:.6f}, lon={lon:.6f}")
            
            return {"status": "ok", "telemetry": telemetry}
        
        elif event_type == "get_telemetry":
            return {"status": "ok", "telemetry": self._latest_telemetry}
        
        return {"status": "ignored", "reason": f"unknown event type: {event_type}"}
```

**坐标转换逻辑**:
- 场景坐标系：x 轴对应经度方向，z 轴对应纬度方向
- 转换公式：`lat = ref_lat + z * scale`，`lon = ref_lon + x * scale`
- `scale_factor` 将场景单位转换为经纬度度数

### 2.2 前端：修改 `main.js` 添加 wabi-sabi HUD

**修改点**:

1. **找到 cargo ship 位置更新逻辑**（约第 100-200 行）
2. **添加 HUD 容器**（HTML 元素）
3. **实现 wabi-sabi 风格样式**（CSS）
4. **更新 HUD 显示内容**（JS 逻辑）

**wabi-sabi 风格设计**:
- 使用手写字体（如 "Noto Serif SC" 或系统衬线字体）
- 颜色：墨色、米色、暗红色
- 边框：不规则的毛笔笔触效果
- 背景：半透明宣纸质感
- 布局：不对称、留白
...
```javascript
// 在 main.js 中添加 wabi-sabi HUD

// 1. 创建 HUD 容器
function createWabiSabiHUD() {
    const hud = document.createElement('div');
    hud.id = 'cargo-telemetry-hud';
    hud.innerHTML = `
        <div class="hud-container">
            ...
            <div class="hud-row">
                ...
                <span class="hud-value" id="hud-angle">0.0°</span>
            </div>
            <div class="hud-row">
                <span class="hud-label">距离</span>
                <span class="hud-value" id="hud-distance">0.0 m</span>
            </div>
            <div class="hud-row">
                <span class="hud-label">纬度</span>
                <span class="hud-value" id="hud-lat">0.000000°</span>
            </div>
            <div class="hud-row">
                <span class="hud-label">经度</span>
...
// 2. 更新 HUD 数据
function updateCargoHUD(data) {
    document.getElementById('hud-angle').textContent = 
        `${data.angle_deg.toFixed(1)}°`;
    document.getElementById('hud-distance').textContent = 
        `${data.distance.toFixed(1)} m`;
    document.getElementById('hud-lat').textContent = 
        `${data.lat.toFixed(6)}°`;
    document.getElementById('hud-lon').textContent = 
        `${data.lon.toFixed(6)}°`;
}
```
...
| 文件路径 | 操作 | 说明 |
|---------|------|------|
| `src/backend/channels/cargo_orbit_telemetry.py` | **新建** | 创建遥测 channel，继承 MarineChannel |
| `src/frontend/digital-twin/main.js` | **修改** | 添加 wabi-sabi HUD 显示角度和距离 |
...
1. **后端 channel 创建**:
   - 已有 `MarineChannel` 基类，继承模式成熟
   - 已有 `CargoShipOrbitChannel` 提供位置数据
   - 坐标转换逻辑简单可靠

2. **前端 HUD 实现**:
   - 纯 CSS/JS 实现，无需额外依赖
   - wabi-sabi 风格可通过 CSS 完全实现
   - 数据更新通过事件监听或轮询

### 4.2 集成风险

1. **数据流**:
   - 需要确保 `CargoShipOrbitChannel.tick()` 调用后触发遥测上报
   - 建议在 `CargoShipOrbitChannel` 的 `tick()` 方法中发送事件到 `CargoOrbitTelemetryChannel`

2. **前端数据获取**:
   - 前端需要从后端获取遥测数据
   - 可以通过 WebSocket 推送或 REST API 轮询

### 4.3 建议的集成方式

**后端集成**（在 `CargoShipOrbitChannel.tick()` 中）：

```python
def tick(self, now=None, channel_registry=None):
    result = super().tick(now, channel_registry)
    
    # 发送遥测事件
    if channel_registry and 'cargo_orbit_telemetry' in channel_registry:
        telemetry_ch = channel_registry['cargo_orbit_telemetry']
        telemetry_ch.process_event({
            'type': 'cargo_position_update',
            'position': self._cargo_ship_position,
            'angle_deg': self.orbit_state.current_angle_deg,
            'heading': self._cargo_ship_heading,
            'distance': self.orbit_config.radius,
        })
...
```javascript
// 在 cargo ship 位置更新回调中添加 HUD 更新
function onCargoPositionUpdate(data) {
    // 更新 3D 场景中的货船位置
    updateCargoShip3D(data.position);
...
1. **新建 `cargo_orbit_telemetry.py`**:
   - 继承 `MarineChannel`
   - 实现 `process_event()` 处理位置更新和遥测查询
   - 实现 x/y/z 到 lat/lon 的坐标转换

2. **修改 `main.js`**:
   - 添加 wabi-sabi 风格的 HUD 容器
   - 实现 HUD 数据更新函数
   - 在 cargo ship 位置更新时调用 HUD 更新

### 5.2 技术风险

- 坐标转换的精度取决于 `scale_factor` 参数，需要根据实际场景校准
- 前端 HUD 的 wabi-sabi 风格需要确保与现有 UI 风格协调
- 数据同步频率需要与后端 tick 频率匹配

### 5.3 建议

1. 先在 `CargoShipOrbitChannel` 中添加事件触发机制
2. 创建 `CargoOrbitTelemetryChannel` 处理遥测数据
3. 最后在前端添加 HUD 显示
4. 进行端到端测试验证数据流
