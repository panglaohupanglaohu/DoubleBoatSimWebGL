...
根据架构设计，我将实现桥楼语义命令系统的完整集成。主要任务是补全 `simple-bridge-chat.js` 中的缺失方法，并确保与后端API的正确集成。

```javascript // src/frontend/digital-twin/simple-bridge-chat.js
/**
 * Simple Bridge Chat - 轻量级舰桥聊天组件
 * 直接集成到数字孪生页面
 */
...
export class SimpleBridgeChat {
  ...
  parseTaskIntent(text) {
    ...
    if (lower.includes('给') && lower.includes('分配任务')) {
      const match = text.match(/给\s*(\S+)\s*(团队)?\s*(PM)?\s*分配任务/);
      if (match) {
        const team = match[1];
        return {
          ...
          team: team,
          description: text,
          priority: lower.includes('紧急') ? 'high' : 'normal'
        };
      }
    }
    ...
    for (const [keyword, intent] of Object.entries(cameraKeywords)) {
      if (lower.includes(keyword.toLowerCase())) {
        return {
          type: 'camera_control',
          intent: intent,
          target: lower.includes('目标') ? 'high_risk_target' : null
        };
      }
    }
  ...
  async dispatchTask(taskIntent) {
    try {
      if (taskIntent.type === 'task_assignment') {
        // 调用智能体团队API分配任务
        ...
        return `✅ 任务已分配给 ${taskIntent.team} 团队\n任务ID: ${result.task_id}\n状态: ${result.status}`;
        
      } else if (taskIntent.type === 'camera_control') {
        // 调用数字孪生相机控制
        const cameraEvent = new CustomEvent('bridge-camera-control', {
          detail: {
        ...
        return `📷 相机控制: ${taskIntent.intent} ${taskIntent.target ? `(目标: ${taskIntent.target})` : ''}`;
      }
      
      return '⚠️ 无法识别的任务类型';
    } catch (error) {
      console.error('任务派发失败:', error);
      return `❌ 任务派发失败: ${error.message}`;
    }
  }
  ...
  async executeOpenBridgeCommand(command) {
    try {
      ...
      console.error('桥楼命令执行失败:', error);
      return {
        error: error.message,
        result: {
          recognized_intent: 'general_assist',
          summary: '命令处理失败，请重试'
        }
      };
    }
    ...
    try {
      ...
      switch (channel) {
        case 'intelligent_navigation':
          endpoint = '/api/v1/ai-native/perception/events?limit=5';
          break;
        case 'intelligent_engine':
          endpoint = '/api/v1/ai-native/phm/status';
          break;
        case 'energy_efficiency':
          endpoint = '/api/v1/ai-native/compliance/status?query=eexi';
          break;
        default:
          throw new Error(`未知的通道: ${channel}`);
      }
      
      const response = await fetch(endpoint);
      
      if (!response.ok) {
        throw new Error(`通道查询失败: ${response.status}`);
      }
      
      return await response.json();
    } catch (error) {
      console.error(`通道 ${channel} 查询失败:`, error);
      return {
        channel: channel,
        error: error.message,
        result: '数据查询失败'
  ...
  async callLLM(prompt, context = '') {
    try {
      // 获取智能体团队配置
      const agentConfig = await this.getAgentTeamConfig();
      
      // 构建完整的提示词
      const fullPrompt = this.buildLLMPrompt(prompt, context);
      ...
      const response = await fetch('/api/v1/agents/chat', {
        ...
        body: JSON.stringify({
          prompt: fullPrompt,
          context: this.shipContext,
          model: agentConfig.default_model || 'deepseek-chat',
          temperature: 0.7
        })
      });
      ...
      const result = await response.json();
      return result.response || result.message || '收到响应但内容为空';
    } catch (error) {
      console.error('LLM调用失败:', error);
      return `🤖 AI响应失败: ${error.message}`;
    }
  }
  ...
  formatCommandResponse(result) {
    if (!result) return '无响应数据';
    
    const { recognized_intent, summary, task_graph, control_state, focus_items } = result;
    
    let response = `📋 **${summary}**\n\n`;
    
    if (task_graph && task_graph.node_count > 0) {
      response += `📊 任务图: ${task_graph.node_count}个节点\n`;
      if (task_graph.execution_order && task_graph.execution_order.length > 0) {
        response += `执行顺序: ${task_graph.execution_order.slice(0, 3).join(' → ')}...\n`;
      }
    }
    
    if (control_state) {
      if (control_state.rcs) {
        const { foil_angle_deg, trim_tab_angle_deg, comfort_target_msdv } = control_state.rcs;
        response += `\n🎮 姿态控制:\n`;
        response += `  T-Foil: ${foil_angle_deg || '--'}°\n`;
        response += `  Trim Tabs: ${trim_tab_angle_deg || '--'}°\n`;
        response += `  舒适目标: ${comfort_target_msdv || '--'} MSDV\n`;
      }
      
      if (control_state.shm) {
        const { fatigue_damage_index, life_remaining_pct } = control_state.shm;
        response += `\n🏗️ 结构健康:\n`;
        response += `  疲劳损伤: ${fatigue_damage_index || '--'}\n`;
        response += `  寿命余度: ${life_remaining_pct || '--'}%\n`;
      }
    }
    
    if (focus_items && focus_items.length > 0) {
      response += `\n🔍 重点关注:\n`;
      focus_items.slice(0, 3).forEach((item, index) => {
        if (typeof item === 'object') {
          response += `  ${item.label || '项目'}: ${item.value || '--'}\n`;
        } else {
          response += `  ${item}\n`;
        }
      });
  ...
  initializeShipContext() {
    ...
    // 尝试从本地存储加载
    try {
      const savedContext = localStorage.getItem('ship_context');
      if (savedContext) {
        this.shipContext = { ...this.shipContext, ...JSON.parse(savedContext) };
      }
    } catch (e) {
      console.warn('无法加载船舶上下文:', e);
    }
  }
  ...
  async updateShipContextFromAPI() {
    try {
      const response = await fetch('/api/v1/ai-native/status/full-pipeline');
      if (response.ok) {
        ...
        // 保存到本地存储
        localStorage.setItem('ship_context', JSON.stringify(this.shipContext));
      }
    } catch (error) {
      console.warn('船舶上下文更新失败:', error);
    }
  }
  ...
  async getAgentTeamConfig() {
    try {
      ...
      console.warn('无法获取智能体团队配置:', error);
    }
   ...
   * 构建LLM提示词
   */
  buildLLMPrompt(prompt, context = '') {
    const shipInfo = `
船舶信息:
- 船名: ${this.shipContext.vessel_name}
- 自主等级: ${this.shipContext.autonomy_level}
- 系统健康: ${this.shipContext.pipeline_health || 'unknown'}
${context}
`;
    
    return `你是一个船舶桥楼助手，负责处理船员的指令和问题。

${shipInfo}

船员指令: "${prompt}"

请以专业、简洁的方式回答，专注于船舶操作、导航、维护和安全相关的问题。如果涉及具体操作，请说明操作步骤和注意事项。`;
  }
}
```

```python // src/backend/api_extensions.py
# -*- coding: utf-8 -*-
"""
API Extensions - 为新 AI Native Channels 添加 API 端点
注意：由于 FastAPI 依赖问题，这里的路由函数将在 main.py 中实现
