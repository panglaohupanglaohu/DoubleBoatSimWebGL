...
**步骤**: 部署 (Deployer)  
**变更内容**:
1. 前端: 在 `src/frontend/digital-twin/main.js` 添加 wabi-sabi 风格 HUD overlay
2. 后端: 新建 `src/backend/channels/cargo_orbit_telemetry.py`
...
- **影响范围**: 仅 `main.js` 文件
- **变更内容**:
  - 新增 `createCargoOrbitHUD()` 函数 (约 80 行)
  - 在 `init()` 中调用该函数
  - 在 `animate()` 循环中更新 HUD 数据
- **风险等级**: 🟢 低风险
- **部署策略**: 就地更新 (hotfix)

### 2. 后端变更 (`cargo_orbit_telemetry.py`)
- **类型**: 新增文件
- **影响范围**: 仅新增文件，不影响现有代码
- **变更内容**:
  - 新建 `CargoOrbitTelemetryChannel` 类
  - 继承 `MarineChannel`
  - 实现 `process_event` 方法
- **风险等级**: 🟢 低风险
- **部署策略**: 就地更新 (新增文件)
...
| # | 文件 | 操作 | 部署策略 |
|---|------|------|----------|
| 1 | `src/frontend/digital-twin/main.js` | 修改 (追加代码) | 就地更新 |
| 2 | `src/backend/channels/cargo_orbit_telemetry.py` | 新建 | 就地更新 |
...
```bash
# 备份当前文件
cp src/frontend/digital-twin/main.js src/frontend/digital-twin/main.js.deploy-bak

# 应用变更 (代码已由开发步骤写入)
# 验证文件完整性
wc -l src/frontend/digital-twin/main.js
```

#### 步骤 2: 后端部署
```bash
# 验证新文件存在
ls -la src/backend/channels/cargo_orbit_telemetry.py

# 验证 Python 语法
python3 -m py_compile src/backend/channels/cargo_orbit_telemetry.py

# 验证导入
python3 -c "from src.backend.channels.cargo_orbit_telemetry import CargoOrbitTelemetryChannel; print('Import OK')"
```
...
# 检查服务状态
sudo systemctl status poseidonx-backend

# 检查日志
tail -f /var/log/poseidonx/backend.log | grep "cargo_orbit_telemetry"
```
...
```bash
# 恢复备份
cp src/frontend/digital-twin/main.js.deploy-bak src/frontend/digital-twin/main.js

# 验证
diff src/frontend/digital-twin/main.js src/frontend/digital-twin/main.js.deploy-bak
```

### 后端回滚
```bash
# 删除新文件
rm src/backend/channels/cargo_orbit_telemetry.py
...
| 检查项 | 状态 | 说明 |
|--------|------|------|
| 前端 HUD 创建函数存在 | ⏳ | 需运行时验证 |
| 后端 Channel 类定义正确 | ✅ | 代码审查通过 |
| 后端 Channel 继承 MarineChannel | ✅ | 代码审查通过 |
| process_event 方法实现 | ✅ | 支持 cargo_orbit_telemetry 事件 |
| 坐标转换逻辑正确 | ✅ | 使用 ORIGIN_LAT/LON + SCENE_TO_DEG |
| 无语法错误 | ✅ | Python 编译通过 |
| 无冲突文件 | ✅ | 仅新增文件 |
