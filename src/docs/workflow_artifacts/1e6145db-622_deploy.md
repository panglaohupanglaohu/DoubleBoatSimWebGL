...
基于前序步骤的产出分析，本次设备健康页面 Agent 的变更属于**大规模功能新增**，具体包括：

#### 后端变更：
- ✅ **新增完整模块**：健康监控 Agent 系统
- ✅ **新增 API 接口**：RESTful API + WebSocket 接口
- ✅ **新增数据模型**：健康指标和告警数据模型
- ✅ **新增依赖包**：psutil, websockets, redis, pandas

#### 前端变更：
- ✅ **新增完整页面**：device-health.html
- ✅ **新增 JavaScript 模块**：健康��控、WebSocket 客户端、图表组件
- ✅ **新增样式文件**：health.css
- ✅ **修改现有文件**：主页面导航、路由配置
...
| 影响维度 | 影响程度 | 说明 |
|----------|----------|------|
| 数据库 | 中 | 新增数据表，需要 schema 迁移 |
| API 接口 | 高 | 新增 /api/health/* 和 /ws/* 接口 |
| 前端路由 | 中 | 新增页面路由，修改导航 |
| 系统依赖 | 高 | 新增 Python 和 JavaScript 依赖 |
| 用户界面 | 高 | 新增完整功能页面 |
...
基于变更分析，本次部署应选择：**🔵 蓝绿部署 (Blue-Green Deployment)**

**选择理由：**
1. **新增完整功能模块**：设备健康监控是独立的新功能
2. **API 接口变更**：新增 REST API 和 WebSocket 接口
3. **数据库 schema 变更**：需要新增数据表
4. **前端页面新增**：新增完整 HTML 页面和 JavaScript 模块
5. **需要灰度验证**：新功能需要逐步验证稳定性

### 2.2 蓝绿部署 vs 热修复对比

| 策略 | 适用场景 | 本次适用性 |
|------|----------|------------|
| 热修复 (Hotfix) | 紧急 bug 修复，小范围代码修改 | ❌ 不适用 |
| 功能部署 (Feature) | 中等规模功能更新，无重大变更 | ❌ 不适用 |
| 蓝绿部署 (Blue-Green) | 大规模功能新增，API/DB 变更 | ✅ **适用** |
...
```bash
# 1. 创建新版本目录
mkdir -p /opt/doubleboat/v2.0/
cp -r /Users/panglaohu/Downloads/DoubleBoatClawSystem/* /opt/doubleboat/v2.0/

# 2. 安装新依赖
cd /opt/doubleboat/v2.0/
pip install -r requirements.txt

# 3. 数据库迁移
python src/backend/db/migrations/create_health_tables.py
...
server {
    listen 80;
    server_name doubleboat.example.com;
    
    # 默认 90% 流量到 Blue
    location / {
        set $upstream blue;
        
        # 10% 流量到 Green (基于 Cookie)
        if ($cookie_canary = "true") {
            set $upstream green;
        }
        
        # 或基于用户ID的灰度
        if ($arg_user_id ~* "^[0-9]*[02468]$") {
            set $upstream green;
        }
    ...
    location /healthcheck {
        proxy_pass http://green/healthcheck;
    }
}
```
...
# 切换脚本：Blue -> Green
#!/bin/bash
# switch_blue_to_green.sh

echo "开始切换到 Green 版本..."

# 1. 停止 Blue 版本
systemctl stop doubleboat-blue

# 2. 更新 Nginx 配置（全量指向 Green）
cp /etc/nginx/sites-available/doubleboat-green /etc/nginx/sites-available/doubleboat
nginx -s reload

# 3. 启动 Green 版本
systemctl start doubleboat-green

# 4. 验证服务
sleep 10
curl -f http://localhost:8001/healthcheck || exit 1

echo "切换完成！Green 版本已上线。"
```
...
# 回滚脚本：Green -> Blue
#!/bin/bash
# rollback_green_to_blue.sh

echo "开始回滚到 Blue 版本..."

# 1. 停止 Green 版本
systemctl stop doubleboat-green

# 2. 恢复 Nginx 配置
cp /etc/nginx/sites-available/doubleboat-blue /etc/nginx/sites-available/doubleboat
nginx -s reload

# 3. 启动 Blue 版本
systemctl start doubleboat-blue

# 4. 验证服务
sleep 10
curl -f http://localhost:8000/healthcheck || exit 1

echo "回滚完成！Blue 版本已恢复。"
```
...
```nginx
# 基于 Cookie 的灰度
add_header Set-Cookie "canary=true; Path=/; Max-Age=3600" if=$canary;

# 基于用户特征的灰度
map $http_user_agent $canary {
    default 0;
    ~*chrome 1;  # Chrome 用户优先体验
}
...
| 部署版本 | v2.0 (设备健康页面 Agent) |
| 部署时间 | 维护窗口 (建议凌晨 2:00-4:00) |
| 预计时长 | 2-4 小时 |
| 负责人 | DevOps 团队 |
...
└── create_health_tables.py
```

#### 后端文件 (修改)：
```
src/backend/main.py              # 新增路由注册
src/backend/api/routes.py        # 添加健康API引用
...
#### 前端文件 (修改)：
```
src/frontend/index.html          # 添加导航链接
src/frontend/js/main.js          # 添加页面路由
src/frontend/css/styles.css      # 添加通用样式
```
...
-- 设备健康摘要表
CREATE TABLE device_health_summary (
...
#### 迁移脚本：
```python
# src/backend/db/migrations/create_health_tables.py
import sqlite3
import sys

def create_tables():
    conn = sqlite3.connect('/var/lib/doubleboat/database.db')
    cursor = conn.cursor()
    
    # 执行上述 SQL 语句
    with open('health_tables.sql', 'r') as f:
        sql_script = f.read()
...
#### Systemd 服务文件 (Green 版本)：
```ini
# /etc/systemd/system/doubleboat-green.service
[Unit]
Description=DoubleBoat System Green Version
...
User=doubleboat
Group=doubleboat
WorkingDirectory=/opt/doubleboat/v2.0
Environment="PATH=/opt/doubleboat/v2.0/venv/bin"
ExecStart=/opt/doubleboat/v2.0/venv/bin/uvicorn src.backend.main:app \
    --host 0.0.0.0 \
    --port 8001 \
    --workers 4 \
    --log-level info
...
#### Nginx 配置：
```nginx
# /etc/nginx/sites-available/doubleboat-green
...
server {
    ...
    # 静态文件
    location / {
        root /opt/doubleboat/v2.0/src/frontend;
        index index.html;
    ...
    # 健康检查
    location /healthcheck {
        proxy_pass http://green_backend/healthcheck;
        access_log off;
    }
}
```
...
#### 健康检查端点：
```python
# src/backend/api/health_routes.py 中添加
@router.get("/healthcheck")
async def health_check():
    """系统健康检查"""
    checks = {
...
```bash
# 1. 停止灰度流量
# 修改 Nginx 配置，100% 流量回 Blue
sudo cp /etc/nginx/sites-available/doubleboat-blue /etc/nginx/sites-available/doubleboat
sudo nginx -s reload

# 2. 停止 Green 服务
sudo systemctl stop doubleboat-green

# 3. 数据回滚（如有必要）
# 备份 Green 版本数据，恢复 Blue 版本数据

# 4. 通知相关人员
echo "已回滚到 Blue 版本，请检查系统状态"
```
...
| 相关方 | 通知时间 | 通知方式 | 内容 |
|--------|----------|----------|------|
| 开发团队 | 部署前 24h | 邮件+Slack | 部署计划、影响范围 |
| 测试团队 | 部署前 12h | Slack | 验证清单、测试环境 |
| 运维团队 | 部署前 6h | 电话会议 | 部署步骤、回滚方案 |
| 产品经理 | 部署前 2h | 邮件 | 功能上线、用户影响 |
| 最终用户 | 部署后 1h | 应用内通知 | 新功能介绍、反馈渠道 |
...
设备健康页面 Agent 功能将于 [时间] 上线，具体安排如下：
...
| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|----------|
| 数据库迁移失败 | 中 | 高 | 1. 提前备份 2. 分步迁移 3. 快速回滚 |
| 新版本性能问题 | 中 | 中 | 1. 灰度发布 2. 性能监控 3. 自动扩容 |
| 依赖包冲突 | 低 | 高 | 1. 虚拟环境隔离 2. 版本锁定 3. 依赖检查 |
| WebSocket 连接问题 | 中 | 中 | 1. 连接池管理 2. 心跳机制 3. 自动重连 |

### 5.2 业务风险

| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|----------|
| 用户不接受新界面 | 低 | 低 | 1. 用户调研 2. 渐进式引导 3. 反馈收集 |
| 数据展示不准确 | 中 | 中 | 1. 数据验证 2. 阈值调整 3. 人工审核 |
| 告警误报过多 | 中 | 低 | 1. 智能过滤 2. 阈值优化 3. 用户配置 |

## 6. 后续优化建议

### 6.1 部署流程优化
1. **自动化部署**：使用 CI/CD 流水线
2. **一键回滚**：完善回滚脚本
3. **监控集成**：与现有监控系统集成
4. **文档自动化**：自动生成部署文档
...
本次设备健康页面 Agent 的部署采用**蓝绿部署策略**，通过灰度发布逐步验证新功能稳定性。部署过程分为四个阶段，包含完整的回滚方案和监控机制。建议在维护窗口执行部署，确保对用户影响最小。

**部署文档路径**：`/Users/panglaohu/Downloads/DoubleBoatClawSystem/docs/deployment/health_agent_deployment.md`
