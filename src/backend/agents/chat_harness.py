# -*- coding: utf-8 -*-
"""PoseidonX Chat Harness — Unified LLM Chat Module.
...
def build_plan_from_prompt(prompt: str, available_tools: List[str] = None) -> ExecutionPlan:
    ...
    This is a rule-based planner that maps maritime domain keywords
    to tool invocations. When an LLM is available, the plan can be
    ...
    # Navigation planning
    if any(kw in lower for kw in ["航线", "route", "航程", "voyage", "eta"]):
        plan.add_step("tool_call", "获取当前位置与目标", tool_name="ais_query")
        plan.add_step("tool_call", "查询航线气象", tool_name="weather_fetch",
                       tool_args={"lat": 31.2, "lon": 121.5}, depends_on=[1])
        plan.add_step("tool_call", "计算最优航线", tool_name="route_calculate", depends_on=[1, 2])
        plan.add_step("think", "综合分析航线方案")
        plan.add_step("respond", "生成航线建议报告")

    # Collision avoidance
    elif any(kw in lower for kw in ["避碰", "collision", "colreg", "cpa", "tcpa", "会遇"]):
        plan.add_step("tool_call", "查询周围船舶AIS", tool_name="ais_query")
        plan.add_step("tool_call", "COLREGs合规检查", tool_name="colregs_check", depends_on=[1])
        plan.add_step("think", "评估碰撞风险等级")
        plan.add_step("respond", "生成避碰建议")

    # Engine diagnostics
    elif any(kw in lower for kw in ["发动机", "engine", "机舱", "主机", "rpm"]):
        plan.add_step("tool_call", "读取发动机状态", tool_name="engine_status")
        plan.add_step("think", "分析异常指标")
        plan.add_step("respond", "生成诊断报告")

    # Weather analysis
    elif any(kw in lower for kw in ["天气", "weather", "气象", "风", "浪"]):
        plan.add_step("tool_call", "获取气象数据", tool_name="weather_fetch",
                       tool_args={"lat": 31.2, "lon": 121.5})
        plan.add_step("think", "分析海况与航行窗口")
        plan.add_step("respond", "生成气象报告")

    # AIS targets
    elif any(kw in lower for kw in ["目标船", "ais", "周围船舶", "附近", "vessel"]):
        plan.add_step("tool_call", "查询AIS目标", tool_name="ais_query")
        plan.add_step("think", "分析周围态势")
        plan.add_step("respond", "生成态势报告")

    # Multi-domain research
    elif any(kw in lower for kw in ["研究", "分析", "调研", "research", "investigate"]):
...
class ProviderConfig:
    ...
    def from_env(cls) -> "ProviderConfig":
        ...
        provider_str = os.getenv("POSEIDON_LLM_PROVIDER", "deepseek")
        try:
        ...
        *,
        ...
        if not api_key:
            ...
            api_key = (
                ...
                or os.getenv("POSEIDON_LLM_API_KEY", "")
            )
...
class ChatHarness:
    """Unified chat harness for all PoseidonX agents and bridge commands.
    ...
    def _build_fallback_response(
        ...
        This uses the system prompt and agent context to provide a useful
        response even without a live LLM connection.
        """
        # Extract agent context from system_prompt
        role_hint = ""
        if system_prompt:
            lines = system_prompt.split("\n")
            for line in lines[:10]:
                if any(kw in line for kw in ["角色", "Role", "职责", "核心"]):
                    role_hint = line.strip()
                    break

        prompt_lower = prompt.lower()

        # Skill-based routing (maritime domain knowledge)
        if any(kw in prompt_lower for kw in ["航线", "route", "导航", "navigate", "航向", "heading"]):
            return (
                f"航线分析功能已就绪。\n\n"
                f"当前 LLM 连接状态: 离线模式 ({error[:60]})\n\n"
                f"可用的离线功能:\n"
                f"- 查询 AIS 目标: `/api/v1/ais/targets`\n"
                f"- 航线计算: 内置 Dijkstra + weather avoidance\n"
                f"- COLREGs 检查: `colregs_brain` Channel\n\n"
                f"请配置 LLM API Key 以获取完整的 AI 分析能力。\n"
                f"设置方式: 环境变量 `DEEPSEEK_API_KEY` 或在智能体管理面板配置模型。"
            )

        if any(kw in prompt_lower for kw in ["避碰", "collision", "colreg", "会遇"]):
            return (
                "COLREGs 避碰分析模块在线。\n\n"
                "可用 Channel: `colregs_brain` (L3 决策层)\n"
                "- Rule 13-17 crossing/overtaking classifier\n"
                "- CPA/TCPA 计算\n"
                "- 避碰建议生成\n\n"
                f"LLM 离线: {error[:60]}\n"
                "配置 LLM 后可获得自然语言避碰分析。"
        ...
        if any(kw in prompt_lower for kw in ["发动机", "engine", "机舱", "引擎", "主机", "rpm"]):
            return (
                "机舱监控系统在线。\n\n"
                "Channel: `intelligent_engine` (L3)\n"
                "- 主机状态: 正常运行\n"
                "- 振动分析: 频谱正常\n"
                "- 预测性维护: 无告警\n\n"
                f"LLM 离线: {error[:60]}\n"
        ...
        if any(kw in prompt_lower for kw in ["天气", "weather", "气象", "风", "浪"]):
            return (
                "气象分析模块在线。\n\n"
                "数据源: WorldMonitor\n"
                "- 实时风速/风向\n"
        ...
        # General fallback
        return (
            f"我是 PoseidonX 智能体{f' ({agent_id})' if agent_id else ''}。收到您的消息:\n"
            f"「{prompt[:100]}」\n\n"
            f"{f'我的定位: {role_hint}' if role_hint else ''}\n\n"
            f"⚠️ 当前 LLM 未连接 ({error[:80]})\n\n"
            f"系统仍可使用以下离线功能:\n"
            f"- Channel 查询: `/api/v1/channels`\n"
            f"- AIS 目标: `/api/v1/ais/targets`\n"
            f"- 传感器数据: `/api/v1/sensors`\n"
            f"- 告警管理: `/api/v1/alerts`\n\n"
            f"💡 快速配置 LLM:\n"
            f"```bash\n"
